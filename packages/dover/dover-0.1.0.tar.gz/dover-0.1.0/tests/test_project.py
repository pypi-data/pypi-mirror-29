...
def test_find_config_file_failure(tmpdir):
    with pytest.raises(project.DoverError) as ex_info:
        cfg_file = project.find_config_file(tmpdir)

    err_msg = 'Dover found no configuration files!'
    assert ex_info.value.args[0] == err_msg


def test_collect_versioned_files():
    # this is using dover package as the test source
    cfg_file = project.find_config_file()
...
def test_collect_versioned_files_failure(tmpdir):
    ...
    err_msg = 'Invalid configuration section: [dover:file:somepath.py]. Path does not exist.'
    assert ex_info.value.args[0] == err_msg
...
def test_versionfile_show_version_lines(sample):
    display = sample.show_version_lines()
    assert display == "None:001  `__version__ = '0.1.0'`"


def test_versionfile_show_version_update(sample):
    display = sample.show_version_update('0.4.0')
    assert display == "None  (0.1.0 -> 0.4.0)"


def test_find_config_file(files):

    config = project.find_config_file(cwd=files.temp_dir)

    assert isinstance(config, Path)
    assert config.name == 'setup.cfg'
...
def test_versionedfile_show_version_lines(files):

    config = project.find_config_file(cwd=files.temp_dir)
    vfiles = project.collect_versioned_files(config)
   
    assert vfiles[0].name == 'setup.py'
    assert vfiles[0].show_version_lines() ==  "setup.py:005  `__version__ = '0.3.0'`" 

    assert vfiles[1].name == 'setup.cfg'
    assert vfiles[1].show_version_lines() ==  "setup.cfg:002  `version = 0.3.0`" 

    assert vfiles[2].name == 'cli.py'
    assert vfiles[2].show_version_lines() ==  "newd/cli.py:002  `__version__ = '0.3.0'`" 

    assert vfiles[3].name == 'README.md'
    assert vfiles[3].show_version_lines() ==  "README.md:000  `#### newd app v0.3.0`" 


def test_assert_versions(files):
...
def test_assert_versions_failure(files):
    ...
    assert ex_info.value.args[0] == ('Not all file versions match:\n\n'
                                     "    setup.py:005  `__version__ = '0.3.0'`\n"
                                     "    setup.py:099  `version = '2.6.1'`\n"
                                     "    setup.cfg:002  `version = 0.3.0`\n"
                                     "    newd/cli.py:002  `__version__ = '0.3.0'`\n"
                                     "    README.md:000  `#### newd app v0.3.0`")


def test_versionedfile_show_version_update(files):

    config = project.find_config_file(cwd=files.temp_dir)
    vfiles = project.collect_versioned_files(config)
   
    assert vfiles[0].name == 'setup.py'
    assert vfiles[0].show_version_update('0.4.0') ==  "setup.py  (0.3.0 -> 0.4.0)" 

    assert vfiles[1].name == 'setup.cfg'
    assert vfiles[1].show_version_update('0.4.0') ==  "setup.cfg  (0.3.0 -> 0.4.0)" 

    assert vfiles[2].name == 'cli.py'
    assert vfiles[2].show_version_update('0.4.0') ==  "newd/cli.py  (0.3.0 -> 0.4.0)" 

    assert vfiles[3].name == 'README.md'
    assert vfiles[3].show_version_update('0.4.0') ==  "README.md  (0.3.0 -> 0.4.0)" 
...
def test_versionedfile_save(files):
    ...
    compare_save_diff(vfiles[0], 
                      files.setup_py_content, 
                      "- __version__ = '0.3.0'\n+ __version__ = '0.4.0'\n")

    compare_save_diff(vfiles[1], 
                      files.setup_cfg_content, 
                      "- version = 0.3.0 \n+ version = 0.4.0 \n")

    compare_save_diff(vfiles[2], 
                      files.cli_py_content, 
                      "- __version__ = '0.3.0'\n+ __version__ = '0.4.0'\n")

    compare_save_diff(vfiles[3], 
