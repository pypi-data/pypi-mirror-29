...
class CwPythonWrapper:
    def __init__(self, botId, botSecret, tick, host):
        self.botId = botId
        self.botSecret = botSecret
        self.tick = tick
        self.host = host
        self.socketIO = SocketIO(self.host, 8080, LoggingNamespace)
    ...
    def on_identification_successful(self, args):
        self.id = args
        identificationBot = IdentificationBot(self.botId, self.botSecret, self.id)
