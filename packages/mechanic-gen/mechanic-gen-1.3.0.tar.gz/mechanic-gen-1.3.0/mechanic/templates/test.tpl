...
class Test{{ cb.test_name }}(TestCase):
    def setUp(self):
        ...
        self.embeddable_attributes = {} # TODO
        self.embeddable_attributes_nested_ids = {} # TODO
        self.POST = {
            'data': {} # TODO
        }
        self.PUT = {
...
# avoid modifying below - generated code at UTC {{ timestamp }} #
    ...
    def test_post_embedded_id_not_found(self):
        for attr, val in self.embeddable_attributes.items():
            post_data = self._get_random_post_data()
            post_data[attr] = str(uuid.uuid4())
            resp = utils.post(BASE_URI, version=VERSION, json=post_data)
            if resp['data'].get('uri'):
                self.uris_to_delete.append(resp['data']['uri'])
            self.assertEqual(resp['code'], 400)

    def test_post_embeddable_objects_but_missing_embed_query_param(self):
        for attr, val in self.embeddable_attributes.items():
            post_data = self._get_random_post_data()
            post_data[attr] = val
            resp = utils.post(BASE_URI, version=VERSION, json=post_data)
    ...
    def test_post_embeddable_objects(self):
        for attr, val in self.embeddable_attributes.items():
            post_data = self._get_random_post_data()
            post_data[attr] = val
            resp = utils.post(BASE_URI,
                              version=VERSION,
                              params={'embed': str(','.join(self.embeddable_attributes.keys()))},
                              json=post_data)
            if resp['data'].get('uri'):
                self.uris_to_delete.append(resp['data']['uri'])
            self.assertEqual(resp['code'], 201)

            if isinstance(post_data[attr], list):
                self.assertEqual(len(post_data[attr]), len(resp['data'][attr]))

                for item in resp['data'][attr]:
                    if item['uri'] not in self.uris_to_delete:
                        self.uris_to_delete = [item['uri']] + self.uris_to_delete

                    # validate embedded object was actually created by getting it
                    get_resp = utils.get(item['uri'], exclude_base=True, version=VERSION)
                    self._validate_get_response(get_resp, get_resp['data']['identifier'])
            elif isinstance(post_data[attr], dict):
                self.assertTrue(resp['data'].get('identifier'))
                self.assertTrue(resp['data'].get('uri'))

                if resp['data'][attr]['uri'] not in self.uris_to_delete:
                    self.uris_to_delete = [resp['data'][attr]['uri']] + self.uris_to_delete

                # validate embedded object was actually created by getting it
                get_resp = utils.get(resp['data'][attr]['uri'], exclude_base=True, version=VERSION)
                self._validate_get_response(get_resp, get_resp['data']['identifier'])

    def test_post_embeddable_objects_nested_ids(self):
        for attr, val in self.embeddable_attributes_nested_ids.items():
            post_data = self._get_random_post_data()
            post_data[attr] = val
            resp = utils.post(BASE_URI,
                              version=VERSION,
                              params={'embed': str(','.join(self.embeddable_attributes_nested_ids.keys()))},
                              json=post_data)
            if resp['data'].get('uri'):
                self.uris_to_delete.append(resp['data']['uri'])
            self.assertEqual(resp['code'], 201)
            ...
            if isinstance(post_data[attr], list):
                ...
                for item in resp['data'][attr]:
                    ...
                    # verify identifier didn't change
                    self.assertTrue(any(item['identifier'] == x['identifier'] for x in post_data[attr]))
            elif isinstance(post_data[attr], dict):
                if resp['data'][attr]['uri'] not in self.uris_to_delete:
                    self.uris_to_delete = [resp['data'][attr]['uri']] + self.uris_to_delete

                # verify identifier didn't change
                self.assertTrue(post_data[attr]['identifier'] == resp['data'][attr]['identifier'])
    ...
    def test_put_embedded_id_not_found(self):
        # first create something to update
        resp = utils.post(BASE_URI, version=VERSION, json=self.POST['data'])
        if resp['data'].get('uri'):
            self.uris_to_delete.append(resp['data']['uri'])
        ...
        for attr, val in self.embeddable_attributes.items():
            put_data = self._get_random_post_data()
            put_data[attr] = str(uuid.uuid4())
            resp = utils.put(BASE_URI + '/%s' % identifier, version=VERSION, json=put_data)
            self.assertEqual(resp['code'], 400)

    def test_put_embeddable_objects_but_missing_embed_query_param(self):
        # first create something to update
        resp = utils.post(BASE_URI, version=VERSION, json=self.POST['data'])
        if resp['data'].get('uri'):
            self.uris_to_delete.append(resp['data']['uri'])
    ...
    def test_put_embeddable_objects(self):
        # first create something to update
        resp = utils.post(BASE_URI, version=VERSION, json=self.POST['data'])
        if resp['data'].get('uri'):
            self.uris_to_delete.append(resp['data']['uri'])
        ...
        for attr, val in self.embeddable_attributes.items():
            put_data = self._get_random_post_data()
            put_data[attr] = val
            resp = utils.put(BASE_URI + '/%s' % identifier,
                             version=VERSION,
                             params={'embed': str(','.join(self.embeddable_attributes.keys()))},
                             json=put_data)
            self.assertEqual(resp['code'], 200)

            if isinstance(put_data[attr], list):
                self.assertEqual(len(put_data[attr]), len(resp['data'][attr]))
                ...
                for item in resp['data'][attr]:
                    ...
                    get_resp = utils.get(item['uri'], exclude_base=True, version=VERSION)
                    self._validate_get_response(get_resp, get_resp['data']['identifier'])
            elif isinstance(put_data[attr], dict):
                self.assertTrue(resp['data'].get('identifier'))
                self.assertTrue(resp['data'].get('uri'))
    ...
    def test_put_embeddable_objects_nested_ids(self):
        # first create something to update
        resp = utils.post(BASE_URI, version=VERSION, json=self.POST['data'])
        if resp['data'].get('uri'):
            self.uris_to_delete.append(resp['data']['uri'])
        ...
        for attr, val in self.embeddable_attributes_nested_ids.items():
            ...
            if isinstance(put_data[attr], list):
                ...
                for item in resp['data'][attr]:
                    ...
                    # verify identifier didn't change
                    self.assertTrue(any(item['identifier'] == x['identifier'] for x in put_data[attr]))
            elif isinstance(put_data[attr], dict):
                if resp['data'][attr]['uri'] not in self.uris_to_delete:
                    self.uris_to_delete = [resp['data'][attr]['uri']] + self.uris_to_delete

                # verify identifier didn't change
                self.assertTrue(put_data[attr]['identifier'] == resp['data'][attr]['identifier'])
