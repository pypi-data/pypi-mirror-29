...
{# #}
{# #}
def {{ method_attr.operationId }}({%- for item in method_attr.parameters %}{% if item.required %}{{ item.name }}{% else %}{{item.name}}=None{% endif %}{%- if not loop.last %}, {% endif %}{% endfor %}):
    return {
        '{{ method_attr.operationId }}': 'check'
    }
        {%- endfor %}
    {%- endif %}
{%- endfor %}
...
class {{ cb.class_name }}({{ cb.base_class_name }}):
    ...
    {# #}
    @staticmethod
    @ns.header({% if method_attr.auth_header %}'Authorization', 'Bearer', required=True{% else %}None{%- endif %})
