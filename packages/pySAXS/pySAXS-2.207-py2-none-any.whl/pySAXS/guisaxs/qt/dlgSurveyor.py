...
EXPERIMENT_PARAMS={'XEUSS':FROM_EDF_HEADER} 




class SurveyorDialog(QtWidgets.QDialog):
    def __init__(self, parent=None, parameterfile=None, outputdir=None,experimentName=None):
        QtWidgets.QWidget.__init__(self, parent)
        #self.ui = dlgSurveyorui.Ui_surveyorDialog()
        self.ui = uic.loadUi(pySAXS.UI_PATH+"dlgSurveyor.ui", self)#
        print experimentName
        if experimentName is not None:
            self.setWindowTitle(str(experimentName)+' - Continuous Radial averaging tool for pySAXS')
        else:
        ...
        if EXPERIMENT_PARAMS.has_key(experimentName):
            self.FROM_EXPERIMENT=EXPERIMENT_PARAMS[experimentName]
        else:
    ...
    def updateListInit(self):
        ...
        if self.FROM_EXPERIMENT is not None:
            headerNames+=self.FROM_EXPERIMENT
