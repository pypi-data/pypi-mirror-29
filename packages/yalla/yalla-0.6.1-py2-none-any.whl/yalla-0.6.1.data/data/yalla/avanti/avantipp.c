...
int nb_work_total, nb_work_current=0, nb_tasks_per_job;
char *masque;
int myrank;
...
master(void)
{
  ...
  for (rank = 1; rank < ntasks; rank++) {
    ...
    MPI_Send(&nb_tasks_per_job, 1, MPI_INTEGER, rank, NB_TASKS_PER_JOB, MPI_COMM_WORLD);
    len=strlen(masque);
...
slave(void)
{
  ...
  MPI_Recv(&nb_tasks_per_job, 1, MPI_INTEGER, 0, NB_TASKS_PER_JOB,
	   MPI_COMM_WORLD, &status);
  ...
  #ifdef DEBUG
  printf("\n===MAESTRO=== ===PROC %d=== masque  /%s/ \n nb_work_total=%d\n nb_tasks_per_job=%d",
	 myrank,masque,nb_work_total,nb_tasks_per_job);
  #endif
...
do_work(unit_of_work_t work)
{
  ...
  sprintf(buffer,masque,work+1);
  sprintf(command,"ibrun -o %d -n %d %s " ,
	  myrank*nb_tasks_per_job,nb_tasks_per_job,buffer);
