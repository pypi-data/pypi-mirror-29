...
def dump_pdf(filepath, outfile=None, pagenos=None, toc=False, codec=None,
             extractdir=None, password=None, debug=0, dumpall=False,
             obj_ids=None):
    """
    Args:
        filepath: "path/to/input.pdf"
        outfile: path for new file; if None, stdout
                 TODO: allow tempfile and/or StringIO
        pagenos: None to extract all, or a list, one-indexed,
                 e.g. [1,2,5] or range(1, 13)
        toc: Output table of contents / outline only in xml format
        codec: None to not process stream (which can be quite large)
               'text': decompressed text, similar to repr()
               'raw': without decompression
               'binary': decompressed binary blob
               When raw or binary, no stream header displayed
                   for ease of saving it to a file
        extractdir: a directory in which to extract embedded files
        password: password for pdf if applicable
        debug: sets debug level; 0 or more
        dumpall: Dump all the objects. By default only the document
                 trailer is output.
        obj_ids: list of PDF object IDs to display

    Returns:
        NOTHING FOR NOW! TODO: allow tempfile and/or StringIO,
                               return string
        If specify outfile, saved to that filename
           otherwise output sent to stdout

    Note:
        Only works in Python 2
        Adapterd from script in PDFminer installation
    ...
    if extractdir is not None:
        proc = extractembedded
        if toc:
            raise Exception, "One of extractdir and toc must be None"
    if toc:
        proc = dumpoutline
    if outfile is None:
        outfp = sys.stdout
    ...
    PDFDocument.debug = debug
    PDFParser.debug = debug
    proc(outfp, filepath, objids, pagenos, password=password,
             dumpall=dumpall, codec=codec, extractdir=extractdir)
