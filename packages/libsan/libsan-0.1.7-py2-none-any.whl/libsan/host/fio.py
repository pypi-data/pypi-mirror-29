...
fio_default_options = {"rw":            "randrw",   #Type of I/O pattern. Supported (read, write, trim, randread, randwrite, rw, randrw, trimwrite)
...
def fio_stress(of, verbose=False, return_output=False, **fio_opts):
    ...
    fio_param = ""
    for key in fio_opts.keys():
        if fio_opts[key]:
            fio_param += "--%s='%s' " % (key, fio_opts[key])

    if "fiojob" in fio_opts.keys():
