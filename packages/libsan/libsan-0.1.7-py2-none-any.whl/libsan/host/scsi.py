...
def query_all_scsi_disks(scsi_disk = None):
    ...
    for disk_id in disk_ids:
        ...
        scsi_info = {"name" : scsi_name,
            ...
            "size"      : size_bytes,
            "size_human": size_mod.size_bytes_2_size_human(size_bytes),
            "state"     : state,
            "timeout"   : timeout,
            "vendor"    : scsi_vendor,
            "model"     : scsi_model,
    ...
    if scsi_disk:
        for disk_id in scsi_disks.keys():
...
def get_hosts(somethings=None):
    ...
    if not scsi_host_ids:
        return None
    scsi_host_ids = array.dedup(scsi_host_ids)
...
def scsi_driver_of_host_id(host_id):
    ...
    if scsi_driver == "(null)" or scsi_driver == "":
        ...
        if pci_id:
            ...
            ret, output = run("lspci -s \"%s\" -v | grep \"Kernel modules:\"" % pci_id, return_output=True, verbose=False)
            if output:
...
def scsi_ids_of_wwid(wwid):
    ...
    for _id in all_scsi_info.keys():
        if all_scsi_info[_id]["wwid"] == wwid:
            if not scsi_ids:
                scsi_ids = []
            scsi_ids.append(_id)
    if scsi_ids:
        scsi_ids = array.dedup(scsi_ids)
    return scsi_ids
...
def get_free_disks(exclude_boot_device=True, exclude_lvm_device=True, exclude_mpath_device=True):
    ...
    all_mp_info = None
    if exclude_mpath_device:
        all_mp_info = libsan.host.mp.multipath_query_all()
        if all_mp_info and "by_wwid" not in all_mp_info.keys():
            #Fail querying mpath, setting it back to None
            all_mp_info = None

    choosed_disks = {}

    for scsi_disk in all_scsi_disks.keys():
        ...
        if all_mp_info and scsi_info["wwid"] in all_mp_info["by_wwid"].keys() and exclude_mpath_device:
