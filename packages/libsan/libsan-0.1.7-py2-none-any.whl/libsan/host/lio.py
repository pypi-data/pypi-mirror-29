...
def lio_query(show_output=False):
    ...
    #lio_field_regex = "\s+o-\\s\(.*?\)\\s$"
    #lio_field_regex = re.compile(r"\s+o-\s(.*?)\s")
            ...
            if tcm_fc_processing_acls:
                ...
                if m:
                    ...
                    if t:
                        ...
                        bs_type = t.group(2)
                        lun_name = t.group(3)
                        #print "INFO: tcm_fc acls: bs_type %s lun_name %s is mapped to %s/%s" % (bs_type, lun_name, tcm_fc_wwn, tcm_fc_init_wwn)
                        details_dict = lio_dict["backstores"][bs_type][lun_name]
                        mapping_dict = {"t_wwpn" : tcm_fc_wwn, "h_wwpn" : tcm_fc_init_wwn, "t_lun_id" : t.group(1), "h_lun_id" : m.group(1)}
                        if "mapping" not in details_dict.keys():
...
def lio_create_backstore(bs_type = None, lun_name = None, lun_size = None,
    ...
    if lun_name not in lio_get_backstores(bs_type).keys():
...
def lio_get_backstores(bs_type = None, lio_dict = None):
    ...
    if "backstores" not in lio_dict.keys():
    ...
    if bs_type not in lio_dict["backstores"].keys():
        return None
...
def lio_get_backstore_details(bs_type, lun_name, lio_dict = None):
    ...
    if lun_name not in bs_dict.keys():
...
def lio_delete_backstore(bs_type = None, lun_name = None):
    ...
    if lun_name  in lio_get_backstores(bs_type).keys():
...
### PSCSI ###
def _lio_create_backstore_pscsi(lun_name, device):
...
def lio_iscsi_add_lun(tgt_iqn, tpg, bs_type, lun_name):
    ...
    \tFalse: If some problem happened
    """
    cmd = "targetcli /iscsi/%s/%s/luns create /backstores/%s/%s add_mapped_luns=false" % (tgt_iqn, tpg, bs_type, lun_name)
    retcode = run(cmd, return_output=False, verbose=True)
    if (retcode != 0):
...
def lio_iscsi_get_luns(tgt_iqn):
    ...
    \tNone if something went wrong
    """
    if tgt_iqn not in lio_get_iscsi_target():
        print "FAIL: %s is not defined on targetcli" % tgt_iqn
        return None

    lio_dict = lio_query()
    if "luns" not in lio_dict["iscsi"][tgt_iqn].keys():
...
def lio_iscsi_create_target_portal(tgt_iqn, tpg, portal_ip, portal_port="3260"):
    ...
    """
    lio_dict = lio_query()
    if "portals" not in lio_dict["iscsi"][tgt_iqn][tpg].keys():
...
def lio_iscsi_map_lun(tgt_iqn, tpg, init_iqn, init_lun_id, bs_type, lun_name):
    ...
    if not lio_iscsi_get_lun_map(tgt_iqn, init_iqn, tpg, tgt_lun_id):
...
def lio_iscsi_unmap_lun(tgt_iqn, init_iqn, tpg, init_lun_id):
    ...
    if not lio_iscsi_get_lun_map(tgt_iqn, init_iqn, tpg, tgt_lun_id):
        _print("FAIL: It seems to have unmapped lun %s, but it did not" % tgt_lun_id)
        return False
    return True
...
def lio_add_iscsi_target(tgt_iqn = None, init_iqn = None, bs_type = "fileio",
    ...
    for target_iqn in new_tgt_iqns:
        ...
        for lun_num in xrange(1, lun_cnt+1):
...
def lio_support_fc_target():
    ...
    if "tcm_fc" not in lio_dict.keys():
...
def lio_delete_fc_target(wwn, lio_dict=None):
...
def lio_get_fc_target_acl(tgt_wwn, lio_dict = None):
    ...
    if "acls" not in lio_dict["tcm_fc"][tgt_wwn].keys():
        #_print("FAIL: %s does not have acls" % tgt_wwn)
        #print lio_dict
        return None

    return lio_dict["tcm_fc"][tgt_wwn]["acls"].keys()
...
def lio_create_fc_target_lun(tgt_wwn, bs_type, lun_name):
    ...
    \tFalse: If some problem happened
    """
    cmd = "targetcli /tcm_fc/%s/luns create /backstores/%s/%s add_mapped_luns=false" % (_wwn_2_tgt_wwn(tgt_wwn), bs_type, lun_name)
    retcode = run(cmd, return_output=False, verbose=True)
    if (retcode != 0):
...
def lio_get_fc_target_luns(tgt_wwn, lio_dict=None):
    ...
    if "luns" not in lio_dict["tcm_fc"][tgt_wwn].keys():
        print "INFO: target %s does not have any LUN\n"
        return None
...
def lio_get_fc_target_lun_id(tgt_wwn, bs_type, lun_name, lio_dict=None):
    ...
    for lun_id in t_luns_dict.keys():
...
def lio_create_fc_target_map_lun(tgt_wwn, init_wwn, init_lun_id, tgt_lun_id, lio_dict=None):
    ...
    cmd = "targetcli /tcm_fc/%s/acls/%s create %s %s" % (_wwn_2_tgt_wwn(tgt_wwn), _wwn_2_tgt_wwn(init_wwn), init_lun_id, tgt_lun_id)
    retcode = run(cmd, return_output=False, verbose=True)
    if (retcode != 0):
...
def lio_fc_target_unmap_lun(tgt_wwn, init_wwn, init_lun_id):
    ...
    if lio_get_fc_target_map_lun(tgt_wwn, init_wwn, tgt_lun_id):
        _print("FAIL: It seems to have unmapped lun %s, but it did not" % tgt_lun_id)
        return False
    return True
...
def lio_get_fc_target_map_lun(tgt_wwn, init_wwn, tgt_lun_id, lio_dict = None):
    ...
    if tgt_lun_id not in init_acls_dict.keys():
...
def lio_get_fc_target_lun_mapping(bs_type, lun_name, lio_dict=None):
    ...
    if "mapping" not in bs_details_dict.keys():
        return None
...
def lio_clean_up_targets(lio_dict=None):
    ...
    for tgt in fc_targets:
        ...
        if not initiators:
            ...
            if not lio_delete_fc_target(tgt, lio_dict=lio_dict):
                success = False
