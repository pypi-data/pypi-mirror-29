...
#            self.san_conf_list["alias"] = san_conf_list["alias"]
        ...
        for san_hw in san_conf_list:
            ...
            if _targets:
                ...
                for target in _targets:
                    if (target in dev_conf_dict["port_of"].keys() or
                        ...
                        ((re.match(t, target) in t for t in dev_conf_dict["port_of"].keys()) and
                         iscsi.is_iqn(target) and dev_conf_dict["model"] == "DELL-EQLOGIC")
                       ):
    ...
    def _load_sw_conf(self, flag_physwitch=None):
        ...
        for san_hw in san_conf_list:
            ...
            for addr in dev_conf_dict["port_of"].keys():
                ...
                if net.is_mac(addr) and switch_obj:
            ...
            if "iscsi_name" in info_dict.keys():
                ...
                if not iscsi.is_target_discovered(new_t_iqn):
                    ...
                    if not sa_val:
                        ...
                        return None
                    if "portal_ip" in sa_val.keys():
                        if not iscsi.discovery_st(sa_val["portal_ip"], ifaces):
    ...
    def setup_iscsi(self):
        ...
        for iface in host_conf["ifaces"].keys():
            ...
            if "mask" in host_conf["ifaces"][iface].keys():
                subnet_mask = host_conf["ifaces"][iface]["mask"]
            if "gateway" in host_conf["ifaces"][iface].keys():
