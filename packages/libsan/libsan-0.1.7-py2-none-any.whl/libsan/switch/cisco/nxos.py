...
class nxos:
    ...
    def _run(self, cmd, return_output=False, verbose=False, invoke_shell = False):
        ...
        print cmd
        ret, output = ssh.run_cmd(session, cmd, return_output = True, verbose = verbose, invoke_shell = invoke_shell)
    ...
    def link_trigger(self, action=None, port_id=None, check_state=True):
        ...
        while attempts < max_attempts:
            ...
            if port_id not in p_state_dict.keys():
            ...
            if attempts == max_attempts:
                _print("FAIL: Failed to bring port %s %s. Current state: %s" % (port_id,
                       action, p_state_dict[port_id]) )
                return False
        return False
    ...
    def port_oscillate(self, port_id, min_uptime, max_uptime, min_downtime,
                      max_downtime, count):
        if (not port_id or min_uptime==None or max_uptime==None or
            min_downtime==None or max_downtime==None or count==None):
    ...
    def mac_2_port_id(self):
        ...
        * 1        90e2.baa3.9528    dynamic   20         F    F  Eth1/10
        ...
        for line in lines:
            m = parse_regex.match(line)
            if m:
                mac = libsan.host.net.standardize_mac(m.group(2))
