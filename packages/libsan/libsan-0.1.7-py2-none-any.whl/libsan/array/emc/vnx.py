...
def _size_human_2_size_netapp(size_human):
    ...
    if not size_mod.size_human_check(size_human):
        _print("FAIL: _size_human_2_size_netapp() - Checking input")
        return None

    m = size_mod.size_human_regex.match((size_human))
    if m.group(2):
        unit = m.group(2)
        unit = unit.replace("i","")
        unit = unit.lower()
...
class vnx:
    ...
    def set_sa_conf(self, sa_conf_dict):
        ...
        if self.sa_conf_dict:
            if "navi_cli_path" in self.sa_conf_dict.keys():
                self.navi_cli_path = self.sa_conf_dict["navi_cli_path"]

            if "navi_scope" in self.sa_conf_dict.keys():
                self.navi_scope = self.sa_conf_dict["navi_scope"]

            if "navi_scope" in self.sa_conf_dict.keys():
                self.navi_scope = self.sa_conf_dict["navi_scope"]

            if "ctrl_storage_pool" in self.sa_conf_dict.keys():
    ...
    def query_all_lun_info(self, recheck = False):
        ...
        for info in getall_info:
            ...
            if m:
                if m.group(1) in self.luns_dict.keys():
            ...
            if m:
                if not lun_info:
                    ...
                    continue
                if ("size" in lun_info.keys() or "size_human" in lun_info.keys()):
                    _print("FAIL: query_all_lun_info() - It looks like we are parsing LUN info for LUN: %s twice" % lun_info["name"])
                    return None
                lun_info["size"] = int(m.group(1)) * self.BLOCK_SIZE_EMC_VNX_VNX
                lun_info["size_human"] = size_mod.size_bytes_2_size_human(lun_info["size"])
                continue
        ...
        for lun_info in self.luns_dict.values():
            lun_info["map_infos"] = []

            if not storage_grp_info:
                continue

            for key in storage_grp_info.keys():
                grp_info = storage_grp_info[key]
                if "map_infos" not in grp_info.keys():
                    continue
                    ...
                    if (grp_map_info["t_lun_id"] == lun_info["lun_id"]):
                        ...
                        if "hba_infos" in grp_info.keys():
                            for hba_info in grp_info["hba_infos"]:
                                ...
                                if "t_wwpn" in hba_info.keys():
                                    map_info["t_wwpn"] = hba_info["t_wwpn"]
                                if "h_wwpn" in hba_info.keys():
                                    map_info["h_wwpn"] = hba_info["h_wwpn"]

                                if "t_iqn" in hba_info.keys():
                                    map_info["t_iqn"] = hba_info["t_iqn"]
                                if "h_iqn" in hba_info.keys():
                        ...
                        #the mapping can be done with only host and target lun IDs
                            if "h_lun_id" in grp_map_info.keys():
                                map_info = {}
                                map_info["st_group_name"] = key
                                map_info["t_lun_id"] = grp_map_info["t_lun_id"]
                                map_info["h_lun_id"] = grp_map_info["h_lun_id"]
    ...
    def lun_create(self, name, size, flag_thinp=False):
        ...
        size_human = size_mod.size_bytes_2_size_human(size)
        size_bytes = size
        ...
        if not capacity or self.ctrl_storage_pool == None:
    ...
    def lun_remove(self, name):
        ...
        unmap_st_grps = []
        if "map_infos" in lun_info_dict.keys():
    ...
    def lun_map(self, name, lun_id=None):
        ...
        for map_info in self.map_info:
            if ("t_wwpn" in map_info.keys() and
                "h_wwpn" in map_info.keys()):
                for st_grp in st_grps_dict.keys():
                    if "hba_infos" not in st_grps_dict[st_grp].keys():
                        continue
                    #_print("DEBUG: st_grp:")
                    #print st_grps_dict[st_grp]
                    for hba_info in st_grps_dict[st_grp]["hba_infos"]:
                        if "h_wwpn" in hba_info.keys() and hba_info["h_wwpn"] ==  map_info["h_wwpn"]:
                            if st_grps_dict[st_grp]["name"] not in st_grps2map:
                                #_print("DEBUG: lun_map")
                                #print hba_info
                                st_grps2map.append(st_grps_dict[st_grp]["name"])

            if ("t_iqn" in map_info.keys() and
                "h_iqn" in map_info.keys()):
                for st_grp in st_grps_dict.keys():
                    if "hba_infos" not in st_grps_dict[st_grp].keys():
                        continue
                    #_print("DEBUG: st_grp:")
                    #print st_grps_dict[st_grp]
                    for hba_info in st_grps_dict[st_grp]["hba_infos"]:
                        if "h_iqn" in hba_info.keys() and hba_info["h_iqn"] ==  map_info["h_iqn"]:
                            if st_grps_dict[st_grp]["name"] not in st_grps2map:
                                #_print("DEBUG: lun_map")
                                #print hba_info
                                st_grps2map.append(st_grps_dict[st_grp]["name"])
        ...
        for my_stgrp in st_grps2map:
            ...
            if lun_id == None:
                _lun_ids = []
            if "map_infos" in st_grps_dict[my_stgrp].keys():
                for map_info in st_grps_dict[my_stgrp]["map_infos"]:
                    if "h_lun_id" in map_info.keys():
                        _lun_ids.append(map_info["h_lun_id"])

            lun_id = libsan.misc.array.lowest_free_number(_lun_ids)
            #_print("DEBUG: lun_map() - map st_grp %s h_lun_id %s t_lun_id %s" % (my_stgrp, lun_id, lun_info_dict["lun_id"]))
    ...
    def lun_unmap(self, name, st_grp_name=None):
        ...
        #make sure all h_wwpns that we need to map are mapped
        luns_dict = self.query_all_lun_info(recheck=True)
        if "map_infos" not in luns_dict[name].keys():
    ...
    def query_all_storage_group_info(self, recheck = False):
        ...
        for info in getall_info:
            ...
            if flag_initiators_begin:
                ...
                if m:
                    if not st_grp_info:
                        _print("FAIL: query_all_storage_group_info() - Does not know to which storage group add HBA info: %s" % (info))
                        continue
                    hba_info = {}
                    ...
                    if "hba_infos" not in st_grp_info.keys():
                    ...
                    for h_info in st_grp_info["hba_infos"]:
                        if (h_info["sp_name"] == hba_info["sp_name"] and
                            ...
                            ("h_wwpn" in hba_info.keys() and h_info["h_wwpn"] == hba_info["h_wwpn"] or
                            "h_iqn" in hba_info.keys() and h_info["h_iqn"] == hba_info["h_iqn"])):
                            found_info = True
                    if not found_info:
            ...
            if flag_map_begin:
                ...
                if m:
                    if not st_grp_info:
                        _print("FAIL: query_all_storage_group_info() - Does not know to which storage group add map: %s - %s" % (m.group(1), m.group(2)))
                        continue
                    map_info = {}
                    map_info["h_lun_id"] = m.group(1)
                    map_info["t_lun_id"] = m.group(2)
                    if "map_infos" not in st_grp_info.keys():
    ...
    def sa_ctrler_t_wwpns(self, ctrler_ip):
        ...
        if self.sa_conf_dict:
            ...
            for key in self.sa_conf_dict.keys():
