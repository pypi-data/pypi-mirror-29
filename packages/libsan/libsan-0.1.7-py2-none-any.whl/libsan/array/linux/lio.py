...
import re #regex
import sys, os
from libsan.misc.size import size_bytes_2_size_human, size_human_check, size_human_2_size_bytes
import libsan.host.ssh as ssh
import libsan.host.scsi as scsi
import libsan.host.conf as conf
...
class lio:
    ...
    def _run(self, cmd, return_output=False, verbose=True, invoke_shell = False):
        session = ssh.connect(self.host, user = self.user, passwd = self.passwd)
        if not session:
    ...
    def _lun_wwn2wwid(self, lun_info_dict):
        """
    ...
    def query_all_lun_info(self, recheck=False):
        ...
        for bs_type in lio_dict["backstores"].keys():
            bs_luns = lio_dict["backstores"][bs_type]
            if not bs_luns:
                #This backstore has no LUN configured
                continue
            for lun in bs_luns.keys():
                lun_info = {}
                lun_info["lun_name"] = lun
                lun_info["backstore_type"] = bs_type
                lun_info["wwid"] = None

                if "wwid" in bs_luns[lun].keys():
                    lun_info["wwid"] = bs_luns[lun]["wwid"]

                if "lun_size" in bs_luns[lun].keys():
                    lun_info["size"] = size_human_2_size_bytes(bs_luns[lun]["lun_size"])
                    lun_info["size_human"] = bs_luns[lun]["lun_size"]

                if "file_path" in bs_luns[lun].keys():
                    lun_info["file_path"] = bs_luns[lun]["file_path"]

                if "mapping" in bs_luns[lun].keys():
    ...
    def lun_create(self, name, size):
        ...
        cmd = "python -c \"import libsan.host.lio as lio; lio.lio_create_backstore(\\\"%s\\\",\\\"%s\\\", \\\"%s\\\")\"" % ("fileio", name, size_bytes)
        ret, output = self._run(cmd, return_output = True, verbose = False)
        if ret != 0:
    ...
    def lun_map(self, name):
        ...
        for map_info in self.map_info:
            if ("t_wwpn" not in map_info.keys() or
                "h_wwpn" not in map_info.keys()):
    ...
    def lun_unmap(self, name, iqn):
        """
        Unmap a LUN to an IQN
        """
        _print("DEBUG: lun_unmap() - TODO!!!")
        return False
    ...
    def _next_free_h_lun_id(self, t_wwpn, h_wwpn):
        ...
        for lun in all_lun_info:
            if "map_infos" not in all_lun_info[lun].keys():
                #LUN is not mapped
                continue
        ...
        return array.lowest_free_number(found_lun_ids)
