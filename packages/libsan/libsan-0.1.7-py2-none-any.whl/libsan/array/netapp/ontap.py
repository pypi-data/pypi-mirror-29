...
class ontap:
    ...
    def _run(self, cmd, ctrler=None, return_output=False, verbose=True):
        ...
        session = ssh.connect(ctrler, user = self.user, passwd = self.passwd)
        if not session:
            _print("FAIL: Could not connect to Storage Array (%s) - user: (%s) pass: (%s)" % (self.host, self.user, self.passwd))
            if return_output:
                return 127, None
            return 127
    ...
    def query_all_lun_info(self, recheck = False):
        ...
        for info in luns_info:
            ...
            if m:
                ...
                lun_info["size_human"] = size_mod.size_bytes_2_size_human(m.group(3))
            ...
            if m:
                ...
                for map in maps:
                    igroup_name = map[0]
                    lun_id = map[1]
                    if igroup_name in igroups_info.keys():
        ...
        for info in igroups_info:
            ...
            if m:
                if not igroup_info:
                    ...
                    continue
                if "members" not in igroup_info.keys():
                    igroup_info["members"] = []
