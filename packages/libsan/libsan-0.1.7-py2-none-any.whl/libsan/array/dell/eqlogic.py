...
import re #regex
import sys, os
#from libsan.misc.size import size_bytes_2_size_human, size_human_check
...
import libsan.array.dell.eqlparse_show as eqlparse_show
...
class eqlogic:
    ...
    def query_members(self, recheck = False):
        ...
        if ret != 0:
            ...
            if output:
                print output
            return None
        self.member_dict = eqlparse_show.parse_show(output)
    ...
    def query_all_luns(self):
        ...
        output_dict = eqlparse_show.parse_show(output)
        if not output_dict:
            return None
    ...
    def _lun_wwid(self, lun_info_dict):
        """
        ...
        if m:
            inversed = m.group(4) + m.group(3) + m.group(2) + m.group(1)
            match1 = m.group(1)
            match2 = m.group(2)
            match3 = m.group(3)
            match4 = m.group(4)
    ...
    def lun_info(self, lun_name):
        ...
        lun_info_dict["size"] = size_mod.size_human_2_size_bytes(lun_info_dict["size_human"])
    ...
    def lun_create(self, name, size, permission="read-write", status="online", sec_size="512"):
        ...
        if not self.san_dev_name:
            ...
            conf.config_add_entry(self.san_dev_name, "iscsi_iqn_%s" % name, info_dict["iscsi_name"], self.san_conf_path)
        return name
    ...
    def lun_remove(self, name):
        ...
        if not self.san_dev_name:
            ...
            conf.config_remove_entry(self.san_dev_name, "iscsi_iqn_%s" % name, self.san_conf_path)
        return True
