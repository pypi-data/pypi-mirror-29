...
from libsan.misc.size import size_human_2_size_bytes
...
def get_disks_of_mpath(mpath_name):
    ...
    if "disk" not in mpath_dict.keys():
        _print("WARN: mpath %s has no disk" % mpath_name)
        return None

    return mpath_dict["disk"].keys()
...
def get_disks_of_mpath_by_wwpn(mpath_name, wwpn):
    ...
    if wwpn:
        if "disk" in mpath_dict.keys():
            port_entries = ["h_wwpn", "t_wwpn"]
            for disk in mpath_dict["disk"].keys():
                for port_entry in port_entries:
                    if port_entry in mpath_dict["disk"][disk].keys():
    ...
    for line in lines:
        ...
        if m:
            ...
            if match_scsi_id:
                ...
                if scsi_host_id in scsi_host_id_2_driver.keys():
                    scsi_driver = scsi_host_id_2_driver[scsi_host_id]

                if scsi_host_id in scsi_host_id_2_h_wwpn.keys():
                    h_wwpn = scsi_host_id_2_h_wwpn[scsi_host_id]

                if h_wwpn:
                    if fc_target_id in fc_target_id_2_t_wwpn.keys():
            ...
            if h_wwpn:
                disk_info_dict["transport_type"] = libsan.host.fc.fc_host_transport_type(libsan.host.fc.fc_host_id_of_wwpn(h_wwpn))
        ...
        if m:
            ...
            continue
    if mpath_name:
        if mpath_name in all_mpath_dict["by_mpath_name"].keys():
...
def mpath_name_of_wwid(wwid):
    ...
    if wwid in mp_devs["by_wwid"].keys():
...
def mpath_names_of_vendor(vendor):
    ...
    for mpath_info in mp_devs["by_mpath_name"].values():
        if "vendor" in mpath_info.keys() and mpath_info["vendor"] == vendor:
...
def get_free_mpaths(exclude_boot_device=True, exclude_lvm_device=True):
    ...
    if not all_mp_info:
        ...
        return None
    if "by_wwid" not in all_mp_info.keys():
        #mpath device was not found
        print all_mp_info.keys()
        return None
    ...
    for mp_wwid in all_mp_info["by_wwid"].keys():
...
def multipath_setup_blacklist():
    ...
    if not all_mp_info_dict:
        return False
    current_wwids = all_mp_info_dict["by_wwid"].keys()
...
def mp_add_blacklist_wwid(wwid):
    ...
    if "blacklist" not in mp_conf.keys():
        mp_conf["blacklist"] = {}

    if "wwids" not in mp_conf["blacklist"].keys():
    ...
    for line in config_str.split("\n"):
        ...
        if m:
            ...
            if key == "device":
                if not section_name:
                    continue
                if "devs" not in config_dict[section_name].keys():
            ...
            if key == "multipath":
                if not section_name:
                    continue
                if "mpaths" not in config_dict[section_name].keys():
        ...
        if m:
            ...
            if current_section == None:
                continue
            if key == "devnode":
                if "devnodes" not in current_section.keys():
            ...
            if key == "wwid" and match_section:
                if "wwids" not in current_section.keys():
...
def _save_config(config_dict, config_file = multipath_conf_path):
    ...
    if "defaults" in config_dict.keys():
        config_str += "defaults {\n"
        for key in config_dict["defaults"].keys():
            config_str += "\t%s %s\n" % (key, config_dict["defaults"][key])
        config_str += "}\n"

    blacklist_sections = ["blacklist", "blacklist_exceptions"]
    for b_section in blacklist_sections:
        if b_section in config_dict.keys():
            config_str += "%s {\n" % b_section

            if "devnodes" in config_dict[b_section].keys():
                for nodes in config_dict[b_section]["devnodes"]:
                    config_str += "\tdevnode \"%s\"\n" % (node)

            if "wwids" in config_dict[b_section].keys():
                for wwid in config_dict[b_section]["wwids"]:
                    config_str += "\twwid \"%s\"\n" % (wwid)

            if "devs" in config_dict[b_section].keys():
    ...
    if "devices" in config_dict.keys():
        config_str += "devices {\n"
        for vendor in config_dict["devices"].keys():
    ...
    if "multipaths" in config_dict.keys():
        config_str += "multipaths {\n"
        for key in config_dict["multipaths"].keys():
...
def mp_add_blacklist_exception_wwid(wwid):
    ...
    mp_conf = mp_query_saved_conf()
    if "blacklist_exceptions" not in mp_conf.keys():
        mp_conf["blacklist_exceptions"] = {}

    if "wwids" not in mp_conf["blacklist_exceptions"].keys():
