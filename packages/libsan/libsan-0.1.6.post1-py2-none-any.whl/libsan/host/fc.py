...
def query_all_fc_disks(host_id=None):
    ...
    fc_disks = {}
    for scsi_id in scsi_disks.keys():
...
def scsi_wwid_of_fc_disks():
    ...
    if not disks_info:
        return None
    wwids = []
    for info in disks_info.values():
        if "wwid" not in info.keys():
            continue
    ...
    for fc_host in fc_hosts:
        ...
        for r_port in r_ports:
            if wwpn_of_rport(r_port) == wwpn:
                if not t_ids:
                    t_ids = []
                if r_port in rport_id_2_target_id_dict.keys():
...
def fc_host_transport_type(scsi_host_id):
    ...
    for key in model_keys:
        if key in host_info.keys():
            if host_info[key] in host_model2transport.keys():
                return host_model2transport[host_info[key]]

    #Second option try to parse model description
    if "model_description" in host_info.keys():
    ...
    if "model_desc" in host_info.keys():
        fc_regex1 = re.compile("Fibre Channel")
        fc_regex2 = re.compile(" FC ")
        fcoe_regex = re.compile("FCoE")
    ...
    if "modeldesc" in host_info.keys():
    ...
    if "protocol" in host_info.keys():
        if host_info["protocol"] == "fc":
            return "FC"
        if host_info["protocol"] == "fcoe":
            return "FCoE"

    if "symbolic_name" in host_info.keys():
        fc_regex = re.compile("Fibre Channel")
        fcoe_regex = re.compile("fcoe")
...
def set_value_rport_parameter(r_port, r_port_param, value):
    ...
    Returns
        ...
        False
    """
    if not r_port or not r_port_param or value == None:
