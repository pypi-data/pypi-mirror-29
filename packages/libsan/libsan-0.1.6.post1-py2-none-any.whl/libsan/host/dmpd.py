...
from libsan.host.lvm import lv_query, lv_activate
...
def _activate_device(vg_name, lv_name):
    ...
    if vg_name + "-" + lv_name not in devices_active:
        ret = lv_activate(lv_name, vg_name)
        if not ret:
...
def thin_repair(source_file=None, source_vg=None, source_lv=None, target_file=None, target_vg=None, target_lv=None, verbose=True):
...
def thin_trim(data_vg, data_lv, metadata_vg=None, metadata_lv=None, metadata_file=None, verbose=True):
    ...
    retcode = run(cmd, verbose=verbose)
    if retcode != 0:
        _print("FAIL: Could not discard free pool space on device %s with metadata device %s." % (data_dev, metadata_dev))
        return False

    return True


def thin_delta(thin1, thin2, source_file=None, source_vg=None, source_lv=None, snapshot=False, verbosity=False, verbose=True):
