...
def dm_query_status(dm_device=None):
    ...
    for device in devices:
        ...
        if dm_type == "linear":
            ...
            for group_nr in xrange(1, int(nr_groups)+1):
                ...
                if not grp_match:
                    ...
                    return None
                if "path_grp" not in dm_status_format_dict.keys():
                ...
                for p_nr in xrange(1, int(nr_paths)+1):
                    ...
                    if not path_match:
                        ...
                        return None
                    if "path" not in path_grp.keys():
                        path_grp["path"] = {}
                    path_grp["path"][p_nr] = {}
                    path = path_grp["path"][p_nr]
                    ...
                    for p_args in xrange(1, int(grp_match.group(4))+1):
                        ...
                        if not path_arg_match:
                            ...
                            return None
                        if "path_status_args" not in path.keys():
                    ...
                    if "path_status_args" in path.keys():
    ...
    if dm_device:
        if dm_device in dev_status_dict.keys():
...
def dm_query_table(dm_device=None):
    ...
    for device in devices:
        ...
        if dm_type == "linear":
            ...
            if dm_table_format_dict["nr_status_feature"] > 0:
                ...
                for i in xrange(dm_table_format_dict["nr_status_feature"]):
            ...
            if dm_table_format_dict["nr_hw_handlers"] > 0:
                ...
                for i in xrange(dm_table_format_dict["nr_hw_handlers"]):
            ...
            remaining_data = ""
            for i in xrange(len(device_params) - param_index):
            ...
            for group_nr in xrange(1, int(nr_groups)+1):
                ...
                if not grp_match:
                    ...
                    return None
                if "path_grp" not in dm_table_format_dict.keys():
                ...
                for path_nr in xrange(1, int(path_grp["nr_paths_grp"])+1):
                    ...
                    if not path_match:
                        ...
                        return None
                    if "path" not in dm_table_format_dict.keys():
                    ...
                    for arg in xrange(1, int(path_grp["nr_path_args"])+1):
    ...
    if dm_device:
        if dm_device in dev_table_dict.keys():
