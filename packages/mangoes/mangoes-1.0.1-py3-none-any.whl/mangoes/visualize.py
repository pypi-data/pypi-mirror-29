...
def plot_tsne(embeddings, words_to_display=None):
    ...
    >>> plt.show()
    """
    emb_2d = mangoes.evaluate.tsne(embeddings)
...
def plot_distances(embeddings, ax, nb_intervals=8, title="Distances between words", scale="symlog"):
    ...
    ax.set_title(title)
    radii = mangoes.evaluate.distances_histogram(embeddings, theta)
...
def plot_isotropy(embeddings, ax, title="Partition function value"):
    ...
    concentration, mean_value, values = mangoes.evaluate.isotropy_from_partition_function(embeddings)
