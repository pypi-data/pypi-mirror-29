...
class Corpus:
    ...
    def __init__(self, content, name=None, language=None, reader=TEXT,
        ...
        self.reader = reader(content, lower, digit)
        ...
        self._params = {"lower": lower, "digit": digit, "name": name, "language": language}
    ...
    def is_annotated(self):
        if self._is_annotated is None:
            self._is_annotated = isinstance(list(self.words_count.keys())[0], tuple)
    ...
    def create_vocabulary(self, attributes=None, filters=None):
        ...
        if attributes:
            words = merge_tokens_count_by_attribute(self.words_count, attributes)
        else:
    ...
    def _count_words(self):
        if issubclass(self.reader.__class__, mangoes.utils.reader.AnnotatedSentenceGenerator):
            # FIXME : the test is not general enough
        ...
        nb_sentences_per_worker = self._nb_sentences//(nb_workers - 1) if self._nb_sentences else None
    ...
    def _count_bigrams(self):
        ...
        nb_workers = 1#multiprocessing.cpu_count() - 1
        data_parallel = mangoes.utils.multiproc.DataParallel(counter,
                                                             _reduce_counter,
                                                             nb_workers=nb_workers)
    ...
    def __setstate__(self, state):
        ...
        self._logger = logging.getLogger("{}.{}".format(__name__, self.__class__.__name__))
        self.reader = self._reader_class(self.content, self.lower, self.digit)
...
def _count_words_raw(sentences, nb_sentences=None):
    ...
    return words_count, real_nb_sentences

def _count_bigrams_raw(sentences, nb_sentences=None):
    bigrams_count = collections.Counter()
    real_nb_sentences = 0

    for sentence in ProgressBar(sentences, total=nb_sentences):
        bigrams_count.update([' '.join(b) for b in nltk.bigrams(sentence)])
        real_nb_sentences += 1

    return bigrams_count, real_nb_sentences
...
def _count_bigrams_annotated(sentences, nb_sentences=None):
    bigrams_count = collections.Counter()
    real_nb_sentences = 0
...
def merge_tokens_count_by_attribute(words_count, attributes):
    filter_token = mangoes.vocabulary.create_token_filter(attributes)
    words = collections.Counter()
    for token in words_count:
        words[filter_token(token)] += words_count[token]
    return words
