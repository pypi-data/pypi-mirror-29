...
class DataParallel:
    ...
    def __init__(self, task, reducer, nb_workers=1):
        self.task = task
    ...
    def run(self, data, *args, **kwargs):
        ...
        if self.nb_workers == 1:
            return self.task(data, *args, **kwargs)

        running_workers = self._start_task_workers(self.nb_workers - 1, *args, **kwargs)
    ...
    def _process_output_queue(self, running_workers):
        ...
        while running_workers > 0:
            ...
            try:
                ...
                running_workers -= 1
                logger.debug("'Mapper' worker n°{} terminated, {} running.".format(worker_id,
                                                                                   running_workers))
    ...
    def _worker(self, worker_id, *args, **kwargs):
        try:
            primary_output = self.task(iter(self.input_queue.get, _Sentinel), *args, **kwargs)
