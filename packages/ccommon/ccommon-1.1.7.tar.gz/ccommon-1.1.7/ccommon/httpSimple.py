...
class HttpSimple():
    ...
    def run(self):
        if self.method == 'get':
            self.rsp = requests.get(url=self.url, data=self.data, headers=self.headers, auth=self.auth)
        else:
            self.rsp = requests.post(url=self.url, data=self.data, headers=self.headers, auth=self.auth)
        self.checkStatus()
