...
def get_opened_issues(jira):
    return {
        str(issue): issue
        for issue in jira.search_issues("assignee = currentUser() and  status in ('In Progress', 'TO TEST')")
    }
...
def parse_issue_key(branch_name):
    ...
    return matched_result.group() if matched_result else None
...
if __name__ == "__main__":
    ...
    print("Tickets available for you:\n" + ",".join(issue_key for issue_key in opened_issues))
