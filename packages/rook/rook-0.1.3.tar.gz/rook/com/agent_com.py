...
class AgentCom(object):
    ...
    def send_rule_status_update(self, status_update):
        self._client.send_rule_status_update(status_update, metadata=self._build_metadata())

    def send_rook_messages(self, messages):
        self._client.send_messages(messages, metadata=self._build_metadata())
    ...
    def _start_new_connnection(self):
        ...
        try:
            self._client.ping(ping_message, timeout=AgentComConfiguration.GRPC_TIMEOUT, metadata=self._build_metadata())
        ...
        command_request = self._client.new_rook_notification(info, metadata=self._build_metadata())
