...
def xmltree2docx(xmltree,docx_filename):
    prettyxml = etree.tostring(xmltree,pretty_print=True)
    with open(os.path.join(handy.__path__,'document/tmp/word/document.xml'),'w+b') as xml_file:
        xml_file.write(prettyxml)
    #
    with ZipFile(docx_filename, "w") as docx_:
        for filename in filenames:
            docx_.write(os.path.join(handy.__path__,'document/tmp/'+filename), filename)
            
def xml2docx(xml_filename,docx_filename):
    shutil.copy(xml_filename, os.path.join(__path__,'document/tmp/word/document.xml'))
    #
    with ZipFile(docx_filename, "w") as docx_:
        for filename in filenames:
            docx_.write(os.path.join(handy.__path__,'document/tmp/'+filename), filename)
