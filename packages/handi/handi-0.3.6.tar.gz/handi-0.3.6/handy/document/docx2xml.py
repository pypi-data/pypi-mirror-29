...
# need to be refined
import site
__path__ = os.path.join(site.getsitepackages()[1],'handy')

def xmltree2docx(xmltree,docx_filename):
    prettyxml = etree.tostring(xmltree,pretty_print=True)
    with open(os.path.join(__path__,'tmp/word/document.xml'),'w+b') as xml_file:
        xml_file.write(prettyxml)
    #
...
def xml2docx(xml_filename,docx_filename):
    shutil.copy(xml_filename, os.path.join(__path__,'tmp/word/document.xml'))
    #
    with ZipFile(docx_filename, "w") as docx_:
        for filename in filenames:
            docx_.write(os.path.join(__path__,'tmp/'+filename), filename)
