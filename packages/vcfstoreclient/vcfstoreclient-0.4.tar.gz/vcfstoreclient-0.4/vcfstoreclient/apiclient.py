...
CACHEFILE = path.join(tempfile.gettempdir(), 'vcfstoreclient.cache')
...
def request_jsonschema():
    ...
    with shelve.open(CACHEFILE) as cache:
        ...
        try:
            ...
            # API is configured to return the schema when an empty body is
            # POST'ed to the /vcfquery resource
            schema = requests.post(
                API_RESOURCE_QUERY, auth=auth, json={}).json()
            cache[key] = schema
        return schema
