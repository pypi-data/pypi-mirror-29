...
class Client(object):
    ...
    def token(self):
        """
        Returns the AOS server active session token if connected, otherwise
        returns None.
        """
        return self.api.headers.get('AUTHTOKEN')
    ...
    def version(self):
        """
        Returns the AOS server version string value if connected, otherwise
        returns None
        """
        return self._version_info['version'] if self._version_info else None

    @property
    def version_info(self):
        """
        Returns the AOS server version-info dict if connected, otherwise
        returns None
        """
    ...
    def fetch_version_info(self):
        ...
        version = get_distribution('apstra-aospy-swagger').version
    ...
    def _init_config(self):
        ...
        if 'port' not in self.config:
            value = os.getenv(Client.ENV['port'])
            if value:
