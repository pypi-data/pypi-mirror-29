...
import apispec_operids


def bugfix_PostApiBlueprintsBlueprintIdProbesRequest(spec):
    # AOS-6405
    schema = spec['definitions'].get('PostApiBlueprintsBlueprintIdProbesRequest')
    if not schema:
        return
...
def bugfix_facade_resource_groups(origin_spec):
    ...
    path = spec_paths.get("/api/blueprints/{blueprint_id}/resource_groups")
    if path:
    ...
    path = spec_paths.get("/api/blueprints/{blueprint_id}/resource_groups/{resource_type}/{group_name}")
    if path:
...
def patch_aos_2_1_platform(spec):
    ...
    apispec_operids.autoassign_operationids(spec)
    apispec_operids.patch_aos_2_1_platform_operation_ids(spec)
...
def patch_aos_2_1_reference_design(spec):
    ...
    apispec_operids.autoassign_operationids(spec)
    apispec_operids.patch_aos_2_1_reference_design_operation_ids(spec)
