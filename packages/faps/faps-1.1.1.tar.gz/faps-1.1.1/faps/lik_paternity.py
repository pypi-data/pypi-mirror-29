...
def lik_paternity(offspring, mothers, males, mu, mother_index=None):
    """
    ...
    lik_missing = 1- np.copy()
    with np.errstate(divide='ignore'):
        lik_trans = np.log(lik_trans) # convert to log.
    ...
    # Correct for dropouts
    lik_missing[dropout_mask < 0] = 0 # set loci with one or more dropouts to zero to allow summing.
    lik_missing = lik_missing.sum(2) # sum over loci.
    lik_missing = lik_missing - np.log(males.nloci - n_dropouts) # scale by the number of valid SNPs.


    return (lik_trans, lik_missing)
