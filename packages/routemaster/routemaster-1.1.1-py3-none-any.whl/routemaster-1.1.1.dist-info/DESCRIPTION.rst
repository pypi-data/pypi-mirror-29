...
a `label <docs/glossary.md>`_\ , the reason for it not progressing to the next state, its
...
`metadata <docs/glossary.md>`_ into the system that *may* change the state of a label,
...
States in a state machine can either be an `\ *action* <docs/glossary.md>`_ or a
`\ *gate* <docs/glossary.md>`_.
...
condition <docs/glossary.md>`_ is met. Routemaster will evaluate the exit condition
