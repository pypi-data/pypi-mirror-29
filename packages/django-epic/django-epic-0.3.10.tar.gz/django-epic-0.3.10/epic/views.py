...
INVENTORY_TYPE_COMPLETE = 'COMPLETE'
INVENTORY_TYPE_PARTIAL = 'PARTIAL'
...
def warehouse_add_inventory(request, pk):
    # default to creating a complete inventory (i.e., when saving the
    # inventory, all parts not listed will be zeroed out by creating
    # additional Deltas for those unlisted parts).
    return inventory_edit(request, None, warehouse=pk,
                          inventory_type=INVENTORY_TYPE_COMPLETE)
...
class Inventory_Edit_Form(ModelForm):
    inventory_type = forms.ChoiceField(choices=[(INVENTORY_TYPE_COMPLETE,
                                                 'complete'),
                                                (INVENTORY_TYPE_PARTIAL,
                                                 'partial')],
                                       required=False,
                                       help_text='When saving as a partial '
                                       'inventory, the quantity of only the '
                                       'listed parts will be updated.  '
                                       'When saving as a complete inventory, '
                                       'additional entries are created '
                                       'to zero out all other parts in the '
                                       'warehouse as of the inventory date.')

    def __init__(self, *args, **kwargs):
        inventory_type = kwargs.pop('inventory_type', INVENTORY_TYPE_PARTIAL)
        ...
        self.fields['inventory_type'].initial = inventory_type,
...
# Default to partial inventory because we don't want to accidentally
# create additional zero Deltas for (now) unlisted parts, unless the
# user explicitly indicates that's what s/he wants by switching to a
# complete inventory.
@transaction.atomic
@permission_required(perms.EDIT)
def inventory_edit(request, pk, warehouse,
                   inventory_type=INVENTORY_TYPE_PARTIAL):
    def prev_url(pk):
        if pk is None:
            return reverse('epic:warehouse_list')
    ...
    if request.method == 'POST':
        ...
        if inv_form.is_valid() and item_form.is_valid():
            ...
            inv_type = inv_form.cleaned_data.get('inventory_type')
            if inv_type == INVENTORY_TYPE_COMPLETE:
                stock = get_stock(warehouses=[inventory.warehouse_id],
                                  as_of_date=inventory.ts)
                for p in stock.warehouse[inventory.warehouse_id]:
                    d = Delta(part_id=p, is_absolute=True, adj=0,
                              txtn=inventory,
