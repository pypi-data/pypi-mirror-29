...
template<typename T, class = typename std::enable_if<std::is_convertible<T,float>::value>::type>
float asFloat(const T x) {
	if(std::is_floating_point<T>::value) return x;
	else {
		float bound=std::numeric_limits<T>::max();
		return float(x)/bound;
	}
}
...
*/
	...
	template<typename T, class = typename std::enable_if<std::is_convertible<T,float>::value>::type>
	std::pair<float,float> nextFloatPair() {
		auto pair=nextPairOf<T>();
		return std::make_pair(asFloat<T>(pair.first),asFloat<T>(pair.second));
	};
