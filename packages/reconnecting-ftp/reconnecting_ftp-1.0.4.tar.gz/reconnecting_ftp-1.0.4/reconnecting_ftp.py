...
import ftplib
from typing import Optional, Callable, TypeVar, List  # pylint: disable=unused-import
...
class Client:
    ...
    def abort(self):
        ...
        return self.__wrap(method=lambda conn: conn.abort())

    def sendcmd(self, cmd):
        """ See ftplib documentation """
        return self.__wrap(method=lambda conn: conn.sendcmd(cmd))

    def voidcmd(self, cmd):
        """ See ftplib documentation """
        return self.__wrap(method=lambda conn: conn.voidcmd(cmd))

    def sendport(self, host, port):
        """ See ftplib documentation """
        return self.__wrap(method=lambda conn: conn.sendport(host, port))

    def sendeprt(self, host, port):
        """ See ftplib documentation """
        return self.__wrap(method=lambda conn: conn.sendeprt(host, port))

    def makeport(self):
        """ See ftplib documentation """
        return self.__wrap(method=lambda conn: conn.makeport())

    def makepasv(self):
        """ See ftplib documentation """
        return self.__wrap(method=lambda conn: conn.makepasv())

    def ntransfercmd(self, cmd, rest=None):
        """ See ftplib documentation """
        return self.__wrap(method=lambda conn: conn.ntransfercmd(cmd, rest))
    ...
    def mlst(self, filename: str, facts: Optional[List[str]] = None):
        ...
        return self.__wrap(method=lambda conn: mlst(connection=conn, filename=filename, facts=facts))
