...
class ConfidenceInterval:
    ...
    @staticmethod
    def mean(xbar, stdev, n, alpha = 0.05, isSigmaKnown = True, precision = 4):

        stderror = stdev/np.sqrt(n)
        if(n >= 30 and isSigmaKnown):
            score = NormalDistribution(0, 1).getZscore(alpha, "right")
        elif(n >= 30 and not isSigmaKnown):
            score = NormalDistribution(0, 1).getZscore(alpha, "right")
        elif(n < 30 and isSigmaKnown):
            score = NormalDistribution(0, 1).getZscore(alpha, "right")
        else:
            score = tDistribution(n-1).getTscore(alpha, "right")
    ...
    def variance(n, stdev, alpha = 5, precision = 4):
        ...
        p = alpha / 100.0
        df = n - 1
        chisq1 = ChiSquareDistribution(n - 1).getChi2score(alpha, "left")
        chisq2 = ChiSquareDistribution(n - 1).getChi2score(alpha, "right")
