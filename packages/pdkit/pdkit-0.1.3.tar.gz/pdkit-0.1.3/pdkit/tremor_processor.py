...
class TremorProcessor:
    ...
    def fft_signal(self, window=WINDOW):
        # next we perform fft on the signal using a hanning window
    ...
    def tremor_amplitude(self, lower_frequency=LOWER_FREQUENCY_TREMOR, upper_frequency=UPPER_FREQUENCY_TREMOR):
        # finally we extract the fft components and sum the ones from lower to upper freq as per [1]
    ...
    def tremor_amplitude_by_welch(self, window=WINDOW, lower_frequency=LOWER_FREQUENCY_TREMOR,
        ...
        # use the Welch method [3] to obtain the power spectral density
        # this is a robust alternative to using fft_signal & calc_tremor_amplitude
        frq, Pxx_den = signal.welch(self.magnitude_sum_acceleration, SAMPLING_FREQUENCY, nperseg=window)
