...
class TremorProcessor:
    ...
    def resample_signal(self, sampling_frequency=SAMPLING_FREQUENCY):
        ...
        fx = interpolate.interp1d(self.time_difference, self.data_m[:, 1])
        fy = interpolate.interp1d(self.time_difference, self.data_m[:, 2])
        fz = interpolate.interp1d(self.time_difference, self.data_m[:, 3])
        ...
        df_resampled.x = fx(new_timestamp)
        df_resampled.y = fy(new_timestamp)
        df_resampled.z = fz(new_timestamp)
        df_resampled.mag_sum_acc = np.sqrt(df_resampled.x ** 2 + df_resampled.y ** 2 + df_resampled.z ** 2)
        # Overwrite the data frame
        self.data_frame = df_resampled
