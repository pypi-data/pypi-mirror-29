...
class Ryanair():
    ...
    def __init__(self):
        ...
        self.iatadf = df.reindex(df.index.drop(0))

        if self.arrivalairports == ['*']:
            self.arrivalairports = list(self.iatadf.IATA)

        elif len([x for x in self.arrivalairports if x in list(self.iatadf.Country)]) > 0:
            self.arrivalairports = [self.iatadf.IATA[i] for i,x in enumerate(list(self.iatadf.Country)) if x in self.arrivalairports]
