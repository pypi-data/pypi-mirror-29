...
class SupervisedRNNModel(object):
    ...
    def predict(self, seq, sess):
        ...
        predictions = sess.run(feed_dict=feed_dict, fetches=fetches)

        #ction_probs = np.max(likelihoods, axis=2)
        #est_actions = np.argmax(likelihoods, axis=2)
