...
class SupervisedRNN(object):
    def __init__(self, optimizer, params, scope):
        ...
        with tf.variable_scope(self.scope):
            ...
            ####### Likelihoods #######
            hashed_indices = bloom_embedding.forward(tf.range(self.item_dim))
            hashed_likelihoods = tf.gather(
                params=tf.reshape(self.sigmoids, [-1, self.embedding_dim]),
                indices=hashed_indices,
                axis=1)
            self.likelihoods = tf.reduce_prod(hashed_likelihoods, axis=2)
            self.likelihoods = tf.reshape(self.likelihoods, shape=[-1, self.unfold_dim, self.item_dim])
            self.predictions = tf.argmax(self.likelihoods, axis=2)
