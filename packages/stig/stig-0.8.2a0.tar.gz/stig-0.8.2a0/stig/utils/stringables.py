...
class StringableMixin():
    ...
    #     cls = type(self)
    #     value = self
    #     print('Calling %r(%r, %r)' % (cls, value, new_kwargs))
    #     return type(self)(self, **new_kwargs)
        ...
        def _get_instance(cls, *value, **kwargs):
            ...
            for const in cls._constructors:
                try:
                    ...
                    log.debug('Not a valid %s: %s', const.typename, e)
                    errors.append(str(e))
                except TypeError as e:
                    log.debug('Not a %s: %s', const.typename, e)
                    errors.append('Not a %s' % const.typename)
            log.debug('Invalid %s value: %r, %r', cls.__name__, value, type(value[0]))
            raise ValueError('; '.join(errors))

        @classproperty
        def typename(cls):
            return ' or '.join((s.typename for s in cls._subclses
                                if s.typename is not None))

        @classproperty
        def syntax(cls):
            log.debug('Getting syntax from %r', cls._constructors)
...
class Option(str, StringableMixin):
    ...
    def __new__(cls, value, *, options=options, aliases=aliases):
        ...
        for o in options:
            log.debug('%r,%r == %r,%r -> %r', value, type(value), o, type(o), value == o)

        if not any(value == option for option in options):
            log.debug('%r, %r is not in %r', value, type(value), options)
            if len(options) == 0:
                raise RuntimeError('No options provided to Option')
            elif len(options) == 1:
...
class _NumberMixin(StringableMixin):
    ...
    def __new__(cls, value, *, unit=unit, convert_to=convert_to, prefix=prefix,
        ...
        # print('Making number from: %s, %s, %s, %s' % (value, unit, convert_to, prefix))
