...
from intermake.engine.constants import EDisplay, EThread
...
from mhelper import Logger, SwitchError, ansi_format_helper, override, string_helper, virtual
...
class GuiHost( PluginHost ):
    ...
    def __invoke_unmanaged( self, plugin: Plugin, args: ArgsKwargs ) -> Optional[object]:
        ...
        try:
            ...
            async_result = AsyncResult( plugin.name, result, None, traceback.format_exc(), None, plugin )
            return result
        except Exception as ex:
            async_result = AsyncResult( plugin.name, None, ex, traceback.format_exc(), None, plugin )
            raise
        finally:
        ...
        def __init__( self, host, window, plugin: Plugin, args: ArgsKwargs, num_threads: int ):
            ...
            for i in range( num_threads ):
                try:
                    ...
                    # Bee failed to start
                    self.bee_finished( None, AsyncResult( str( plugin ), None, ex, traceback.format_exc(), ["Worker thread failed to start."], plugin ) )
        ...
        class __BusyBee( QThread, IProgressReceiver ):
            ...
            def run( self ) -> None:  # WORKER
                ...
                try:
                    ...
                    result = AsyncResult( self.__plugin.name, true_result, None, None, self.__mandate.get_message_records(), self.__plugin )
                except TaskCancelledError as ex:
                    result = AsyncResult( self.__plugin.name, None, ex, traceback.format_exc(), self.__mandate.get_message_records(), self.__plugin )
                except Exception as ex:
                    result = AsyncResult( self.__plugin.name, None, ex, traceback.format_exc(), self.__mandate.get_message_records(), self.__plugin )
