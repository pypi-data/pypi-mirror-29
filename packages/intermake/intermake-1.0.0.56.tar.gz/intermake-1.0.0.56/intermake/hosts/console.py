...
from intermake.hosts.base import ERunMode, PluginHost, RunHostArgs, ERunStatus
...
class ConsoleHost( PluginHost, IProgressReceiver ):  # this host is always single-threaded so it acts as its own progress receiver
    ...
    def on_run_host( self, args: RunHostArgs ):
        ...
        if run_mode == ERunMode.ARG:
            ...
            # This host does not "run", but we do import the names into the global namespace
    ...
    def call_virtual_run( self, plugin: Plugin, args: ArgsKwargs ) -> Optional[object]:
        ...
        * Raises exceptions in case of error, since everything is verbatim
        ...
        try:
            ...
            async_result = AsyncResult( plugin.name, result, None, None, mandate.get_message_records(), plugin )
        except BaseException as ex:
            async_result = AsyncResult( plugin.name, None, ex, exception_helper.full_traceback(), mandate.get_message_records(), plugin )
