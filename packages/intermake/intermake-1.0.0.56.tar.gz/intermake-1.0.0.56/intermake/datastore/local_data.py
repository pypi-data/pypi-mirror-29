...
from mhelper import file_helper, io_helper, SimpleProxy, PropertySetInfo, exception_helper, Logger
...
class _AutoStore:
    ...
    def __read_item( self, key ):
        ...
        try:
            ...
            autostore_warnings.append( exception_helper.full_traceback() )
            _LOG( "read {} = failure", key )
            warnings.warn( "Failed to restore settings from «{}» due to the error «{}: {}». Use `autostore_warnings` to obtain the full traceback.".format( file_name, type( ex ).__name__, ex ), UserWarning )
            return None
