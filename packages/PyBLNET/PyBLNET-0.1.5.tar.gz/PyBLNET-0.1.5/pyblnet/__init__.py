...
params = (
                ...
                # TODO self.config_core.get('year_format')
                ('assumption', 'DateOrder_**Day.Month.Year--'),
                # alternative 'DateOrder_**Month.Day.Year--'
...
class BLNET(object):
    ...
    def set_digital_value(self, id, value):
        ...
        if id < 1: raise ValueError('Device id can\'t be lesser than 1')
        # ensure to be logged in
        if not self.log_in(): return False
        
        # transform input value from 'EIN' or 'AUS'
        if value == 'AUTO':
