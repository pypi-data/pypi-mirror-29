...
class DataService:
    ...
    def __init__(self, username=None, password_hashed=None):
        if not username:
            ...
            password_hashed = to_md5(user_config["password"])
            print("a",username, password_hashed)
    ...
    def post(self, url, **kwargs):
        ...
        request = HTTPRequest(url, "POST", header, json.dumps(kwargs))
        try:
            ret = self.client.fetch(request).body
            return json.loads(ret)
        except JSONDecodeError:
    ...
    def get_match(self, match_id):
        ...
        ret = None if not response else response[0]
    ...
    def get_positions(self, match_id, time=None, virtual_camera_id="-1", source="human"):
        ...
        return self.post(url, virtualCameraId=virtual_camera_id, lastModified=last_modified, source=source)
    ...
    def push_labels(self, match, start_index=0, virtual_camera_id="-1", source="human"):
        ...
        message_body= self._create_label_body(match.match_id, match.positions, start_index, virtual_camera_id, source)
    ...
    def _create_label_body(match_id,  position_list, start_index=0, virtual_camera_id="-1" ,source="human"):
        ...
        for i in range(start_index, len(position_list)):
            ...
            message_body["positions"][-1]["timestamp"]= str(position_list.positions[0])
            message_body["positions"][-1]["x"]= str(position_list.positions[1])
            message_body["positions"][-1]["y"]= str(position_list.positions[2])
            message_body["positions"][-1]["zoom"]= str(position_list.positions[3])
            message_body["positions"][-1]["source"]= str(source)
        return message_body

    def pull_labels(self, match, time_from=None):
        """
        Updates the labels of a match. Will clear all locally created labels.
        ...
        # Get positions
        positions = sorted(self.get_positions(match_id, time=time_from),
                           key=lambda k: k['timestamp'])
        highest_timestamp = int(positions[-1]["timestamp"]) + 1
        label.positions = np.zeros((highest_timestamp, 7), dtype=np.float32)
        for p in positions:
