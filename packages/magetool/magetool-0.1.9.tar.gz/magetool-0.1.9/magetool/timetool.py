...
#UTC格式的时间转换为本地时间
def utcStrTimeToLocalTimeStr(tstr,UTC_FORMAT='%Y-%m-%dT%H:%M:%S.%fZ'):
    t1 = datetime.datetime.strptime(utc, UTC_FORMAT)
    lt1 = utc2local(t1)
    tFormat = r'%Y-%m-%d %H:%M:%S'
    return lt1,tFormat

def utcStrTimeToTime(tstr,UTC_FORMAT='%Y-%m-%dT%H:%M:%S.%fZ'):
    ldate,dformat = utcStrTimeToLocalTimeStr(tstr,UTC_FORMAT)
