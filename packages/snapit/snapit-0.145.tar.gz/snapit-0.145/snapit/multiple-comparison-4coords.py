...
def main():
    ...
    for input_file_File_name_base_number in input_file_File_name_base_number_list:
        ...
        dataDict['exonLength'] = [int(exon1End) - int(exon1Start)
                                  for exon1End, exon1Start in zip(dataDict['exonEnd'], dataDict['exonStart'])]

        # Get exon sequences from dictionary
        print("\nGetting exon sequences...")
        junction = 30
        dataDict['exonSequences'] = [
            [str(Chromosome_to_Sequence_dict[chromosome][int(exon0End) - junction:int(exon0End)]).upper(),
             str(Chromosome_to_Sequence_dict[chromosome][int(exon1Start):int(exon1End)]).upper(),
             str(Chromosome_to_Sequence_dict[chromosome][int(exon2Start):int(exon2Start) + junction]).upper()]
            for chromosome, exon0End, exon1Start, exon1End, exon2Start
            in zip(dataDict['chr'], dataDict['upstreamEE'],
        ...
        print("\nQuality control:\n\tChecking for faulty reverse complements...")
        ...
        dataDict['exon1FrameShift'] = [exonLength % 3 != 0 for exonLength in dataDict['exonLength']]
        ...
        # Map to UniProtKb topological annotation from dictionary
        ...
        iterables = [
            (exonSequences[0], exonSequences[1], exonSequences[2], strand, index, 0.0001, 30, extracellular, file_folder)
        ...
        with Pool(100) as p:
            ...
            filtered = full[a & b & d & e & g]
            extracell = full[a & b & d & e & g & h]
            manual = full[a & b & d & e & g & i]

            # Only keep relevant columns
            keyList = ['ENSG', 'GeneName', 'description', 'UniProtKb', 'strand',
                       'upstreamEE',
                       'exonStart', 'exonEnd', 'downstreamES', 'exonLength', 'subcell',
                       'exon1FrameShift',
