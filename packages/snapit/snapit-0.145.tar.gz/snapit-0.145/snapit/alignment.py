...
import subprocess
from .sequences import makeFasta, translate_dna
...
def BLASTp(query, index, file_folder):
    ...
    :return: List of lists of BLAST hit, expected score, query sequence for every sequence in query (list[list[str, float, str], ...])
    """
    # Make temp folder
...
def UTR_test_and_find_best_match(exon0Seq, exon2Seq, CDS, matching_aa_seq):
    ...
    :param exon0Seq: DNA sequence of upstream exon (str)
    :param exon2Seq: DNA sequence of downstream exon (str)
    :param CDS: CDS sequence of protein (str)
    :param matching_aa_seq: AA sequence of protein (str)
    :return: List of aligment score, start index, end index, UTR5, UTR3, CDS for splicing event (list[float, int, int, bool, bool, str])
    """
    output_match_list = []
    # Translate CDS
...
def localizeCassetteExon(upExonEnd, downExonStart, extraDomains, notAligned):
    ...
    :param notAligned: Exons that did not align to CDS (str)
    :return: Exons that localize to the extracellular/membrane domain (str)
    """
    exonsMatched = "N/A"
...
def findIsoform(seq0, seq1, seq2, strand, index, threshold, junction, extracellular, file_folder):
    """
    Find possible parent isoforms that display splicing pattern

    :param seq0: DNA sequence of upstream exon (str)
    :param seq1: DNA sequence of cassette exon (str)
    :param seq2: DNA sequence of downstream exon (str)
    :param strand: '+' or '-' strand (str)
    :param index: ID number of splicing event (int)
    ...
    :return: List of lists of possible BLAST hits when cassette exon is spliced in or skipped (list[list[str, ...], list[str, ...]])
    """
    ...
    if len(seq0) >= 3 and len(seq1) >= 3 and len(seq2) >= 3 and extracellular:
        try:
            ...
            isoform1_list = []
            blast = BLASTp(possibleAASeqsInc, index, file_folder)
            try:
            ...
            isoform2_list = []
            blast = BLASTp(possibleAASeqsSkip, index, file_folder)
            try:
                eScore = defaultdict(list)
                for x in blast:
                    eScore[x[0], x[2]].append(x[1])
