"""
multiple-comparison-command.py
...
def main():
    parser = argparse.ArgumentParser(prog='SNAP-IT', description='Run SNAP-IT from a single command-line.')
    parser.add_argument('-i', dest='input_file', required=True, help='The input file name from rMATS')
    parser.add_argument('-g', dest='genome_build', choices=['19', '37', '38'], required=True,
                        help='The reference genome build used in rMATS:\n\'38\' - GRCh38/hg38\n\'37\' or \'19\' - GRCh37/hg19')
    parser.add_argument('-t', dest='threshold', default='0.9',
                        help='The percent match threshold for aligning exons to CDS:\n(0.0 - 1.0)')
    # if args.update is not found in annotations folder, default to update to newest annotation
    parser.add_argument('-e', dest='e_score', default='0.0001',
                        help='The e-score threshold for BLASTing hits to possible isoforms')
    parser.add_argument('-j', dest='junction', default='30',
                        help='The number of bases upstream and downstream to be assigned to the exon junction')
    args = parser.parse_args()
    # args = parser.parse_args(['-i', 'hello', '-g', '19', '-a', '02092018', '-t', '0.8'])
    ...
    # Read input file
    input_files_folder = file_folder + "/Input_Files/"
    input_file_name = args.input_file.replace(input_files_folder,"").replace(".txt", "")

    # List chosen input files
    print("\nYou have selected " + input_file_name)
    ...
    elif args.genome_build == '37' or '19':
    ...
    with open(args.input_file, "rU") as rMATS_file, \
            open(args.input_file, "r") as initial_data:
    ...
    dataDict['exonLengths'] = [[int(exon0End) - int(exon0Start),
                                int(exon1End) - int(exon1Start),
                                int(exon2End) - int(exon2Start)]
    ...
    dataDict['exonSequences'] = [[snapit.reverse_complement(exonSequences[2]),
                                  snapit.reverse_complement(exonSequences[1]),
                                  snapit.reverse_complement(exonSequences[0])]
    ...
    dataDict['faultyRevComp'] = [True if exonSequences[0] == snapit.reverse_complement(exonSequences[2]) else False
    ...
    with Pool(30) as p:
        dataDict['alignment'] = p.starmap(snapit.UTR_test_and_find_best_match, iterables)
    ...
    dataDict['notAligned'] = ["Both" if alignment[0][0] < float(args.threshold) and alignment[1][0] < float(args.threshold)
                              else "Upstream" if alignment[0][0] < float(args.threshold)
    else "Downstream" if alignment[1][0] < float(args.threshold)
    ...
    dataDict['matchedExonUP'] = [
        snapit.localizeCassetteExon(aaPositions[0], aaPositions[1], extracellDomains, notAligned)
    ...
    dataDict['matchedExonTMHMM'] = [
        snapit.localizeCassetteExon(aaPositions[0], aaPositions[1], extramemDomains, notAligned)
    ...
    iterables = [
        (exonSequences[0], exonSequences[1], exonSequences[2], strand, index, float(args.e_score), int(args.junction), extracellular)
        for exonSequences, strand, index, extracellular
        in zip(dataDict['exonSequences'], dataDict['strand'], gene_id, dataDict['extracellular'])]

    # Multiprocess BLAST
    with Pool(100) as p:
        dataDict['isoforms'] = p.starmap(snapit.findIsoform, iterables)
    ...
    # Upstream and downstream exons cannot be faulty reverse complements
