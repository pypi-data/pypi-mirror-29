"""
setup1_Download_Files_GRCh37.py
...
folder_list_to_make = ["/reference_data", "/reference_data/data_to_run_main", "/reference_data/initial_download", "/reference_data/setup", "/Install"]
main_folder = file_folder + "/reference_data/data_to_run_main/"
...
print("\nDownloading...\n"
      "1\tHomo_sapiens.GRCh37.75.cds.all.fa\n"
...
hg19_file = file_folder + "/reference_data/data_to_run_main/hg19.fa"
if not does_file_exist(hg19_file, make = False):
    print("\nDownloading the human genome...")
    hg19_gz = file_folder + "/reference_data/data_to_run_main/hg19_chromFa.tar.gz"
    hg19_link = "http://hgdownload.soe.ucsc.edu/goldenPath/hg19/bigZips/chromFa.tar.gz"
    urlretrieve(hg19_link, hg19_gz)
    # Unzip
    tar = tarfile.open(hg19_gz, 'r:gz')
    subprocess.call("cd " + main_folder + "\ntar -zxvf hg19_chromFa.tar.gz", shell = True)
    subprocess.call("cd " + main_folder + "\ncat chr*.fa > hg19.fa", shell = True)
    # Delete zipped file
    remove(hg19_gz)
    for x in tar.getnames():
        remove(main_folder + x)

# Download Ensembl human CDS annotation
Ensembl_ENSG_CDS_file = file_folder + "/reference_data/initial_download/Homo_sapiens.GRCh37.cds.all.fa"
if not does_file_exist(Ensembl_ENSG_CDS_file, make = False):
    print("\nDownloading all CDS from Ensembl...")
    Ensembl_ENSG_CDS_gz = file_folder + "/reference_data/initial_download/Homo_sapiens.GRCh37.75.cds.all.fa.gz"
    Ensembl_ENSG_CDS_link = "ftp://ftp.ensembl.org/pub/release-75/fasta/homo_sapiens/cds/Homo_sapiens.GRCh37.75.cds.all.fa.gz"
