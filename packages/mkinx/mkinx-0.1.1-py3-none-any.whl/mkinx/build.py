...
class MkinxFileHandler(PatternMatchingEventHandler):
    ...
    def on_modified(self, event):
        ...
        if event.src_path.split('.')[-1] == 'rst':
            ...
            os.system('mkinx build -F -p {}'.format(project))
...
def get_listed_projects():
    ...
    for _, l in enumerate(lines):
        ...
        if len(listed_projects) > 0 and '# About' in l:
...
def init(args):
    ...
    if 'y' in input('Include example project showcasing Sphinx\
        ...
        print(
            '\n\n The "RuntimeWarning: numpy.dtype size changed [...]" warning',
            'is expected')
    ...
    if not example_project:
        print('\n\n')
    print(bcolors.OKGREEN, 'Succes!', bcolors.ENDC,
          'You can now start your Docs in ./{}\n'.format(args.project_name),
          bcolors.BOLD, '$ cd ./{}'.format(
              args.project_name
          ), bcolors.ENDC)
    print('Start the server from within your Docs to see them \n(default port',
          'is 8443 but you can change it with the -s flag):')
    print(bcolors.BOLD, '{} $ mkinx serve\n'.format(args.project_name),
          bcolors.ENDC)
