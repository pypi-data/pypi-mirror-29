...
class NumericalScheme(utils.ReprObject):
    ...
    Args:
        ...
        estimate_timestep (bool, optional): try to estimate a timestep.
            Defaults to ``True``. If ``False``, The ``fallback_max_timestep``
            is used.
    ...
    def __init__(self, description=None, long_description=None,
                 ...
                 ignore_nonlinear=None, estimate_timestep=True):
        ...
        if estimate_timestep is not None:
            self.estimate_timestep = estimate_timestep
    ...
    def estimate_timestep(self):
        """
        Estimate the timestep or not

        :type: :any:`bool`
        """
        try:
            return self._estimate_timestep
        except AttributeError:
            self._estimate_timestep = True
        return self._estimate_timestep

    @estimate_timestep.setter
    def estimate_timestep(self, newestimate_timestep):
        self._estimate_timestep = bool(newestimate_timestep)
    ...
    def max_timestep_estimate(self, time=None, variablevalue=None):
        ...
        def smaller_than_time_constant(time=None, variablevalue=None):
            ...
            if lin < 0:
                logger.debug("{}: not a decay equation, linpart is {}"\
                    .format(eq.variable.id, lin))
                raise ValueError
            return 0.1 * tau  # timestep must be smaller than time constant
        # fallback function
    ...
    def integrate(self, time=None, until=None):
        ...
        logger.debug("{}: current maximum timestep is {}".format(
            self.equation.variable.id, current_max_timestep))
        if until is None:
            until = time + current_max_timestep
        logger.debug("{}: integrating until time {}".format(
            self.equation.variable.id, until))
        time_now = time
        while time_now < until:
            logger.debug(("{}: current time {} is smaller than until time {}"
                          ).format(self.equation.variable.id, time_now, until))
            ...
            logger.debug(("{}: integrate one step from time {} with "
                          "timestep {}").format(self.equation.variable.id,
                          time_now, timestep))
            # integrate one step
            self.integrate_step(time=time_now, timestep=timestep)
            time_now += timestep
        logger.debug(("{}: reached until time {}").format(
            self.equation.variable.id, time_now))
