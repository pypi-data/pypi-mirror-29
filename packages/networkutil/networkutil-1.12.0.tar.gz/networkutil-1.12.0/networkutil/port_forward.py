...
class Proxy:
    ...
    def __init__(self, host, port, f_host, f_port, udp = False):
        ...
        self.udp = udp
        self.protocol = socket.SOCK_DGRAM if udp == True else socket.SOCK_STREAM
        ...
        try:
            ...
            logging.error(u'Something went wrong, unable to bind server ({ip}) to port ({port})'.format(ip = self.host, port = self.port))
            logging.error(err)
...
def check_args(argv):
    ...
    for opt, arg in opts:
        if opt == u'-h':
            logging.info(u'If you are running from command line: python port_forward.py -d {u\'SERVER IP\': u\'10.10.1.40\', '
