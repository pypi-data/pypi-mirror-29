...
from keras.layers import Activation, BatchNormalization, Conv1D, Dense, Dropout, Flatten, MaxPooling1D, PADDING, reg
...
class large_model(Net):
	...
	def build_model(self, mode=False):
		...
		model.add(Conv1D(16, 64, strides=2, padding=PADDING, input_shape=self.input_shape, trainable=not self.transfer,
						 kernel_regularizer = reg.l2(REGULARIZATION)))
		model.add(BatchNormalization(trainable=not self.transfer))
		model.add(Activation('relu'))
		model.add(MaxPooling1D(pool_size=8))
		model.add(Conv1D(32, 32, strides=2, padding=PADDING, trainable=not self.transfer,
						 kernel_regularizer = reg.l2(REGULARIZATION)))
		model.add(BatchNormalization(trainable=not self.transfer))
		model.add(Activation('relu'))
		model.add(MaxPooling1D(pool_size=8))
		model.add(Conv1D(64, 16, strides=2, padding=PADDING,
						 kernel_regularizer = reg.l2(REGULARIZATION)))
		model.add(BatchNormalization())
		model.add(Activation('relu'))
		model.add(Conv1D(128, 8, strides=2, padding=PADDING,
						 kernel_regularizer = reg.l2(REGULARIZATION)))
		model.add(BatchNormalization())
		model.add(Activation('relu'))
		model.add(Flatten())
