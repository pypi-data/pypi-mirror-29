...
class DecisionTree(object):
    ...
    def _debug(self, indent: int=0, delta_indent: int=4) -> None:
        '''
        ASCII representation of branches. WARNING: recursive. Not advised for
        large trees. `indent' sets how many spaces between the start of a
        message and the left of the screen (starts at 0 by default).
        `delta_indent' determines how many extra spaces to indent the next
        level of recursion.
        '''
        indent *= ' '
        if self._has_branches:
            print('%s%s' % (indent, self._best_feature))
        else:
            print('  %s%s' % (indent, self.prediction))
        for branch in self._branches:
            test = branch._test.funcs[1]
            print('  %s%s %s' % (indent, test.func.__name__, test.args[0]))
            branch._debug(len(indent) + delta_indent, delta_indent)
