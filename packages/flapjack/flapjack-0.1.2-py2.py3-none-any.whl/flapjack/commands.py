...
class Build(Command):
    ...
    def __init__(self):
        super().__init__()
        self.parser.add_argument('-n', '--nonet', action='store_true',
                                 help="Don't download sources (requires all sources to be available offline)")

    def execute(self, args):
        builder_args = ['--require-changes', '--repo', _REPO]
        if args.nonet:
            builder_args += ['--ignore-download-failures']
        exitcode = ext.flatpak_builder(*builder_args)
        if exitcode != 0:
            return exitcode
