...
def main():
    ...
    if config_name not in configs:
        print(f'Please Make Sure Config Name Is In {configs}')
        exit()
    ...
    if len(args) != 0:
        ...
        if len(args) != 0 and args[0] == '-u':
            expect = common_scp(**configs[config_name], filename=file_path, upload=True)
        else:
            expect = common_scp(**configs[config_name], filename=file_path)
        expect.interact()
    else:
        if 'yit' in configs[config_name]:
            configs[config_name].pop('yit')
            expect = yit_login(**configs[config_name])
        else:
            expect = common_login(**configs[config_name])
        expect.interact()
