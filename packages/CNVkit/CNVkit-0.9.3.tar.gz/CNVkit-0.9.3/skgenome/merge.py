...
def _flatten_tuples_split(keyed_rows, combine, split_columns):
    ...
    if len(rows) == 1:
        ...
        #   for each original region/row being flattened here,
        #   - determine its size;
        #   - determine the sizes and proportions of its pieces after cutting;
        #   - before applying the 'combine' function for each field in
        #   split_columns, multiple each of those fields by its proportion of
        #   the original row size
        #   (these values are in rows_in_play, but not in an obvious way)

        # e.g. gene1 has weight 6, gene2 has weight 9, and they overlap 50%
        #   ->
        # ...   gene        weight          frac
        #       gene1       2               .25
        #       gene1,gene2 4 + 6 = 10      .75
        #       gene2       3               .25
        # uhg. SO! following the below,
        #   Before applying frac (i.e. ignoring split_columns):
        # ...   gene        weight          frac
        #       gene1       6               ---
        #       gene1,gene2 6 + 9 = 15      ---
        #       gene2       9               ---
        #   Then frac is applied:
        # ...   gene        weight          frac
        #       gene1       1.5             .25
        #       gene1,gene2 11.25           .75
        #       gene2       2.25            .25

        extra_cols = [x for x in first_row._fields[3:] if x in combine]
        breaks = sorted(set(itertools.chain(*[(r.start, r.end) for r in rows])))
        # Split fractions
        # XXX this needs to be done per original row
        if len(breaks) <= 2:
            fracs = [1.0]
        else:
            breaks = np.array(breaks)
            total_span = breaks[-1] - breaks[0]
            distances = breaks[1:] - breaks[0]
            fracs = distances / total_span
        for bp_start, bp_end, frac in zip(breaks[:-1], breaks[1:], fracs):
            # Find the row(s) overlapping this region
            # i.e. any not already seen and not already passed
            rows_in_play = [row for row in rows
                            if row.start <= bp_start and row.end >= bp_end]
