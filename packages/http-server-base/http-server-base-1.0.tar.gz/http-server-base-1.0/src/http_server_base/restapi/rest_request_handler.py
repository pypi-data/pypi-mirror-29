...
from typing import List, Union, Callable, Iterable, TypeVar, Any, Dict, Tuple, Type
...
class Rest_RequestHandler(Logged_RequestHandler):
    ...
    def get_argumnet_value(self, param_name:str, param_types:Tuple[Type], param_default, required:bool, source: dict, source_type:str, support_capitalization_style_switch:bool) -> Any:
        ...
        _success = False
        for _param_type in param_types:
            if (_param_type is None):
                _success = True
                break
                
            elif (value is None and param_default is None):
                _success = True
                break
        ...
        if (not _success):
            message = f"Argument '{param_name}' from {source_type} must be '{_param_type}', but not '{type(value)}'."
            self.resp_error(400, message)
    ...
    def get_args(self, source: dict, args_description:CanonicalArgumentListType, source_type:str, support_capitalization_style_switch:bool):
        ...
        if (not args_description is None):
            for param_name, param_types, param_default, required in args_description:
                value = self.get_argumnet_value(param_name, param_types, param_default, required, source, source_type, support_capitalization_style_switch)
    ...
    T = TypeVar('T')
    def resp_paging(self, result_keys:List[T], result_values:Union[List[Union[JsonSerializable, bytes]], Callable[[T], Union[JsonSerializable, bytes]]], page:int=None, per_page:int=None, *, arg_name_page:str= 'page', arg_name_per_page:str= 'per_page', code:int=200, message=None, url_template=None):
