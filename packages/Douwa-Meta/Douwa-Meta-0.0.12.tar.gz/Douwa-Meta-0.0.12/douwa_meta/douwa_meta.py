...
class TimestampMixin(object):
    create_time = db.Column(db.DateTime, default=lambda: utcnow())
    modify_time = db.Column(db.DateTime, onupdate=lambda: utcnow(), default=lambda: utcnow())
    tenant_id = db.Column(db.String(50), default='test')  # 租户id
    creator_uuid = db.Column(db.String(50), default='test')  # 创建人id
    creator_ugid = db.Column(db.String(50), default='test')  # 创建人组id
    ...
    if meta_columns:
        class Meta(SoftDeleteMixin, TimestampMixin, meta_columns,db.Model):
            ...
            def meta_parent(self):
                if not self.meta_parent_id:
                    return {}
                else:
                    db_session = get_session()
                    data = db_session.query(Meta).filter(Meta.meta_id == self.meta_parent_id).first()
                    return {
...
class CommonSchema(Schema):
    modify_time = fields.DateTime(allow_none=True)
    create_time = fields.DateTime(allow_none=True)
    tenant_id = fields.String(default='test',allow_none=True)
    creator_uuid = fields.String(default='test',allow_none=True)
    creator_ugid = fields.String(default='test',allow_none=True)
    creator_user = fields.Dict(default=lambda: {'user_id': 'test', 'user_name': 'test'},allow_none=True)
    creator_group = fields.Dict(default={'a':1},allow_none=True)
...
class MetaListView(Resource,Check,Jurisdiction):
    ...
    def __init__(self):
        self.reqparse = reqparse.RequestParser()
        self.META_EXCLUDE = ("meta_parent","meta_id", 'create_time', 'modify_time', 'creator_uuid', 'creator_ugid', 'creator_user', 'creator_group','tenant_id')
        if request.method == 'GET':
            self.reqparse.add_argument('page', type=int, location='args')
            self.reqparse.add_argument('per_page', type=int, location='args')
...
class MetaView(Resource,Jurisdiction,Check):
    # method_decorators = [permission("部门"), authorization]
    def __init__(self):
        self.META_EXCLUDE = ("meta_parent","meta_id", 'level','create_time', 'modify_time', 'creator_uuid', 'creator_ugid', 'creator_user', 'creator_group','tenant_id')
