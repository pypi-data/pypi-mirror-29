...
### What kind of models can be contributed

The requirements on the software side are the following:
- **Dependencies:** all the dependencies have to be installable able via conda or pip (many bioinformatics packages (including R packages and C libraries) are available though the bioconda channel of conda: bioconda.github.io)
- **Framework:** in case the model is not implemented in Keras/tensorflow/pytorch/scikit-learn you have to implement a python class with a predict_on_batch method that takes (nested) numpy array(s) as input and returns numpy array(s) of predictions. 

The former is pretty general. You can use binaries from different languages using python's system/subprocess calls.
