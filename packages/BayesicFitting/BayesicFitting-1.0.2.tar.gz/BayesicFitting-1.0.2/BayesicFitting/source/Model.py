...
#   TBC if needed
    ...
    #  *****DERIVATIVE*********************************************************
    def derivative( self, xdata, param, useNum=False, upto=None ):
        """
        ...
        upto : None or Model
            calculate derivative until Model|None is found in the chain
        ...
        df = self._recursiveDerivative( xdata, param, result, df,
                    useNum=useNum, upto=upto )
        return df

    def _recursiveDerivative( self, xdata, param, result, df,
                useNum=False, upto=None ):
        """
        ...
        if self._operation <= self.ADD :
            ...
            nextres = super( Model, self ).result( result, par )
            df *= nextdf
        ...
        if nextres is None:
            nextres = super( Model, self ).result( xdata, par )
        result = self.operate( result, param[np:], nextres )
    ...
    def _recursivePartial( self, xdata, param, at, result, partial, useNum=False ):
        ...
        if self._operation == self.SUB :
            ...
            nextres = super( Model, self ).result( result, par )
            model = self
            dfdx = model._head.derivative( xdata, param, upto=model )
            nextpartial = numpy.multiply( partial.transpose(), dfdx ).transpose()
