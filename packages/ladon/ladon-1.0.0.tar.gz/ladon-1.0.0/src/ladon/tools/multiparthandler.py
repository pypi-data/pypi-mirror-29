...
class MultiPartReader(object):
    def __init__(self, chunk_size, boundary, req, content_size=None):
        ...
        self.rx_boundary_split = re.compile(
    ...
    def write(self, data, eop=False):
        ...
        if self.attachment_headers_parsed == True:
            ...
            if self.part_count == 0:
                ...
                self.interface_request_headers = dict(
                    map(upperkey, rx_headers.findall(self.attachment_headers)))
            else:
            ...
            if eop == True:
                if self.fd != None:
                    ...
                    self.attachments[self.part_count -
                                     1]['size'] = self.attachment_bytes_size
                    self.attachments[self.part_count -
                                     1]['headers'] = headers_dict
                    if b'CONTENT_ID' in headers_dict:
                        self.attachments_by_id[headers_dict[b'CONTENT_ID']
                                               ] = self.attachments[self.part_count-1]

                if not self.eos:
                    self.attachments[self.part_count] = {}
                    self.fd, self.attachments[self.part_count]['path'] = tempfile.mkstemp(
                        '', 'content_', self.tmpdir)
                    self.part_count += 1
                self.attachment_bytes_size = 0
                self.attachment_headers = b''
                self.attachment_headers_parsed = False
