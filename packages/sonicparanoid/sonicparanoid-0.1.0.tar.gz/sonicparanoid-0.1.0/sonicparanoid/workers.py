...
def consume_mmseqs_search_1cpu(jobs_queue, results_queue, inDir, outDir, dbDir, sensitivity, cutoff):
    while True:
        ...
        # it MUST use 1 CPU
        parsedOutput, search_time, convert_time, parse_time, tot_time = mmseqs_1pass(inSeq, dbSeq, dbDir=dbDir, outDir=outDir, tmpDirName=tmpMMseqsDirName, sensitivity=sensitivity, evalue=1000, cutoff=cutoff, threads=1, debug=False)
...
def mmseqs_search(inSeq, dbSeq, dbDir=os.getcwd(), outDir=os.getcwd(), tmpDirName=None, sensitivity=4.0, evalue=1000, threads=4, cleanUp=False, debug=False):
    ...
    minUngappedScore = 30
    if sensitivity == 4:
        # this value is better trade-off for the default mode
        minUngappedScore = 15
...
def perform_mmseqs_multiproc_alignments(required_align, inDir, outDir, dbDir, create_idx=True, sensitivity=6, cutoff=0.5, threads=4, debug=False):
    ...
    # create mmseqs databases (sequencially)
    ''' needs to be parallel
    for sp in reqSpDict:
        tmpQuerySeq = os.path.join(inDir, sp)
        #print(tmpQuerySeq)
        # check the query db name
        queryDBname = os.path.basename(tmpQuerySeq)
        queryDBname = queryDBname.split('.', 1)[0] # take the left part of the file name
        queryDBname = '%s.mmseqs2db'%queryDBname
        queryDBpath = '%s%s'%(dbDir, queryDBname)
        # create the database if does not exist yet
        if not os.path.isfile(queryDBpath):
            mmseqs_createdb(tmpQuerySeq, outDir=dbDir, debug=False)
            mmseqs_createindex(queryDBpath, debug=False)
    #'''
    ...
    # call the method inside workers
    runningJobs = [mp.Process(target=consume_mmseqs_search_1cpu, args=(align_queue, results_queue, inDir, outDir, dbDir, sensitivity, cutoff)) for i_ in range(threads)]
...
def write_seq_lengths(proteome, out_path, debug=False):
    ...
    # dump the pick file
    #pickle.dump(picDict, open(picklPath, 'wb'))
