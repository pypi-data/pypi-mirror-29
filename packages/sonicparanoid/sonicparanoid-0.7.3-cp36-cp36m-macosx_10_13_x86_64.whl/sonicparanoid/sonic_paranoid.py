...
def get_params():
    ...
    parser.add_argument('-mgs', '--max-gene-per-sp', type=int, required=False, help='Limits the maximum number of gene per species in the multi-species output table. This option is reduces the verbosity of the multi-species output file when comparing a high number of species (especially eukaryotes). Default=10', default=10)
...
def main():
    ...
    #sys.exit('DEBUG :: sonic_paranoid.py')
    ...
    # set the execution path for mmseqs
    # orthodetect.set_mmseqs_path(mmseqsPath)
    ...
    if updateId is None:
        if multiOnly: #skip everything and execute directly multi-species clustering
            ...
            spFile, pairsFile = orthodetect.run_sonicparanoid2_multiproc(inDir, outDir=outDir, threads=cpus, sharedDir=alignDir, mmseqsDbDir=dbDirectory, create_idx=idx_dbs, sensitivity=sensitivity, cutoff=coff, confCutoff=0.05, lenDiffThr=args.max_len_diff, overwrite_all=overwrite, overwrite_tbls=owOrthoTbls, update_run=update_run, debug=debug)
            ##spFile, pairsFile = orthodetect.run_sonicparanoid2(inDir, outDir=outDir, threads=cpus, sharedDir=alignDir, mmseqsDbDir=dbDirectory, sensitivity=sensitivity, cutoff=coff, confCutoff=0.05, lenDiffThr=args.max_len_diff, reuse=True, noMmseqs=skipMmseqs, overwrite=overwrite, debug=debug)
