...
def __run_setup():
    ...
    sys.stdout.flush()
    if host_os == 'Linux64':
        print "please input your sudo password if asked"
        if which('apt-get') != None:
            popen('sudo dpkg --add-architecture i386', shell=True)
            popen('sudo apt-get -qq update', shell=True)
...
def install_esptool():
    ...
    try:
        ...
        if os.path.isdir('./tmp') == False:
            os.mkdir('./tmp')
        os.chdir('./tmp')
        urllib.urlretrieve('http://alios-things-public.oss-cn-hangzhou.aliyuncs.com/aos-esptool.zip', 'esptool.zip')
        if which('unzip') != None:
