...
def ls(folder: '_coconut.typing.Optional[str]'=None, options: '_coconut.typing.Sequence[str]'=[]):  # line 806
    ...
    if relPath.startswith(os.pardir):  # line 812
    ...
    for file in files:  # for each file list all tracking patterns that match, or none (e.g. in picky mode after commit)  # line 824
        ...
        printo("%s %s%s" % ("IGN" if ignore is not None else ("TRK" if len(matches) > 0 else DOT_SYMBOL * 3), file, "  (%s)" % ignore if ignore is not None else ("  (%s)" % ("; ".join(matches)) if len(matches) > 0 else "")))  # line 836
...
def log(options: '_coconut.typing.Sequence[str]'=[]):  # line 838
    ...
    last = {}  # type: Dict[str, PathInfo]  # path infos from previous revision  # line 847
    commit = None  # type: CommitInfo  # line 848
    for no in range(maxi + 1):  # line 849
        if no in m.commits:  # line 850
            commit = m.commits[no]  # line 850
        else:  # TODO clean this code  # line 851
            n = Metadata()  # type: Metadata  # line 852
            n.loadBranch(n.getParentBranch(m.branch, no))  # line 853
            commit = n.commits[no]  # line 854
        nxts = next(changesetIterator)  # type: Dict[str, PathInfo]  # line 855
        news = frozenset(nxts.keys())  # type: FrozenSet[str]  # line 856
        _add = news - olds  # type: FrozenSet[str]  # line 857
        _del = olds - news  # type: FrozenSet[str]  # line 858
#    _mod_:Dict[str,PathInfo] = {k: nxts[k] for k in news - _add - _del}
        _mod = frozenset([_ for _, info in {k: nxts[k] for k in news - _add - _del}.items() if last[_].size != info.size or (last[_].hash != info.hash if m.strict else last[_].mtime != info.mtime)])  # type: FrozenSet[str]  # line 860
#    _mov:FrozenSet[str] = detectMoves(ChangeSet(nxts, {o: None for o in olds})  # TODO determine moves - can we reuse detectMoves(changes)?
        _txt = len([a for a in _add if m.isTextType(a)])  # type: int  # line 862
        printo("  %s r%s @%s (+%02d/-%02d/%s%02d/T%02d) |%s|%s" % ("*" if commit.number == maxi else " ", ("%%%ds" % nl) % commit.number, strftime(commit.ctime), len(_add), len(_del), PLUSMINUS_SYMBOL if m.c.useUnicodeFont else "~", len(_mod), _txt, ((lambda _coconut_none_coalesce_item: "" if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(commit.message)), "TAG" if ((lambda _coconut_none_coalesce_item: "" if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(commit.message)) in m.tags else ""))  # line 863
        if '--changes' in options:  # TODO moves detection?  # line 864
            m.listChanges(ChangeSet({a: None for a in _add}, {d: None for d in _del}, {m: None for m in _mod}, {}))  # TODO moves detection?  # line 864
        if '--diff' in options:  #  _diff(m, changes)  # needs from revision diff  # line 865
            pass  #  _diff(m, changes)  # needs from revision diff  # line 865
        olds = news  # replaces olds for next revision compare  # line 866
        last = {k: v for k, v in nxts.items()}  # create new reference  # line 867

def dump(argument: 'str', options: '_coconut.typing.Sequence[str]'=[]):  # line 869
    ''' Exported entire repository as archive for easy transfer. '''  # line 870
    debug(MARKER + "Dumping repository to archive...")  # line 871
    m = Metadata()  # type: Metadata  # to load the configuration  # line 872
    progress = '--progress' in options  # type: bool  # line 873
    delta = not '--full' in options  # type: bool  # line 874
    skipBackup = '--skip-backup' in options  # type: bool  # line 875
    import warnings  # line 876
    import zipfile  # line 876
    try:  # HINT zlib is the library that contains the deflated algorithm  # line 877
        import zlib  # HINT zlib is the library that contains the deflated algorithm  # line 877
        compression = zipfile.ZIP_DEFLATED  # HINT zlib is the library that contains the deflated algorithm  # line 877
    except:  # line 878
        compression = zipfile.ZIP_STORED  # line 878

    if argument is None:  # line 880
        Exit("Argument missing (target filename)")  # line 880
    argument = argument if "." in argument else argument + DUMP_FILE  # TODO this logic lacks a bit, "v1.2" would not receive the suffix  # line 881
    entries = []  # type: List[str]  # line 882
    if os.path.exists(encode(argument)) and not skipBackup:  # line 883
        try:  # line 884
            debug("Creating backup...")  # line 885
            shutil.copy2(encode(argument), encode(argument + BACKUP_SUFFIX))  # line 886
            if delta:  # line 887
                with zipfile.ZipFile(argument, "r") as _zip:  # list of pure relative paths without leading dot, normal slashes  # line 888
                    entries = _zip.namelist()  # list of pure relative paths without leading dot, normal slashes  # line 888
        except Exception as E:  # line 889
            Exit("Error creating backup copy before dumping. Please resolve and retry. %r" % E)  # line 889
    debug("Dumping revisions...")  # line 890
    if delta:  # don't show duplicate entries warnings  # line 891
        warnings.filterwarnings('ignore', 'Duplicate name.*', UserWarning, "zipfile", 0)  # don't show duplicate entries warnings  # line 891
    with zipfile.ZipFile(argument, "a" if delta else "w", compression) as _zip:  # create  # line 892
        _zip.debug = 0  # suppress debugging output  # line 893
        _zip.comment = ("Repository dump from %r" % strftime()).encode(UTF8)  # line 894
        repopath = os.path.join(os.getcwd(), metaFolder)  # type: str  # line 895
        indicator = ProgressIndicator(PROGRESS_MARKER[1 if m.c.useUnicodeFont else 0]) if progress else None  # type: _coconut.typing.Optional[ProgressIndicator]  # line 896
        totalsize = 0  # type: int  # line 897
        start_time = time.time()  # type: float  # line 898
        for dirpath, dirnames, filenames in os.walk(repopath):  # TODO use index knowledge instead of walking to avoid adding stuff not needed?  # line 899
            dirpath = decode(dirpath)  # line 900
            if dirpath.endswith(BACKUP_SUFFIX):  # don't backup backups  # line 901
                continue  # don't backup backups  # line 901
            printo(ljust(dirpath))  # TODO improve progress indicator output to | dir | dumpuing file  # line 902
            dirnames[:] = sorted([decode(d) for d in dirnames])  # HINT sort for reproducible delta dumps  # line 903
            filenames[:] = sorted([decode(f) for f in filenames])  # line 904
            for filename in filenames:  # line 905
                abspath = os.path.join(dirpath, filename)  # type: str  # line 906
                relpath = os.path.join(metaFolder, os.path.relpath(abspath, repopath)).replace(os.sep, "/")  # type: str  # line 907
                totalsize += os.stat(encode(abspath)).st_size  # line 908
                show = indicator.getIndicator() if progress else None  # type: _coconut.typing.Optional[str]  # line 909
                if relpath.endswith(BACKUP_SUFFIX):  # don't backup backups  # line 910
                    continue  # don't backup backups  # line 910
                if not delta or relpath.endswith(metaFile) or relpath not in entries:  # always update metadata, otherwise only add new revision files  # line 911
                    if show:  # line 912
                        printo("\r" + ljust("Dumping %s @%.2f MiB/s %s" % (show, totalsize / (MEBI * (time.time() - start_time)), filename)), nl="")  # line 912
                    _zip.write(abspath, relpath)  # write entry into archive  # line 913
        if delta:  # line 914
            _zip.comment = ("Delta dump from %r" % strftime()).encode(UTF8)  # line 914
    info("\r" + ljust(MARKER + "Finished dumping entire repository @%.2f MiB/s." % (totalsize / (MEBI * (time.time() - start_time)))))  # clean line  # line 915

def config(arguments: 'List[str]', options: 'List[str]'=[]):  # line 917
    command, key, value = (arguments + [None] * 2)[:3]  # line 918
    if command not in ["set", "unset", "show", "list", "add", "rm"]:  # line 919
        Exit("Unknown config command")  # line 919
    local = "--local" in options  # type: bool  # line 920
    m = Metadata()  # type: Metadata  # loads layered configuration as well. TODO warning if repo not exists  # line 921
    c = m.c if local else m.c.__defaults  # type: configr.Configr  # line 922
    if command == "set":  # line 923
        if None in (key, value):  # line 924
            Exit("Key or value not specified")  # line 924
        if key not in (["defaultbranch"] + ([] if local else CONFIGURABLE_FLAGS) + CONFIGURABLE_LISTS):  # line 925
            Exit("Unsupported key for %s configuration %r" % ("local " if local else "global", key))  # line 925
        if key in CONFIGURABLE_FLAGS and value.lower() not in TRUTH_VALUES + FALSE_VALUES:  # line 926
            Exit("Cannot set flag to '%s'. Try on/off instead" % value.lower())  # line 926
        c[key] = value.lower() in TRUTH_VALUES if key in CONFIGURABLE_FLAGS else (removePath(key, value.strip()) if key not in CONFIGURABLE_LISTS else [removePath(key, v) for v in safeSplit(value, ";")])  # TODO sanitize texts?  # line 927
    elif command == "unset":  # line 928
        if key is None:  # line 929
            Exit("No key specified")  # line 929
        if key not in c.keys():  # HINT: Works on local configurations when used with --local  # line 930
            Exit("Unknown key")  # HINT: Works on local configurations when used with --local  # line 930
        del c[key]  # line 931
    elif command == "add":  # line 932
        if None in (key, value):  # line 933
            Exit("Key or value not specified")  # line 933
        if key not in CONFIGURABLE_LISTS:  # line 934
            Exit("Unsupported key %r" % key)  # line 934
        if key not in c.keys():  # prepare empty list, or copy from global, add new value below  # line 935
            c[key] = [_ for _ in c.__defaults[key]] if local else []  # prepare empty list, or copy from global, add new value below  # line 935
        elif value in c[key]:  # line 936
            Exit("Value already contained, nothing to do")  # line 936
        if ";" in value:  # line 937
            c[key].append(removePath(key, value))  # line 937
        else:  # line 938
            c[key].extend([removePath(key, v) for v in value.split(";")])  # line 938
    elif command == "rm":  # line 939
        if None in (key, value):  # line 940
            Exit("Key or value not specified")  # line 940
        if key not in c.keys():  # line 941
            Exit("Unknown key %r" % key)  # line 941
        if value not in c[key]:  # line 942
            Exit("Unknown value %r" % value)  # line 942
        c[key].remove(value)  # line 943
        if local and len(c[key]) == 0 and "--prune" in options:  # remove local enty, to fallback to global  # line 944
            del c[key]  # remove local enty, to fallback to global  # line 944
    else:  # Show or list  # line 945
        if key == "flags":  # list valid configuration items  # line 946
            printo(", ".join(CONFIGURABLE_FLAGS))  # list valid configuration items  # line 946
        elif key == "lists":  # line 947
            printo(", ".join(CONFIGURABLE_LISTS))  # line 947
        elif key == "texts":  # line 948
            printo(", ".join([_ for _ in defaults.keys() if _ not in (CONFIGURABLE_FLAGS + CONFIGURABLE_LISTS)]))  # line 948
        else:  # line 949
            out = {3: "[default]", 2: "[global] ", 1: "[local]  "}  # type: Dict[int, str]  # line 950
            c = m.c  # always use full configuration chain  # line 951
            try:  # attempt single key  # line 952
                assert key is not None  # line 953
                c[key]  # line 953
                l = key in c.keys()  # type: bool  # line 954
                g = key in c.__defaults.keys()  # type: bool  # line 954
                printo("%s %s %r" % (key.rjust(20), out[3] if not (l or g) else (out[1] if l else out[2]), c[key]))  # line 955
            except:  # normal value listing  # line 956
                vals = {k: (repr(v), 3) for k, v in defaults.items()}  # type: Dict[str, Tuple[str, int]]  # line 957
                vals.update({k: (repr(v), 2) for k, v in c.__defaults.items()})  # line 958
                vals.update({k: (repr(v), 1) for k, v in c.__map.items()})  # line 959
                for k, vt in sorted(vals.items()):  # line 960
                    printo("%s %s %s" % (k.rjust(20), out[vt[1]], vt[0]))  # line 960
                if len(c.keys()) == 0:  # line 961
                    info("No local configuration stored")  # line 961
                if len(c.__defaults.keys()) == 0:  # line 962
                    info("No global configuration stored")  # line 962
        return  # in case of list, no need to store anything  # line 963
    if local:  # saves changes of repoConfig  # line 964
        m.repoConf = c.__map  # saves changes of repoConfig  # line 964
        m.saveBranches()  # saves changes of repoConfig  # line 964
        Exit("OK", code=0)  # saves changes of repoConfig  # line 964
    else:  # global config  # line 965
        f, h = saveConfig(c)  # only saves c.__defaults (nested Configr)  # line 966
        if f is None:  # line 967
            error("Error saving user configuration: %r" % h)  # line 967
        else:  # line 968
            Exit("OK", code=0)  # line 968

def move(relPath: 'str', pattern: 'str', newRelPath: 'str', newPattern: 'str', options: 'List[str]'=[], negative: 'bool'=False):  # line 970
    ''' Path differs: Move files, create folder if not existing. Pattern differs: Attempt to rename file, unless exists in target or not unique.
      for "mvnot" don't do any renaming (or do?)
  '''  # line 973
    debug(MARKER + "Renaming %r to %r" % (pattern, newPattern))  # line 974
    force = '--force' in options  # type: bool  # line 975
    soft = '--soft' in options  # type: bool  # line 976
    if not os.path.exists(encode(relPath.replace(SLASH, os.sep))) and not force:  # line 977
        Exit("Source folder doesn't exist. Use --force to proceed anyway")  # line 977
    m = Metadata()  # type: Metadata  # line 978
    patterns = m.branches[m.branch].untracked if negative else m.branches[m.branch].tracked  # type: List[str]  # line 979
    matching = fnmatch.filter(os.listdir(relPath.replace(SLASH, os.sep)) if os.path.exists(encode(relPath.replace(SLASH, os.sep))) else [], os.path.basename(pattern))  # type: List[str]  # find matching files in source  # line 980
    matching[:] = [f for f in matching if len([n for n in m.c.ignores if fnmatch.fnmatch(f, n)]) == 0 or len([p for p in m.c.ignoresWhitelist if fnmatch.fnmatch(f, p)]) > 0]  # line 981
    if not matching and not force:  # line 982
        Exit("No files match the specified file pattern. Use --force to proceed anyway")  # line 982
    if not (m.track or m.picky):  # line 983
        Exit("Repository is in simple mode. Simply use basic file operations to modify files, then execute 'sos commit' to version the changes")  # line 983
    if pattern not in patterns:  # list potential alternatives and exit  # line 984
        for tracked in (t for t in patterns if os.path.dirname(t) == relPath):  # for all patterns of the same source folder  # line 985
            alternative = fnmatch.filter(matching, os.path.basename(tracked))  # type: _coconut.typing.Sequence[str]  # find if it matches any of the files in the source folder, too  # line 986
            if alternative:  # line 987
                info("  '%s' matches %d files" % (tracked, len(alternative)))  # line 987
        if not (force or soft):  # line 988
            Exit("File pattern '%s' is not tracked on current branch. 'sos move' only works on tracked patterns" % pattern)  # line 988
    basePattern = os.path.basename(pattern)  # type: str  # pure glob without folder  # line 989
    newBasePattern = os.path.basename(newPattern)  # type: str  # line 990
    if basePattern.count("*") < newBasePattern.count("*") or (basePattern.count("?") - basePattern.count("[?]")) < (newBasePattern.count("?") - newBasePattern.count("[?]")) or (basePattern.count("[") - basePattern.count("\\[")) < (newBasePattern.count("[") - newBasePattern.count("\\[")) or (basePattern.count("]") - basePattern.count("\\]")) < (newBasePattern.count("]") - newBasePattern.count("\\]")):  # line 991
        Exit("Glob markers from '%s' to '%s' don't match, cannot move/rename tracked matching files" % (basePattern, newBasePattern))  # line 995
    oldTokens = None  # type: _coconut.typing.Sequence[GlobBlock]  # line 996
    newToken = None  # type: _coconut.typing.Sequence[GlobBlock]  # line 996
    oldTokens, newTokens = tokenizeGlobPatterns(os.path.basename(pattern), os.path.basename(newPattern))  # line 997
    matches = convertGlobFiles(matching, oldTokens, newTokens)  # type: _coconut.typing.Sequence[Tuple[str, str]]  # computes list of source - target filename pairs  # line 998
    if len({st[1] for st in matches}) != len(matches):  # line 999
        Exit("Some target filenames are not unique and different move/rename actions would point to the same target file")  # line 999
    matches = reorderRenameActions(matches, exitOnConflict=not soft)  # attempts to find conflict-free renaming order, or exits  # line 1000
    if os.path.exists(encode(newRelPath)):  # line 1001
        exists = [filename[1] for filename in matches if os.path.exists(encode(os.path.join(newRelPath, filename[1]).replace(SLASH, os.sep)))]  # type: _coconut.typing.Sequence[str]  # line 1002
        if exists and not (force or soft):  # line 1003
            Exit("%s files would write over existing files in %s cases. Use --force to execute it anyway" % ("Moving" if relPath != newRelPath else "Renaming", "all" if len(exists) == len(matches) else "some"))  # line 1003
    else:  # line 1004
        os.makedirs(encode(os.path.abspath(newRelPath.replace(SLASH, os.sep))))  # line 1004
    if not soft:  # perform actual renaming  # line 1005
        for (source, target) in matches:  # line 1006
            try:  # line 1007
                shutil.move(encode(os.path.abspath(os.path.join(relPath, source).replace(SLASH, os.sep))), encode(os.path.abspath(os.path.join(newRelPath, target).replace(SLASH, os.sep))))  # line 1007
            except Exception as E:  # one error can lead to another in case of delicate renaming order  # line 1008
                error("Cannot move/rename file '%s' to '%s'" % (source, os.path.join(newRelPath, target)))  # one error can lead to another in case of delicate renaming order  # line 1008
    patterns[patterns.index(pattern)] = newPattern  # line 1009
    m.saveBranches()  # line 1010

def parse(root: 'str', cwd: 'str', cmd: 'str'):  # line 1012
    ''' Main operation. Main has already chdir into VCS root folder, cwd is original working directory for add, rm, mv. '''  # line 1013
    debug("Parsing command-line arguments...")  # line 1014
    try:  # line 1015
        onlys, excps = parseOnlyOptions(cwd, sys.argv)  # extracts folder-relative information for changes, commit, diff, switch, update  # line 1016
        command = sys.argv[1].strip() if len(sys.argv) > 1 else ""  # line 1017
        arguments = [c.strip() for c in sys.argv[2:] if not c.startswith("--")]  # type: List[_coconut.typing.Optional[str]]  # line 1018
        options = [c.strip() for c in sys.argv[2:] if c.startswith("--")]  # line 1019
        debug("Processing command %r with arguments %r and options %r." % (command, [_ for _ in arguments if _ is not None], options))  # line 1020
        if command[:1] in "amr":  # line 1021
            relPath, pattern = relativize(root, os.path.join(cwd, arguments[0] if arguments else "."))  # line 1021
        if command[:1] == "m":  # line 1022
            if len(arguments) < 2:  # line 1023
                Exit("Need a second file pattern argument as target for move command")  # line 1023
            newRelPath, newPattern = relativize(root, os.path.join(cwd, arguments[1]))  # line 1024
        arguments[:] = (arguments + [None] * 3)[:3]  # line 1025
        if command[:1] == "a":  # addnot  # line 1026
            add(relPath, pattern, options, negative="n" in command)  # addnot  # line 1026
        elif command[:1] == "b":  # line 1027
            branch(arguments[0], arguments[1], options)  # line 1027
        elif command[:3] == "com":  # line 1028
            commit(arguments[0], options, onlys, excps)  # line 1028
        elif command[:2] == "ch":  # "changes" (legacy)  # line 1029
            changes(arguments[0], options, onlys, excps)  # "changes" (legacy)  # line 1029
        elif command[:2] == "ci":  # line 1030
            commit(arguments[0], options, onlys, excps)  # line 1030
        elif command[:3] == 'con':  # line 1031
            config(arguments, options)  # line 1031
        elif command[:2] == "de":  # line 1032
            destroy(arguments[0], options)  # line 1032
        elif command[:2] == "di":  # line 1033
            diff(arguments[0], options, onlys, excps)  # line 1033
        elif command[:2] == "du":  # line 1034
            dump(arguments[0], options)  # line 1034
        elif command[:1] == "h":  # line 1035
            usage(APPNAME, version.__version__)  # line 1035
        elif command[:2] == "lo":  # line 1036
            log(options)  # line 1036
        elif command[:2] == "li":  # line 1037
            ls(os.path.relpath((lambda _coconut_none_coalesce_item: cwd if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(arguments[0]), root), options)  # line 1037
        elif command[:2] == "ls":  # line 1038
            ls(os.path.relpath((lambda _coconut_none_coalesce_item: cwd if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(arguments[0]), root), options)  # line 1038
        elif command[:1] == "m":  # mvnot  # line 1039
            move(relPath, pattern, newRelPath, newPattern, options, negative="n" in command)  # mvnot  # line 1039
        elif command[:2] == "of":  # line 1040
            offline(arguments[0], arguments[1], options)  # line 1040
        elif command[:2] == "on":  # line 1041
            online(options)  # line 1041
        elif command[:1] == "r":  # rmnot  # line 1042
            remove(relPath, pattern, negative="n" in command)  # rmnot  # line 1042
        elif command[:2] == "st":  # line 1043
            status(arguments[0], cwd, cmd, options, onlys, excps)  # line 1043
        elif command[:2] == "sw":  # line 1044
            switch(arguments[0], options, onlys, excps)  # line 1044
        elif command[:1] == "u":  # line 1045
            update(arguments[0], options, onlys, excps)  # line 1045
        elif command[:1] == "v":  # line 1046
            usage(APPNAME, version.__version__, short=True)  # line 1046
        else:  # line 1047
            Exit("Unknown command '%s'" % command)  # line 1047
        Exit(code=0)  # regular exit  # line 1048
    except (Exception, RuntimeError) as E:  # line 1049
        exception(E)  # line 1050
        Exit("An internal error occurred in SOS. Please report above message to the project maintainer at  https://github.com/ArneBachmann/sos/issues  via 'New Issue'.\nPlease state your installed version via 'sos version', and what you were doing")  # line 1051

def main():  # line 1053
    global debug, info, warn, error  # to modify logger  # line 1054
    logging.basicConfig(level=level, stream=sys.stderr, format=("%(asctime)-23s %(levelname)-8s %(name)s:%(lineno)d | %(message)s" if '--log' in sys.argv else "%(message)s"))  # line 1055
    _log = Logger(logging.getLogger(__name__))  # line 1056
    debug, info, warn, error = _log.debug, _log.info, _log.warn, _log.error  # line 1056
    for option in (o for o in ['--log', '--verbose', '-v', '--sos'] if o in sys.argv):  # clean up program arguments  # line 1057
        sys.argv.remove(option)  # clean up program arguments  # line 1057
    if '--help' in sys.argv or len(sys.argv) < 2:  # line 1058
        usage(APPNAME, version.__version__)  # line 1058
    command = sys.argv[1] if len(sys.argv) > 1 else None  # type: _coconut.typing.Optional[str]  # line 1059
    root, vcs, cmd = findSosVcsBase()  # root is None if no .sos folder exists up the folder tree (still working online); vcs is checkout/repo root folder; cmd is the VCS base command  # line 1060
    debug("Found root folders for SOS|VCS: %s|%s" % (("-" if root is None else root), ("-" if vcs is None else vcs)))  # line 1061
    defaults["defaultbranch"] = (lambda _coconut_none_coalesce_item: "default" if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(vcsBranches.get(cmd, "trunk"))  # sets dynamic default with SVN fallback  # line 1062
    defaults["useChangesCommand"] = cmd == "fossil"  # sets dynamic default with SVN fallback  # line 1063
    if force_sos or root is not None or (("" if command is None else command))[:2] == "of" or (("" if command is None else command))[:1] in "hv":  # in offline mode or just going offline TODO what about git config?  # line 1064
        cwd = os.getcwd()  # line 1065
        os.chdir(cwd if command[:2] == "of" else (cwd if root is None else root))  # line 1066
        parse(root, cwd, cmd)  # line 1067
    elif force_vcs or cmd is not None:  # online mode - delegate to VCS  # line 1068
        info("%s: Running '%s %s'" % (COMMAND.upper(), cmd, " ".join(sys.argv[1:])))  # line 1069
        import subprocess  # only required in this section  # line 1070
        process = subprocess.Popen([cmd] + sys.argv[1:], shell=False, stdin=subprocess.PIPE, stdout=sys.stdout, stderr=sys.stderr)  # line 1071
        inp = ""  # type: str  # line 1072
        while True:  # line 1073
            so, se = process.communicate(input=inp)  # line 1074
            if process.returncode is not None:  # line 1075
                break  # line 1075
            inp = sys.stdin.read()  # line 1076
        if sys.argv[1][:2] == "co" and process.returncode == 0:  # successful commit - assume now in sync again (but leave meta data folder with potential other feature branches behind until "online")  # line 1077
            if root is None:  # line 1078
                Exit("Cannot determine VCS root folder: Unable to mark repository as synchronized and will show a warning when leaving offline mode")  # line 1078
            m = Metadata(root)  # type: Metadata  # line 1079
            m.branches[m.branch] = dataCopy(BranchInfo, m.branches[m.branch], inSync=True)  # mark as committed  # line 1080
            m.saveBranches()  # line 1081
    else:  # line 1082
        Exit("No offline repository present, and unable to detect VCS file tree")  # line 1082


# Main part
verbose = os.environ.get("DEBUG", "False").lower() == "true" or '--verbose' in sys.argv or '-v' in sys.argv  # imported from utility, and only modified here  # line 1086
level = logging.DEBUG if verbose else logging.INFO  # line 1087
force_sos = '--sos' in sys.argv  # type: bool  # line 1088
force_vcs = '--vcs' in sys.argv  # type: bool  # line 1089
_log = Logger(logging.getLogger(__name__))  # line 1090
debug, info, warn, error = _log.debug, _log.info, _log.warn, _log.error  # line 1090
if __name__ == '__main__':  # line 1091
    main()  # line 1091
