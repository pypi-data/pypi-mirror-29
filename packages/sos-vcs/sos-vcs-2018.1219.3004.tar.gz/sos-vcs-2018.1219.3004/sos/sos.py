...
def dump(argument: 'str', options: '_coconut.typing.Sequence[str]'=[]):  # line 861
    ...
    delta = '--delta' in options  # type: bool  # line 865
    import warnings  # line 866
    import zipfile  # line 866
    try:  # HINT zlib is the library that contains the deflated algorithm  # line 867
        import zlib  # HINT zlib is the library that contains the deflated algorithm  # line 867
        compression = zipfile.ZIP_DEFLATED  # HINT zlib is the library that contains the deflated algorithm  # line 867
    except:  # line 868
        compression = zipfile.ZIP_STORED  # line 868

    if argument is None:  # line 870
        Exit("Argument missing (target filename)")  # line 870
    argument = argument if "." in argument else argument + DUMP_FILE  # TODO this logic lacks a bit, "v1.2" would not receive the suffix  # line 871
    entries = []  # type: List[zipfile.ZipInfo]  # line 872
    if os.path.exists(encode(argument)):  # line 873
        try:  # line 874
            debug("Creating backup...")  # line 875
            shutil.copy2(encode(argument), encode(argument + BACKUP_SUFFIX))  # line 876
            if delta:  # line 877
                with zipfile.ZipFile(argument, "r") as _zip:  # list of pure relative paths without leading dot, normal slashes  # line 878
                    entries = _zip.namelist()  # list of pure relative paths without leading dot, normal slashes  # line 878
        except Exception as E:  # line 879
            Exit("Error creating backup copy before dumping. Please resolve and retry. %r" % E)  # line 879
    debug("Dumping revisions...")  # line 880
    if delta:  # don't show duplicate entries warnings  # line 881
        warnings.filterwarnings('ignore', 'Duplicate name.*', UserWarning, "zipfile", 0)  # don't show duplicate entries warnings  # line 881
    with zipfile.ZipFile(argument, "a", compression) as _zip:  # create  # line 882
        _zip.debug = 0  # no debugging output  # line 883
        _zip.comment = ("Repository dump from %r" % strftime()).encode(UTF8)  # line 884
        repopath = os.path.join(os.getcwd(), metaFolder)  # type: str  # line 885
        indicator = ProgressIndicator() if progress else None  # type: _coconut.typing.Optional[ProgressIndicator]  # line 886
        totalsize = 0  # type: int  # line 887
        start_time = time.time()  # type: float  # line 888
        for dirpath, dirnames, filenames in os.walk(repopath):  # TODO use index knowledge instead of walking to avoid adding stuff not needed?  # line 889
            dirpath = decode(dirpath)  # line 890
            if dirpath.endswith(BACKUP_SUFFIX):  # don't backup backups  # line 891
                continue  # don't backup backups  # line 891
            printo(dirpath.ljust(termWidth))  # TODO improve progress indicator output to | dir | dumpuing file  # line 892
            dirnames[:] = [decode(d) for d in dirnames]  # line 893
            filenames[:] = [decode(f) for f in filenames]  # line 894
            for filename in filenames:  # line 895
                abspath = os.path.join(dirpath, filename)  # type: str  # line 896
                relpath = os.path.relpath(abspath, repopath).replace(os.sep, "/")  # type: str  # line 897
                totalsize += os.stat(encode(abspath)).st_size  # line 898
                show = indicator.getIndicator() if progress else None  # type: _coconut.typing.Optional[str]  # line 899
                if relpath.endswith(BACKUP_SUFFIX):  # don't backup backups  # line 900
                    continue  # don't backup backups  # line 900
                if not delta or relpath.endswith(metaFile) or relpath not in entries:  # always update metadata, otherwise only add new revision files  # line 901
                    if show:  # line 902
                        printo(("\rDumping %s @%.2f MiB/s %s" % (show, totalsize / (MEBI * (time.time() - start_time)), filename)).ljust(termWidth), nl="")  # line 902
                    _zip.write(abspath, relpath)  # write entry into archive  # line 903
        if delta:  # line 904
            _zip.comment = ("Delta dump from %r" % strftime()).encode(UTF8)  # line 904
    info("\r" + (MARKER + "Finished dumping entire repository @%.2f MiB/s." % (totalsize / (MEBI * (time.time() - start_time)))).ljust(termWidth))  # clean line  # line 905

def config(arguments: 'List[str]', options: 'List[str]'=[]):  # line 907
    command, key, value = (arguments + [None] * 2)[:3]  # line 908
    if command not in ["set", "unset", "show", "list", "add", "rm"]:  # line 909
        Exit("Unknown config command")  # line 909
    local = "--local" in options  # type: bool  # line 910
    m = Metadata()  # type: Metadata  # loads layered configuration as well. TODO warning if repo not exists  # line 911
    c = m.c if local else m.c.__defaults  # type: configr.Configr  # line 912
    if command == "set":  # line 913
        if None in (key, value):  # line 914
            Exit("Key or value not specified")  # line 914
        if key not in (["defaultbranch"] + ([] if local else CONFIGURABLE_FLAGS) + CONFIGURABLE_LISTS):  # line 915
            Exit("Unsupported key for %s configuration %r" % ("local " if local else "global", key))  # line 915
        if key in CONFIGURABLE_FLAGS and value.lower() not in TRUTH_VALUES + FALSE_VALUES:  # line 916
            Exit("Cannot set flag to '%s'. Try on/off instead" % value.lower())  # line 916
        c[key] = value.lower() in TRUTH_VALUES if key in CONFIGURABLE_FLAGS else (removePath(key, value.strip()) if key not in CONFIGURABLE_LISTS else [removePath(key, v) for v in safeSplit(value, ";")])  # TODO sanitize texts?  # line 917
    elif command == "unset":  # line 918
        if key is None:  # line 919
            Exit("No key specified")  # line 919
        if key not in c.keys():  # HINT: Works on local configurations when used with --local  # line 920
            Exit("Unknown key")  # HINT: Works on local configurations when used with --local  # line 920
        del c[key]  # line 921
    elif command == "add":  # line 922
        if None in (key, value):  # line 923
            Exit("Key or value not specified")  # line 923
        if key not in CONFIGURABLE_LISTS:  # line 924
            Exit("Unsupported key %r" % key)  # line 924
        if key not in c.keys():  # prepare empty list, or copy from global, add new value below  # line 925
            c[key] = [_ for _ in c.__defaults[key]] if local else []  # prepare empty list, or copy from global, add new value below  # line 925
        elif value in c[key]:  # line 926
            Exit("Value already contained, nothing to do")  # line 926
        if ";" in value:  # line 927
            c[key].append(removePath(key, value))  # line 927
        else:  # line 928
            c[key].extend([removePath(key, v) for v in value.split(";")])  # line 928
    elif command == "rm":  # line 929
        if None in (key, value):  # line 930
            Exit("Key or value not specified")  # line 930
        if key not in c.keys():  # line 931
            Exit("Unknown key %r" % key)  # line 931
        if value not in c[key]:  # line 932
            Exit("Unknown value %r" % value)  # line 932
        c[key].remove(value)  # line 933
        if local and len(c[key]) == 0 and "--prune" in options:  # remove local enty, to fallback to global  # line 934
            del c[key]  # remove local enty, to fallback to global  # line 934
    else:  # Show or list  # line 935
        if key == "flags":  # list valid configuration items  # line 936
            printo(", ".join(CONFIGURABLE_FLAGS))  # list valid configuration items  # line 936
        elif key == "lists":  # line 937
            printo(", ".join(CONFIGURABLE_LISTS))  # line 937
        elif key == "texts":  # line 938
            printo(", ".join([_ for _ in defaults.keys() if _ not in (CONFIGURABLE_FLAGS + CONFIGURABLE_LISTS)]))  # line 938
        else:  # line 939
            out = {3: "[default]", 2: "[global] ", 1: "[local]  "}  # type: Dict[int, str]  # line 940
            c = m.c  # always use full configuration chain  # line 941
            try:  # attempt single key  # line 942
                assert key is not None  # line 943
                c[key]  # line 943
                l = key in c.keys()  # type: bool  # line 944
                g = key in c.__defaults.keys()  # type: bool  # line 944
                printo("%s %s %r" % (key.rjust(20), out[3] if not (l or g) else (out[1] if l else out[2]), c[key]))  # line 945
            except:  # normal value listing  # line 946
                vals = {k: (repr(v), 3) for k, v in defaults.items()}  # type: Dict[str, Tuple[str, int]]  # line 947
                vals.update({k: (repr(v), 2) for k, v in c.__defaults.items()})  # line 948
                vals.update({k: (repr(v), 1) for k, v in c.__map.items()})  # line 949
                for k, vt in sorted(vals.items()):  # line 950
                    printo("%s %s %s" % (k.rjust(20), out[vt[1]], vt[0]))  # line 950
                if len(c.keys()) == 0:  # line 951
                    info("No local configuration stored")  # line 951
                if len(c.__defaults.keys()) == 0:  # line 952
                    info("No global configuration stored")  # line 952
        return  # in case of list, no need to store anything  # line 953
    if local:  # saves changes of repoConfig  # line 954
        m.repoConf = c.__map  # saves changes of repoConfig  # line 954
        m.saveBranches()  # saves changes of repoConfig  # line 954
        Exit("OK", code=0)  # saves changes of repoConfig  # line 954
    else:  # global config  # line 955
        f, h = saveConfig(c)  # only saves c.__defaults (nested Configr)  # line 956
        if f is None:  # line 957
            error("Error saving user configuration: %r" % h)  # line 957
        else:  # line 958
            Exit("OK", code=0)  # line 958

def move(relPath: 'str', pattern: 'str', newRelPath: 'str', newPattern: 'str', options: 'List[str]'=[], negative: 'bool'=False):  # line 960
    ''' Path differs: Move files, create folder if not existing. Pattern differs: Attempt to rename file, unless exists in target or not unique.
      for "mvnot" don't do any renaming (or do?)
  '''  # line 963
    debug(MARKER + "Renaming %r to %r" % (pattern, newPattern))  # line 964
    force = '--force' in options  # type: bool  # line 965
    soft = '--soft' in options  # type: bool  # line 966
    if not os.path.exists(encode(relPath.replace(SLASH, os.sep))) and not force:  # line 967
        Exit("Source folder doesn't exist. Use --force to proceed anyway")  # line 967
    m = Metadata()  # type: Metadata  # line 968
    patterns = m.branches[m.branch].untracked if negative else m.branches[m.branch].tracked  # type: List[str]  # line 969
    matching = fnmatch.filter(os.listdir(relPath.replace(SLASH, os.sep)) if os.path.exists(encode(relPath.replace(SLASH, os.sep))) else [], os.path.basename(pattern))  # type: List[str]  # find matching files in source  # line 970
    matching[:] = [f for f in matching if len([n for n in m.c.ignores if fnmatch.fnmatch(f, n)]) == 0 or len([p for p in m.c.ignoresWhitelist if fnmatch.fnmatch(f, p)]) > 0]  # line 971
    if not matching and not force:  # line 972
        Exit("No files match the specified file pattern. Use --force to proceed anyway")  # line 972
    if not (m.track or m.picky):  # line 973
        Exit("Repository is in simple mode. Simply use basic file operations to modify files, then execute 'sos commit' to version the changes")  # line 973
    if pattern not in patterns:  # list potential alternatives and exit  # line 974
        for tracked in (t for t in patterns if os.path.dirname(t) == relPath):  # for all patterns of the same source folder  # line 975
            alternative = fnmatch.filter(matching, os.path.basename(tracked))  # type: _coconut.typing.Sequence[str]  # find if it matches any of the files in the source folder, too  # line 976
            if alternative:  # line 977
                info("  '%s' matches %d files" % (tracked, len(alternative)))  # line 977
        if not (force or soft):  # line 978
            Exit("File pattern '%s' is not tracked on current branch. 'sos move' only works on tracked patterns" % pattern)  # line 978
    basePattern = os.path.basename(pattern)  # type: str  # pure glob without folder  # line 979
    newBasePattern = os.path.basename(newPattern)  # type: str  # line 980
    if basePattern.count("*") < newBasePattern.count("*") or (basePattern.count("?") - basePattern.count("[?]")) < (newBasePattern.count("?") - newBasePattern.count("[?]")) or (basePattern.count("[") - basePattern.count("\\[")) < (newBasePattern.count("[") - newBasePattern.count("\\[")) or (basePattern.count("]") - basePattern.count("\\]")) < (newBasePattern.count("]") - newBasePattern.count("\\]")):  # line 981
        Exit("Glob markers from '%s' to '%s' don't match, cannot move/rename tracked matching files" % (basePattern, newBasePattern))  # line 985
    oldTokens = None  # type: _coconut.typing.Sequence[GlobBlock]  # line 986
    newToken = None  # type: _coconut.typing.Sequence[GlobBlock]  # line 986
    oldTokens, newTokens = tokenizeGlobPatterns(os.path.basename(pattern), os.path.basename(newPattern))  # line 987
    matches = convertGlobFiles(matching, oldTokens, newTokens)  # type: _coconut.typing.Sequence[Tuple[str, str]]  # computes list of source - target filename pairs  # line 988
    if len({st[1] for st in matches}) != len(matches):  # line 989
        Exit("Some target filenames are not unique and different move/rename actions would point to the same target file")  # line 989
    matches = reorderRenameActions(matches, exitOnConflict=not soft)  # attempts to find conflict-free renaming order, or exits  # line 990
    if os.path.exists(encode(newRelPath)):  # line 991
        exists = [filename[1] for filename in matches if os.path.exists(encode(os.path.join(newRelPath, filename[1]).replace(SLASH, os.sep)))]  # type: _coconut.typing.Sequence[str]  # line 992
        if exists and not (force or soft):  # line 993
            Exit("%s files would write over existing files in %s cases. Use --force to execute it anyway" % ("Moving" if relPath != newRelPath else "Renaming", "all" if len(exists) == len(matches) else "some"))  # line 993
    else:  # line 994
        os.makedirs(encode(os.path.abspath(newRelPath.replace(SLASH, os.sep))))  # line 994
    if not soft:  # perform actual renaming  # line 995
        for (source, target) in matches:  # line 996
            try:  # line 997
                shutil.move(encode(os.path.abspath(os.path.join(relPath, source).replace(SLASH, os.sep))), encode(os.path.abspath(os.path.join(newRelPath, target).replace(SLASH, os.sep))))  # line 997
            except Exception as E:  # one error can lead to another in case of delicate renaming order  # line 998
                error("Cannot move/rename file '%s' to '%s'" % (source, os.path.join(newRelPath, target)))  # one error can lead to another in case of delicate renaming order  # line 998
    patterns[patterns.index(pattern)] = newPattern  # line 999
    m.saveBranches()  # line 1000

def parse(root: 'str', cwd: 'str', cmd: 'str'):  # line 1002
    ''' Main operation. Main has already chdir into VCS root folder, cwd is original working directory for add, rm, mv. '''  # line 1003
    debug("Parsing command-line arguments...")  # line 1004
    try:  # line 1005
        onlys, excps = parseOnlyOptions(cwd, sys.argv)  # extracts folder-relative information for changes, commit, diff, switch, update  # line 1006
        command = sys.argv[1].strip() if len(sys.argv) > 1 else ""  # line 1007
        arguments = [c.strip() for c in sys.argv[2:] if not c.startswith("--")]  # type: List[_coconut.typing.Optional[str]]  # line 1008
        options = [c.strip() for c in sys.argv[2:] if c.startswith("--")]  # line 1009
        debug("Processing command %r with arguments %r and options %r." % (command, [_ for _ in arguments if _ is not None], options))  # line 1010
        if command[:1] in "amr":  # line 1011
            relPath, pattern = relativize(root, os.path.join(cwd, arguments[0] if arguments else "."))  # line 1011
        if command[:1] == "m":  # line 1012
            if len(arguments) < 2:  # line 1013
                Exit("Need a second file pattern argument as target for move command")  # line 1013
            newRelPath, newPattern = relativize(root, os.path.join(cwd, arguments[1]))  # line 1014
        arguments[:] = (arguments + [None] * 3)[:3]  # line 1015
        if command[:1] == "a":  # addnot  # line 1016
            add(relPath, pattern, options, negative="n" in command)  # addnot  # line 1016
        elif command[:1] == "b":  # line 1017
            branch(arguments[0], arguments[1], options)  # line 1017
        elif command[:3] == "com":  # line 1018
            commit(arguments[0], options, onlys, excps)  # line 1018
        elif command[:2] == "ch":  # "changes" (legacy)  # line 1019
            changes(arguments[0], options, onlys, excps)  # "changes" (legacy)  # line 1019
        elif command[:2] == "ci":  # line 1020
            commit(arguments[0], options, onlys, excps)  # line 1020
        elif command[:3] == 'con':  # line 1021
            config(arguments, options)  # line 1021
        elif command[:2] == "de":  # line 1022
            destroy(arguments[0], options)  # line 1022
        elif command[:2] == "di":  # line 1023
            diff(arguments[0], options, onlys, excps)  # line 1023
        elif command[:2] == "du":  # line 1024
            dump(arguments[0], options)  # line 1024
        elif command[:1] == "h":  # line 1025
            usage(APPNAME, version.__version__)  # line 1025
        elif command[:2] == "lo":  # line 1026
            log(options)  # line 1026
        elif command[:2] == "li":  # line 1027
            ls(os.path.relpath((lambda _coconut_none_coalesce_item: cwd if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(arguments[0]), root), options)  # line 1027
        elif command[:2] == "ls":  # line 1028
            ls(os.path.relpath((lambda _coconut_none_coalesce_item: cwd if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(arguments[0]), root), options)  # line 1028
        elif command[:1] == "m":  # mvnot  # line 1029
            move(relPath, pattern, newRelPath, newPattern, options, negative="n" in command)  # mvnot  # line 1029
        elif command[:2] == "of":  # line 1030
            offline(arguments[0], arguments[1], options)  # line 1030
        elif command[:2] == "on":  # line 1031
            online(options)  # line 1031
        elif command[:1] == "r":  # rmnot  # line 1032
            remove(relPath, pattern, negative="n" in command)  # rmnot  # line 1032
        elif command[:2] == "st":  # line 1033
            status(arguments[0], cwd, cmd, options, onlys, excps)  # line 1033
        elif command[:2] == "sw":  # line 1034
            switch(arguments[0], options, onlys, excps)  # line 1034
        elif command[:1] == "u":  # line 1035
            update(arguments[0], options, onlys, excps)  # line 1035
        elif command[:1] == "v":  # line 1036
            usage(APPNAME, version.__version__, short=True)  # line 1036
        else:  # line 1037
            Exit("Unknown command '%s'" % command)  # line 1037
        Exit(code=0)  # regular exit  # line 1038
    except (Exception, RuntimeError) as E:  # line 1039
        exception(E)  # line 1040
        Exit("An internal error occurred in SOS. Please report above message to the project maintainer at  https://github.com/ArneBachmann/sos/issues  via 'New Issue'.\nPlease state your installed version via 'sos version', and what you were doing")  # line 1041

def main():  # line 1043
    global debug, info, warn, error  # to modify logger  # line 1044
    logging.basicConfig(level=level, stream=sys.stderr, format=("%(asctime)-23s %(levelname)-8s %(name)s:%(lineno)d | %(message)s" if '--log' in sys.argv else "%(message)s"))  # line 1045
    _log = Logger(logging.getLogger(__name__))  # line 1046
    debug, info, warn, error = _log.debug, _log.info, _log.warn, _log.error  # line 1046
    for option in (o for o in ['--log', '--verbose', '-v', '--sos'] if o in sys.argv):  # clean up program arguments  # line 1047
        sys.argv.remove(option)  # clean up program arguments  # line 1047
    if '--help' in sys.argv or len(sys.argv) < 2:  # line 1048
        usage(APPNAME, version.__version__)  # line 1048
    command = sys.argv[1] if len(sys.argv) > 1 else None  # type: _coconut.typing.Optional[str]  # line 1049
    root, vcs, cmd = findSosVcsBase()  # root is None if no .sos folder exists up the folder tree (still working online); vcs is checkout/repo root folder; cmd is the VCS base command  # line 1050
    debug("Found root folders for SOS|VCS: %s|%s" % (("-" if root is None else root), ("-" if vcs is None else vcs)))  # line 1051
    defaults["defaultbranch"] = (lambda _coconut_none_coalesce_item: "default" if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(vcsBranches.get(cmd, "trunk"))  # sets dynamic default with SVN fallback  # line 1052
    defaults["useChangesCommand"] = cmd == "fossil"  # sets dynamic default with SVN fallback  # line 1053
    if force_sos or root is not None or (("" if command is None else command))[:2] == "of" or (("" if command is None else command))[:1] in "hv":  # in offline mode or just going offline TODO what about git config?  # line 1054
        cwd = os.getcwd()  # line 1055
        os.chdir(cwd if command[:2] == "of" else (cwd if root is None else root))  # line 1056
        parse(root, cwd, cmd)  # line 1057
    elif force_vcs or cmd is not None:  # online mode - delegate to VCS  # line 1058
        info("%s: Running '%s %s'" % (COMMAND.upper(), cmd, " ".join(sys.argv[1:])))  # line 1059
        import subprocess  # only required in this section  # line 1060
        process = subprocess.Popen([cmd] + sys.argv[1:], shell=False, stdin=subprocess.PIPE, stdout=sys.stdout, stderr=sys.stderr)  # line 1061
        inp = ""  # type: str  # line 1062
        while True:  # line 1063
            so, se = process.communicate(input=inp)  # line 1064
            if process.returncode is not None:  # line 1065
                break  # line 1065
            inp = sys.stdin.read()  # line 1066
        if sys.argv[1][:2] == "co" and process.returncode == 0:  # successful commit - assume now in sync again (but leave meta data folder with potential other feature branches behind until "online")  # line 1067
            if root is None:  # line 1068
                Exit("Cannot determine VCS root folder: Unable to mark repository as synchronized and will show a warning when leaving offline mode")  # line 1068
            m = Metadata(root)  # type: Metadata  # line 1069
            m.branches[m.branch] = dataCopy(BranchInfo, m.branches[m.branch], inSync=True)  # mark as committed  # line 1070
            m.saveBranches()  # line 1071
    else:  # line 1072
        Exit("No offline repository present, and unable to detect VCS file tree")  # line 1072


# Main part
verbose = os.environ.get("DEBUG", "False").lower() == "true" or '--verbose' in sys.argv or '-v' in sys.argv  # imported from utility, and only modified here  # line 1076
level = logging.DEBUG if verbose else logging.INFO  # line 1077
force_sos = '--sos' in sys.argv  # type: bool  # line 1078
force_vcs = '--vcs' in sys.argv  # type: bool  # line 1079
_log = Logger(logging.getLogger(__name__))  # line 1080
debug, info, warn, error = _log.debug, _log.info, _log.warn, _log.error  # line 1080
if __name__ == '__main__':  # line 1081
    main()  # line 1081
