...
def log(options: '_coconut.typing.Sequence[str]'=[]):  # line 837
    ...
    last = {}  # type: Dict[str, PathInfo]  # path infos from previous revision  # line 846
    ...
    for no in range(maxi + 1):  # line 848
        ...
        _add = news - olds  # type: FrozenSet[str]  # line 856
        _del = olds - news  # type: FrozenSet[str]  # line 857
#    _mod_:Dict[str,PathInfo] = {k: nxts[k] for k in news - _add - _del}
        _mod = frozenset([_ for _, info in {k: nxts[k] for k in news - _add - _del}.items() if last[_].size != info.size or (last[_].hash != info.hash if m.strict else last[_].mtime != info.mtime)])  # type: FrozenSet[str]  # line 859
#    _mov:FrozenSet[str] = detectMoves(ChangeSet(nxts, {o: None for o in olds})  # TODO determine moves - can we reuse detectMoves(changes)?
        _txt = len([a for a in _add if m.isTextType(a)])  # type: int  # line 861
        printo("  %s r%s @%s (+%02d/-%02d/%s%02d/T%02d) |%s|%s" % ("*" if commit.number == maxi else " ", ("%%%ds" % nl) % commit.number, strftime(commit.ctime), len(_add), len(_del), PLUSMINUS_SYMBOL if m.c.useUnicodeFont else "~", len(_mod), _txt, ((lambda _coconut_none_coalesce_item: "" if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(commit.message)), "TAG" if ((lambda _coconut_none_coalesce_item: "" if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(commit.message)) in m.tags else ""))  # line 862
        if '--changes' in options:  # TODO moves detection?  # line 863
            m.listChanges(ChangeSet({a: None for a in _add}, {d: None for d in _del}, {m: None for m in _mod}, {}))  # TODO moves detection?  # line 863
        if '--diff' in options:  #  _diff(m, changes)  # needs from revision diff  # line 864
            pass  #  _diff(m, changes)  # needs from revision diff  # line 864
        olds = news  # replaces olds for next revision compare  # line 865
        last = {k: v for k, v in nxts.items()}  # create new reference  # line 866

def dump(argument: 'str', options: '_coconut.typing.Sequence[str]'=[]):  # line 868
    ''' Exported entire repository as archive for easy transfer. '''  # line 869
    debug(MARKER + "Dumping repository to archive...")  # line 870
    m = Metadata()  # type: Metadata  # to load the configuration  # line 871
    progress = '--progress' in options  # type: bool  # line 872
    delta = not '--full' in options  # type: bool  # line 873
    skipBackup = '--skip-backup' in options  # type: bool  # line 874
    import warnings  # line 875
    import zipfile  # line 875
    try:  # HINT zlib is the library that contains the deflated algorithm  # line 876
        import zlib  # HINT zlib is the library that contains the deflated algorithm  # line 876
        compression = zipfile.ZIP_DEFLATED  # HINT zlib is the library that contains the deflated algorithm  # line 876
    except:  # line 877
        compression = zipfile.ZIP_STORED  # line 877

    if argument is None:  # line 879
        Exit("Argument missing (target filename)")  # line 879
    argument = argument if "." in argument else argument + DUMP_FILE  # TODO this logic lacks a bit, "v1.2" would not receive the suffix  # line 880
    entries = []  # type: List[str]  # line 881
    if os.path.exists(encode(argument)) and not skipBackup:  # line 882
        try:  # line 883
            debug("Creating backup...")  # line 884
            shutil.copy2(encode(argument), encode(argument + BACKUP_SUFFIX))  # line 885
            if delta:  # line 886
                with zipfile.ZipFile(argument, "r") as _zip:  # list of pure relative paths without leading dot, normal slashes  # line 887
                    entries = _zip.namelist()  # list of pure relative paths without leading dot, normal slashes  # line 887
        except Exception as E:  # line 888
            Exit("Error creating backup copy before dumping. Please resolve and retry. %r" % E)  # line 888
    debug("Dumping revisions...")  # line 889
    if delta:  # don't show duplicate entries warnings  # line 890
        warnings.filterwarnings('ignore', 'Duplicate name.*', UserWarning, "zipfile", 0)  # don't show duplicate entries warnings  # line 890
    with zipfile.ZipFile(argument, "a" if delta else "w", compression) as _zip:  # create  # line 891
        _zip.debug = 0  # suppress debugging output  # line 892
        _zip.comment = ("Repository dump from %r" % strftime()).encode(UTF8)  # line 893
        repopath = os.path.join(os.getcwd(), metaFolder)  # type: str  # line 894
        indicator = ProgressIndicator(PROGRESS_MARKER[1 if m.c.useUnicodeFont else 0]) if progress else None  # type: _coconut.typing.Optional[ProgressIndicator]  # line 895
        totalsize = 0  # type: int  # line 896
        start_time = time.time()  # type: float  # line 897
        for dirpath, dirnames, filenames in os.walk(repopath):  # TODO use index knowledge instead of walking to avoid adding stuff not needed?  # line 898
            dirpath = decode(dirpath)  # line 899
            if dirpath.endswith(BACKUP_SUFFIX):  # don't backup backups  # line 900
                continue  # don't backup backups  # line 900
            printo(ljust(dirpath))  # TODO improve progress indicator output to | dir | dumpuing file  # line 901
            dirnames[:] = sorted([decode(d) for d in dirnames])  # HINT sort for reproducible delta dumps  # line 902
            filenames[:] = sorted([decode(f) for f in filenames])  # line 903
            for filename in filenames:  # line 904
                abspath = os.path.join(dirpath, filename)  # type: str  # line 905
                relpath = os.path.join(metaFolder, os.path.relpath(abspath, repopath)).replace(os.sep, "/")  # type: str  # line 906
                totalsize += os.stat(encode(abspath)).st_size  # line 907
                show = indicator.getIndicator() if progress else None  # type: _coconut.typing.Optional[str]  # line 908
                if relpath.endswith(BACKUP_SUFFIX):  # don't backup backups  # line 909
                    continue  # don't backup backups  # line 909
                if not delta or relpath.endswith(metaFile) or relpath not in entries:  # always update metadata, otherwise only add new revision files  # line 910
                    if show:  # line 911
                        printo("\r" + ljust("Dumping %s @%.2f MiB/s %s" % (show, totalsize / (MEBI * (time.time() - start_time)), filename)), nl="")  # line 911
                    _zip.write(abspath, relpath)  # write entry into archive  # line 912
        if delta:  # line 913
            _zip.comment = ("Delta dump from %r" % strftime()).encode(UTF8)  # line 913
    info("\r" + ljust(MARKER + "Finished dumping entire repository @%.2f MiB/s." % (totalsize / (MEBI * (time.time() - start_time)))))  # clean line  # line 914

def config(arguments: 'List[str]', options: 'List[str]'=[]):  # line 916
    command, key, value = (arguments + [None] * 2)[:3]  # line 917
    if command not in ["set", "unset", "show", "list", "add", "rm"]:  # line 918
        Exit("Unknown config command")  # line 918
    local = "--local" in options  # type: bool  # line 919
    m = Metadata()  # type: Metadata  # loads layered configuration as well. TODO warning if repo not exists  # line 920
    c = m.c if local else m.c.__defaults  # type: configr.Configr  # line 921
    if command == "set":  # line 922
        if None in (key, value):  # line 923
            Exit("Key or value not specified")  # line 923
        if key not in (["defaultbranch"] + ([] if local else CONFIGURABLE_FLAGS) + CONFIGURABLE_LISTS):  # line 924
            Exit("Unsupported key for %s configuration %r" % ("local " if local else "global", key))  # line 924
        if key in CONFIGURABLE_FLAGS and value.lower() not in TRUTH_VALUES + FALSE_VALUES:  # line 925
            Exit("Cannot set flag to '%s'. Try on/off instead" % value.lower())  # line 925
        c[key] = value.lower() in TRUTH_VALUES if key in CONFIGURABLE_FLAGS else (removePath(key, value.strip()) if key not in CONFIGURABLE_LISTS else [removePath(key, v) for v in safeSplit(value, ";")])  # TODO sanitize texts?  # line 926
    elif command == "unset":  # line 927
        if key is None:  # line 928
            Exit("No key specified")  # line 928
        if key not in c.keys():  # HINT: Works on local configurations when used with --local  # line 929
            Exit("Unknown key")  # HINT: Works on local configurations when used with --local  # line 929
        del c[key]  # line 930
    elif command == "add":  # line 931
        if None in (key, value):  # line 932
            Exit("Key or value not specified")  # line 932
        if key not in CONFIGURABLE_LISTS:  # line 933
            Exit("Unsupported key %r" % key)  # line 933
        if key not in c.keys():  # prepare empty list, or copy from global, add new value below  # line 934
            c[key] = [_ for _ in c.__defaults[key]] if local else []  # prepare empty list, or copy from global, add new value below  # line 934
        elif value in c[key]:  # line 935
            Exit("Value already contained, nothing to do")  # line 935
        if ";" in value:  # line 936
            c[key].append(removePath(key, value))  # line 936
        else:  # line 937
            c[key].extend([removePath(key, v) for v in value.split(";")])  # line 937
    elif command == "rm":  # line 938
        if None in (key, value):  # line 939
            Exit("Key or value not specified")  # line 939
        if key not in c.keys():  # line 940
            Exit("Unknown key %r" % key)  # line 940
        if value not in c[key]:  # line 941
            Exit("Unknown value %r" % value)  # line 941
        c[key].remove(value)  # line 942
        if local and len(c[key]) == 0 and "--prune" in options:  # remove local enty, to fallback to global  # line 943
            del c[key]  # remove local enty, to fallback to global  # line 943
    else:  # Show or list  # line 944
        if key == "flags":  # list valid configuration items  # line 945
            printo(", ".join(CONFIGURABLE_FLAGS))  # list valid configuration items  # line 945
        elif key == "lists":  # line 946
            printo(", ".join(CONFIGURABLE_LISTS))  # line 946
        elif key == "texts":  # line 947
            printo(", ".join([_ for _ in defaults.keys() if _ not in (CONFIGURABLE_FLAGS + CONFIGURABLE_LISTS)]))  # line 947
        else:  # line 948
            out = {3: "[default]", 2: "[global] ", 1: "[local]  "}  # type: Dict[int, str]  # line 949
            c = m.c  # always use full configuration chain  # line 950
            try:  # attempt single key  # line 951
                assert key is not None  # line 952
                c[key]  # line 952
                l = key in c.keys()  # type: bool  # line 953
                g = key in c.__defaults.keys()  # type: bool  # line 953
                printo("%s %s %r" % (key.rjust(20), out[3] if not (l or g) else (out[1] if l else out[2]), c[key]))  # line 954
            except:  # normal value listing  # line 955
                vals = {k: (repr(v), 3) for k, v in defaults.items()}  # type: Dict[str, Tuple[str, int]]  # line 956
                vals.update({k: (repr(v), 2) for k, v in c.__defaults.items()})  # line 957
                vals.update({k: (repr(v), 1) for k, v in c.__map.items()})  # line 958
                for k, vt in sorted(vals.items()):  # line 959
                    printo("%s %s %s" % (k.rjust(20), out[vt[1]], vt[0]))  # line 959
                if len(c.keys()) == 0:  # line 960
                    info("No local configuration stored")  # line 960
                if len(c.__defaults.keys()) == 0:  # line 961
                    info("No global configuration stored")  # line 961
        return  # in case of list, no need to store anything  # line 962
    if local:  # saves changes of repoConfig  # line 963
        m.repoConf = c.__map  # saves changes of repoConfig  # line 963
        m.saveBranches()  # saves changes of repoConfig  # line 963
        Exit("OK", code=0)  # saves changes of repoConfig  # line 963
    else:  # global config  # line 964
        f, h = saveConfig(c)  # only saves c.__defaults (nested Configr)  # line 965
        if f is None:  # line 966
            error("Error saving user configuration: %r" % h)  # line 966
        else:  # line 967
            Exit("OK", code=0)  # line 967

def move(relPath: 'str', pattern: 'str', newRelPath: 'str', newPattern: 'str', options: 'List[str]'=[], negative: 'bool'=False):  # line 969
    ''' Path differs: Move files, create folder if not existing. Pattern differs: Attempt to rename file, unless exists in target or not unique.
      for "mvnot" don't do any renaming (or do?)
  '''  # line 972
    debug(MARKER + "Renaming %r to %r" % (pattern, newPattern))  # line 973
    force = '--force' in options  # type: bool  # line 974
    soft = '--soft' in options  # type: bool  # line 975
    if not os.path.exists(encode(relPath.replace(SLASH, os.sep))) and not force:  # line 976
        Exit("Source folder doesn't exist. Use --force to proceed anyway")  # line 976
    m = Metadata()  # type: Metadata  # line 977
    patterns = m.branches[m.branch].untracked if negative else m.branches[m.branch].tracked  # type: List[str]  # line 978
    matching = fnmatch.filter(os.listdir(relPath.replace(SLASH, os.sep)) if os.path.exists(encode(relPath.replace(SLASH, os.sep))) else [], os.path.basename(pattern))  # type: List[str]  # find matching files in source  # line 979
    matching[:] = [f for f in matching if len([n for n in m.c.ignores if fnmatch.fnmatch(f, n)]) == 0 or len([p for p in m.c.ignoresWhitelist if fnmatch.fnmatch(f, p)]) > 0]  # line 980
    if not matching and not force:  # line 981
        Exit("No files match the specified file pattern. Use --force to proceed anyway")  # line 981
    if not (m.track or m.picky):  # line 982
        Exit("Repository is in simple mode. Simply use basic file operations to modify files, then execute 'sos commit' to version the changes")  # line 982
    if pattern not in patterns:  # list potential alternatives and exit  # line 983
        for tracked in (t for t in patterns if os.path.dirname(t) == relPath):  # for all patterns of the same source folder  # line 984
            alternative = fnmatch.filter(matching, os.path.basename(tracked))  # type: _coconut.typing.Sequence[str]  # find if it matches any of the files in the source folder, too  # line 985
            if alternative:  # line 986
                info("  '%s' matches %d files" % (tracked, len(alternative)))  # line 986
        if not (force or soft):  # line 987
            Exit("File pattern '%s' is not tracked on current branch. 'sos move' only works on tracked patterns" % pattern)  # line 987
    basePattern = os.path.basename(pattern)  # type: str  # pure glob without folder  # line 988
    newBasePattern = os.path.basename(newPattern)  # type: str  # line 989
    if basePattern.count("*") < newBasePattern.count("*") or (basePattern.count("?") - basePattern.count("[?]")) < (newBasePattern.count("?") - newBasePattern.count("[?]")) or (basePattern.count("[") - basePattern.count("\\[")) < (newBasePattern.count("[") - newBasePattern.count("\\[")) or (basePattern.count("]") - basePattern.count("\\]")) < (newBasePattern.count("]") - newBasePattern.count("\\]")):  # line 990
        Exit("Glob markers from '%s' to '%s' don't match, cannot move/rename tracked matching files" % (basePattern, newBasePattern))  # line 994
    oldTokens = None  # type: _coconut.typing.Sequence[GlobBlock]  # line 995
    newToken = None  # type: _coconut.typing.Sequence[GlobBlock]  # line 995
    oldTokens, newTokens = tokenizeGlobPatterns(os.path.basename(pattern), os.path.basename(newPattern))  # line 996
    matches = convertGlobFiles(matching, oldTokens, newTokens)  # type: _coconut.typing.Sequence[Tuple[str, str]]  # computes list of source - target filename pairs  # line 997
    if len({st[1] for st in matches}) != len(matches):  # line 998
        Exit("Some target filenames are not unique and different move/rename actions would point to the same target file")  # line 998
    matches = reorderRenameActions(matches, exitOnConflict=not soft)  # attempts to find conflict-free renaming order, or exits  # line 999
    if os.path.exists(encode(newRelPath)):  # line 1000
        exists = [filename[1] for filename in matches if os.path.exists(encode(os.path.join(newRelPath, filename[1]).replace(SLASH, os.sep)))]  # type: _coconut.typing.Sequence[str]  # line 1001
        if exists and not (force or soft):  # line 1002
            Exit("%s files would write over existing files in %s cases. Use --force to execute it anyway" % ("Moving" if relPath != newRelPath else "Renaming", "all" if len(exists) == len(matches) else "some"))  # line 1002
    else:  # line 1003
        os.makedirs(encode(os.path.abspath(newRelPath.replace(SLASH, os.sep))))  # line 1003
    if not soft:  # perform actual renaming  # line 1004
        for (source, target) in matches:  # line 1005
            try:  # line 1006
                shutil.move(encode(os.path.abspath(os.path.join(relPath, source).replace(SLASH, os.sep))), encode(os.path.abspath(os.path.join(newRelPath, target).replace(SLASH, os.sep))))  # line 1006
            except Exception as E:  # one error can lead to another in case of delicate renaming order  # line 1007
                error("Cannot move/rename file '%s' to '%s'" % (source, os.path.join(newRelPath, target)))  # one error can lead to another in case of delicate renaming order  # line 1007
    patterns[patterns.index(pattern)] = newPattern  # line 1008
    m.saveBranches()  # line 1009

def parse(root: 'str', cwd: 'str', cmd: 'str'):  # line 1011
    ''' Main operation. Main has already chdir into VCS root folder, cwd is original working directory for add, rm, mv. '''  # line 1012
    debug("Parsing command-line arguments...")  # line 1013
    try:  # line 1014
        onlys, excps = parseOnlyOptions(cwd, sys.argv)  # extracts folder-relative information for changes, commit, diff, switch, update  # line 1015
        command = sys.argv[1].strip() if len(sys.argv) > 1 else ""  # line 1016
        arguments = [c.strip() for c in sys.argv[2:] if not c.startswith("--")]  # type: List[_coconut.typing.Optional[str]]  # line 1017
        options = [c.strip() for c in sys.argv[2:] if c.startswith("--")]  # line 1018
        debug("Processing command %r with arguments %r and options %r." % (command, [_ for _ in arguments if _ is not None], options))  # line 1019
        if command[:1] in "amr":  # line 1020
            relPath, pattern = relativize(root, os.path.join(cwd, arguments[0] if arguments else "."))  # line 1020
        if command[:1] == "m":  # line 1021
            if len(arguments) < 2:  # line 1022
                Exit("Need a second file pattern argument as target for move command")  # line 1022
            newRelPath, newPattern = relativize(root, os.path.join(cwd, arguments[1]))  # line 1023
        arguments[:] = (arguments + [None] * 3)[:3]  # line 1024
        if command[:1] == "a":  # addnot  # line 1025
            add(relPath, pattern, options, negative="n" in command)  # addnot  # line 1025
        elif command[:1] == "b":  # line 1026
            branch(arguments[0], arguments[1], options)  # line 1026
        elif command[:3] == "com":  # line 1027
            commit(arguments[0], options, onlys, excps)  # line 1027
        elif command[:2] == "ch":  # "changes" (legacy)  # line 1028
            changes(arguments[0], options, onlys, excps)  # "changes" (legacy)  # line 1028
        elif command[:2] == "ci":  # line 1029
            commit(arguments[0], options, onlys, excps)  # line 1029
        elif command[:3] == 'con':  # line 1030
            config(arguments, options)  # line 1030
        elif command[:2] == "de":  # line 1031
            destroy(arguments[0], options)  # line 1031
        elif command[:2] == "di":  # line 1032
            diff(arguments[0], options, onlys, excps)  # line 1032
        elif command[:2] == "du":  # line 1033
            dump(arguments[0], options)  # line 1033
        elif command[:1] == "h":  # line 1034
            usage(APPNAME, version.__version__)  # line 1034
        elif command[:2] == "lo":  # line 1035
            log(options)  # line 1035
        elif command[:2] == "li":  # line 1036
            ls(os.path.relpath((lambda _coconut_none_coalesce_item: cwd if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(arguments[0]), root), options)  # line 1036
        elif command[:2] == "ls":  # line 1037
        ...
        elif command[:1] == "m":  # mvnot  # line 1038
            move(relPath, pattern, newRelPath, newPattern, options, negative="n" in command)  # mvnot  # line 1038
        elif command[:2] == "of":  # line 1039
            offline(arguments[0], arguments[1], options)  # line 1039
        elif command[:2] == "on":  # line 1040
            online(options)  # line 1040
        elif command[:1] == "r":  # rmnot  # line 1041
            remove(relPath, pattern, negative="n" in command)  # rmnot  # line 1041
        elif command[:2] == "st":  # line 1042
            status(arguments[0], cwd, cmd, options, onlys, excps)  # line 1042
        elif command[:2] == "sw":  # line 1043
            switch(arguments[0], options, onlys, excps)  # line 1043
        elif command[:1] == "u":  # line 1044
            update(arguments[0], options, onlys, excps)  # line 1044
        elif command[:1] == "v":  # line 1045
            usage(APPNAME, version.__version__, short=True)  # line 1045
        else:  # line 1046
            Exit("Unknown command '%s'" % command)  # line 1046
        Exit(code=0)  # regular exit  # line 1047
    except (Exception, RuntimeError) as E:  # line 1048
        exception(E)  # line 1049
        Exit("An internal error occurred in SOS. Please report above message to the project maintainer at  https://github.com/ArneBachmann/sos/issues  via 'New Issue'.\nPlease state your installed version via 'sos version', and what you were doing")  # line 1050

def main():  # line 1052
    global debug, info, warn, error  # to modify logger  # line 1053
    logging.basicConfig(level=level, stream=sys.stderr, format=("%(asctime)-23s %(levelname)-8s %(name)s:%(lineno)d | %(message)s" if '--log' in sys.argv else "%(message)s"))  # line 1054
    _log = Logger(logging.getLogger(__name__))  # line 1055
    debug, info, warn, error = _log.debug, _log.info, _log.warn, _log.error  # line 1055
    for option in (o for o in ['--log', '--verbose', '-v', '--sos'] if o in sys.argv):  # clean up program arguments  # line 1056
        sys.argv.remove(option)  # clean up program arguments  # line 1056
    if '--help' in sys.argv or len(sys.argv) < 2:  # line 1057
        usage(APPNAME, version.__version__)  # line 1057
    command = sys.argv[1] if len(sys.argv) > 1 else None  # type: _coconut.typing.Optional[str]  # line 1058
    root, vcs, cmd = findSosVcsBase()  # root is None if no .sos folder exists up the folder tree (still working online); vcs is checkout/repo root folder; cmd is the VCS base command  # line 1059
    debug("Found root folders for SOS|VCS: %s|%s" % (("-" if root is None else root), ("-" if vcs is None else vcs)))  # line 1060
    defaults["defaultbranch"] = (lambda _coconut_none_coalesce_item: "default" if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(vcsBranches.get(cmd, "trunk"))  # sets dynamic default with SVN fallback  # line 1061
    defaults["useChangesCommand"] = cmd == "fossil"  # sets dynamic default with SVN fallback  # line 1062
    if force_sos or root is not None or (("" if command is None else command))[:2] == "of" or (("" if command is None else command))[:1] in "hv":  # in offline mode or just going offline TODO what about git config?  # line 1063
        cwd = os.getcwd()  # line 1064
        os.chdir(cwd if command[:2] == "of" else (cwd if root is None else root))  # line 1065
        parse(root, cwd, cmd)  # line 1066
    elif force_vcs or cmd is not None:  # online mode - delegate to VCS  # line 1067
        info("%s: Running '%s %s'" % (COMMAND.upper(), cmd, " ".join(sys.argv[1:])))  # line 1068
        import subprocess  # only required in this section  # line 1069
        process = subprocess.Popen([cmd] + sys.argv[1:], shell=False, stdin=subprocess.PIPE, stdout=sys.stdout, stderr=sys.stderr)  # line 1070
        inp = ""  # type: str  # line 1071
        while True:  # line 1072
            so, se = process.communicate(input=inp)  # line 1073
            if process.returncode is not None:  # line 1074
                break  # line 1074
            inp = sys.stdin.read()  # line 1075
        if sys.argv[1][:2] == "co" and process.returncode == 0:  # successful commit - assume now in sync again (but leave meta data folder with potential other feature branches behind until "online")  # line 1076
            if root is None:  # line 1077
                Exit("Cannot determine VCS root folder: Unable to mark repository as synchronized and will show a warning when leaving offline mode")  # line 1077
            m = Metadata(root)  # type: Metadata  # line 1078
            m.branches[m.branch] = dataCopy(BranchInfo, m.branches[m.branch], inSync=True)  # mark as committed  # line 1079
            m.saveBranches()  # line 1080
    else:  # line 1081
        Exit("No offline repository present, and unable to detect VCS file tree")  # line 1081


# Main part
verbose = os.environ.get("DEBUG", "False").lower() == "true" or '--verbose' in sys.argv or '-v' in sys.argv  # imported from utility, and only modified here  # line 1085
level = logging.DEBUG if verbose else logging.INFO  # line 1086
force_sos = '--sos' in sys.argv  # type: bool  # line 1087
force_vcs = '--vcs' in sys.argv  # type: bool  # line 1088
_log = Logger(logging.getLogger(__name__))  # line 1089
debug, info, warn, error = _log.debug, _log.info, _log.warn, _log.error  # line 1089
if __name__ == '__main__':  # line 1090
    main()  # line 1090
