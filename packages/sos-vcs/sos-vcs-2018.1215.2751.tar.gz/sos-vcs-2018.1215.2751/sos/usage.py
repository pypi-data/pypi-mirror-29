...
Usage: {cmd} <command> [<argument>] [<option1>, ...]        When operating in offline mode, or command is one of "help", "offline", "version"
  ...
  Working with files:
    ...
    diff [<branch>][/<revision>]                          List changes in file tree (or `--from` specified revision) vs. last (or specified) revision
      ...
      --ignore-whitespace | --nows                          Ignore white spaces during comparison
  ...
  Common options:
    ...
    --verbose                    Enable verbose output, including show compression ratios""".format(appname=appname, cmd="sos", CMD="SOS"))  # line 90
    sys.exit(0)  # line 91
