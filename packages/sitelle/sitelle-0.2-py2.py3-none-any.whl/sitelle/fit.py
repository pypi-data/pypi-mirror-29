...
def sky_model_to_remove(mean_spectrum, axis, sky_axis, sky_model):
    ...
    :param mean_spectrum: The spectrum containing skylines. If it comes from an integrated region, pay attention to give the **mean** spectrum, i.e divided by the number of integrated pixels
...
def parse_line_params(rest_lines, fit, error=True, wavenumber=True):
    ...
    :returns: a pandas Dataframe containing all information about the fitted lines
...
def chi2_func(axis, spectrum, fit_params, delta = 8):
    ...
    fit_params : output from an `orcs` fit
		fit parameters containing information about the lines (output from an orcs fit)
    delta : int, optional
		delta around line peak that should be used to compute the chi2. Default = 8

	Returns
	-------
	chi2 : float
		a chi square value, computed only around +- sigma * fwhm for each line
    """
...
def remove_OH_line(spectrum, theta, cube, **kwargs):
    ...
    fit = fit_gas_lines([spectrum, theta], inputparams, params,
                        lines, V_range, snr_guess, silent=True)
...
def fit_gas_lines(z_dim, inputparams, params, lines, V_range, snr_guess=None, silent=False):
    ...
    :return out: a cube containing at each cell a dict with 'line_spectra': the fitted line spectra, 'fit_params': the fitetd parameters of the lines, 'chi2': chi2 list, each elemen,t corresponds to a velocity in V_range
    """
    spectrum = z_dim[0]
    theta = z_dim[1]
    chi2_list = []
