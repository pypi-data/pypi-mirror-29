...
class SQSEngine(BaseEngine):
    ...
    def __getattribute__(self, attr):
        ...
        if type(attribute) == types.MethodType and debug: # Debug mode is turned on and we're planning to execute a function
            ...
            self.logger.debug("Dummy call for {} since dev_mode is turned on".format(attribute.__name__))
            return empty # return the empty function instead of the real function
        return attribute
