...
class SQSEngine(BaseEngine):
    ...
    def send_metric(self, datasource_name, project_names, metric):
        ...
        for project_name in project_names:
            ...
            # Set the creator field to the sha of the user and project name.
            sha = hashlib.sha224()
            sha.update("{}_{}SpEcIaLsAlT".format(metric['user'], project_name).encode("utf-8"))
            metric['creator'] = sha.hexdigest()

            # Delete the user field:
            # We need to make a shallow copy of it and push that as if there
            # are multiple projects for the metric, the user key won't exist
            # the second time around.
            localMetric = dict(metric)
            del localMetric['user']

            # Finally publish the message:
            self.sns_client.publish(
                    TopicArn = topic_arn,
                    Message = json.dumps(localMetric),
