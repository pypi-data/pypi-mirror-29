...
class TextReplacements:
    ...
    default = {
      # Prefix (semi)colons with a non-breakable space
      ':': ' :', ';': ' ;',
      # Replace non-breakable spaces by breakable spaces, because we manage
      # ourselves non-breakable spaces.
      ' ': ' ',
        ...
        for row in s.split('\n'):
            if row.startswith('- '):
                ...
                if not inList:
                    self.addTag('ul')
                    inList = True
                self.add('<li>%s</li>' % self.convertText(row[2:]))
            else:
