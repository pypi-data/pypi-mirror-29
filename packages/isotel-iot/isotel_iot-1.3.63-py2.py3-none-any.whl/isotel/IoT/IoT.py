...
class Group:
    ...
    def get_connections(self, adapter=None):
        """
        Get connections
        
        :returns: adapter status in json form
        """
        uri = self.server_url + "/connections"
        if adapter:
            uri += "/" + adapter
        return self.request.get(uri, self.timeout)
    ...
    def get_adapter_status(self, adapter, level=None, connection='serial'):
        ...
        :returns: adapter status in json form
        """
        uri = self.server_url + "/" + connection + "/" + adapter
        if level:
            uri += "/" + level
        return self.request.get(uri, self.timeout)
...
class Device:
    ...
    def get_value(self, parameter, after=None):
        ...
        result = self._parse_time(self.get(param, after))
