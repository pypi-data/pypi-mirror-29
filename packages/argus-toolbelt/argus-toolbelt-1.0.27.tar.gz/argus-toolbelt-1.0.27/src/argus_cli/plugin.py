...
from argus_cli.helpers.formatting import to_caterpillar_case, to_camel_case, to_snake_case
...
def run_command(plugin_name: tuple, command: str, arguments: dict = None) -> None:
    ...
    if arguments:
        formatted_arguments = {to_snake_case(key): value for key, value in arguments.items() if value is not None}
