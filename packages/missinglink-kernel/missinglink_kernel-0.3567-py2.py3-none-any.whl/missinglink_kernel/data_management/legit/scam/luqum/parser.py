...
def parser(debug_mode=False, write_tables=False):
    logger = logging.getLogger('ply')

    debug_log = logger if debug_mode else None
    error_log = logger
    return yacc.yacc(debug=debug_mode, write_tables=write_tables, errorlog=error_log, debuglog=debug_log)
