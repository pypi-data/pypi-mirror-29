...
class DrawerConnectivity(object):
    ...
    def links_from_file(self, prefix):
        ...
        return genfromtxt(full_path, delimiter=',', dtype="int")
        ...
        for node_src in self.config.node_list:
            for node_dst in self.config.node_list:
                if node_src < node_dst:
                    ...
                    if self.show_stability:
                        if self.good_links_stable[node_src][node_dst] == 1 \
                                and self.good_links_stable[node_dst][node_src] == 1:
                            continue
                    # but only good/good link
                    elif self.good_links[node_src][node_dst] == 1 \
                            and self.good_links[node_dst][node_src] == 1:
        ...
        if self.show_stability:
            for node_src in self.config.node_list:
                for node_dst in self.config.node_list:
                    ...
                    if node_src < node_dst \
                            and self.good_links_stable[node_src][node_dst] == 1 \
                            and self.good_links_stable[node_dst][node_src] == 1:
