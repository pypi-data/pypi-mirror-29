...
class BoundaryAttackLinfinity24(Attack):
    ...
    * optional multithreading for candidate point generation
    ...
    def __init__(self, model=None, criterion=Misclassification()):
        super(BoundaryAttackLinfinity24, self).__init__(
    ...
    def __call__(
        ...
        alternative_generator: bool
            ...
            should be used. Must be False for the Linfinity attack.
        ...
        assert not alternative_generator
        self.generate_candidate = self.generate_candidate_linfinity

        return super(BoundaryAttackLinfinity24, self).__call__(
            image=image,
            label=label,
            unpack=unpack,
    ...
    def _apply_inner(
        ...
        # recent successes
        self.stats_spherical_adversarial = collections.deque(maxlen=100 * 10)
        self.stats_step_adversarial = collections.deque(maxlen=30 * 25)
    ...
    def prepare_generate_candidates(original, perturbed):
        ...
        source_norm = linf_norm(unnormalized_source_direction)
    ...
    @staticmethod
    def generate_candidate_linfinity(
            rnd_normal_queue,
            bounds,
            original,
            perturbed,
        ...
        # TODO: check if the scale of this step is reasonable
        perturbation *= spherical_step * Linf
        # adds the current difference to the original
        perturbation -= unnormalized_source_direction
        # make sure it has equal distance as before (spherical step)
        # not done in v2
        # np.clip(perturbation, -Linf, Linf, out=perturbation)
        spherical_candidate = original + perturbation
        ...
        Linf_new = (1 - source_step) * Linf
        np.clip(perturbation, -Linf_new, Linf_new, out=perturbation)
