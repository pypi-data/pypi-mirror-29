...
def apply_charts(release_id, chart_set, charts, sandbox_name=None):
    ...
    for chart in charts:
        ...
        apply_chart(chart, config, namespace, value_filenames, secrets_config)

    if release_config.get('lockVolumes', False):
        volume.lock_volumes(release_id, chart_set, charts, sandbox_name)


def apply_chart(chart, config, namespace, value_filenames, secrets_config):
    ...
    helm_timeout = config.get('helmTimeout')
    timeout_arg = ''

    if helm_timeout:
        timeout_arg = f'--timeout {helm_timeout} '
    ...
    execute(f'helm upgrade {timeout_arg}-i {release_name} --namespace {namespace} {chart_directory} {value_args}')
