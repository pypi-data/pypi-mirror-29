...
class TestLockVolumes(TestCase):
    ...
    @patch('aloft.volume.execute')
    @patch('time.sleep')
    def test_should_retry_if_cant_get_volume_name(self,
                                                  mock_sleep,
                                                  mock_execute,
                                                  mock_print_details,
                                                  mock_print_action,
        ...
        os.environ['ALOFT_CONFIG'] = f'test-config'
        mock_get_resources.return_value = [{'spec': {}, 'metadata': {'name': 'TEST_VOLUME_ID_NAME'}}]
        mock_get_resource_by_name.return_value = {
            'apiVersion': 'TEST_API_VERSION',
            'kind': 'PersistentVolume',
            'metadata': {'name': 'TEST_VOLUME_ID_NAME'},
        ...
        mock_get_resource_by_name.assert_not_called()
        mock_delete_ec2_tags.assert_not_called()
        mock_create_ec2_tags.assert_not_called()
        mock_save_content_to_s3.assert_not_called()
