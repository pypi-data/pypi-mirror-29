...
class Ec2(botohelper.botohelpermain.Main):
    ...
    def delete_vpc(self, vpc_name):
        ...
        for peering_conn in list(vpc_object.accepted_vpc_peering_connections.all()):
            if peer_conn.accepter_vpc.vpc_id != vpc_object.vpc_id and peer_conn.accepter_vpc.vpc_id not in peered_vpc_list:
                peered_vpc_list.append(peer_conn.accepter_vpc.vpc_id)
            if peer_conn.requester_vpc.vpc_id != vpc_object.vpc_id and peer_conn.requester_vpc.vpc_id not in peered_vpc_list:
                peered_vpc_list.append(peer_conn.requester_vpc.vpc_id)
            peering_conn.delete()
