...
def calculate_size(wavelength = "", phases = "", refined_par = ""):
  ...
  for key in ycos:
    ...
    try:
      ...
      sigma_csize = np.nan
    """ prepare dictionary """
    tmp_dict = {"ab-grain-size": (absize, sigma_absize), "c-grain-size": (csize, sigma_csize)}
      ...
      with open(mic, "r") as reading:
        for line in reading:
          ...
          if "standard deviation (anisotropy):" in line:
            ...
            break
    tmp_dict = {"ab-grain-size": (absize, sigma_absize), "mic": (csize, sigma_csize)}
    res_dict[phase] = tmp_dict
...
def prepare_refinement_result(phases = "", refined_par = "", size_info = "", frac_info = "", spacegroups = ""):
  ...
  def insert_text2(newline, name, value, error):
    ...
    unitdict = {"Cell" : "[$\\AA$]", "ph. vol. frac." : "[%]", "abc-grain-size" : "[nm]", "ab-grain-size" : "[nm]", "c-grain-size" : "[nm]"}
    unit = ""
