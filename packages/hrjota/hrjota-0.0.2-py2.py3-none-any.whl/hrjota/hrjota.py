...
def roll(cluster, service, image, dry, timeout, interval, rollback):
    """roll the ecs service to the new image"""
    # grab the old definition
    old_task_definition = get_definition(service)
    old_arn = old_task_definition['taskDefinition']['taskDefinitionArn']
        ...
        while is_rolling(cluster=cluster, service=service):
            if time_waited >= timeout:
                ...
                updated = ECS.update_service(
                    cluster=cluster,
                    service=service,
                    taskDefinition=old_arn,
                    forceNewDeployment=True
                )
                return False
            click.echo('[+] waiting for roll to complete')
