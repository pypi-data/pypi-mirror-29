...
class ModelRunner:
    ...
    def run_train(self, context: InputMessage):
        ...
        model_client.log_train_progress(model, 'started')
        ...
        model_client.log_train_progress(model, 'complete')
        return model._id
    ...
    def _load_state_and_cache(self, model_client: ModelClient, model: Model, k: str) -> bytes:
        data  = self.cache.load_state(model, k) or model_client.load_state(model, k)
