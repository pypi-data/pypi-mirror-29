...
class ModelStatusContextualizer:
    ...
    def contextualize(self, func):
        def wrapper(*args, **kwargs):
            val = {"started": {"args": args, "kwargs": kwargs}}
            self.modelclient.log_event(self.model, self._status_key, val)
            try:
                result = func(*args, **kwargs)
            except Exception as e:
                ex_type, ex, tb = sys.exc_info()
                val = {"erred": {"ex_type": ex_type, "exception":  ex, "traceback": tb}}
                self.modelclient.log_event(self.model, self._status_key, val)
                raise e
            self.modelclient.log_event(self.model, self._status_key, {"completed": result})
            return result
        return wrapper
