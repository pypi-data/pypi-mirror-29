...
class MyTestCase(unittest.TestCase):
    ...
    def test_incar(self):
        ...
        """
        print(os.getcwd())
        iv = pychemia.code.vasp.read_incar('test/data/vasp_01/INCAR')
        self.assertEqual(len(iv), 12)
        self.assertEqual(iv.EDIFF, 1E-7)
        wf = tempfile.NamedTemporaryFile()
        iv.write(wf.name)
        wf.close()
        iv4dir = pychemia.code.vasp.read_incar('test/data/vasp_01')
        self.assertEqual(iv, iv4dir)
        self.assertRaises(ValueError, pychemia.code.vasp.read_incar, 'test/data')
        iv3 = pychemia.code.vasp.VaspInput(variables={'EDIFF': 1E-6})
        self.assertEqual(iv3['EDIFF'], 1E-6)
        iv = pychemia.code.vasp.read_incar('test/data/vasp_02')
        iv.EDIFF *= 1.3
        td = tempfile.mkdtemp()
    ...
    def test_bad_outcar(self):
        ...
        """
        vo = pychemia.code.vasp.VaspOutput('test/data/vasp_04/OUTCAR')
    ...
    def test_encut_setup(self):
        ...
        """
        iv = pychemia.code.vasp.read_incar('test/data/vasp_06')
        iv.set_encut(ENCUT=1.2, POTCAR='test/data/vasp_06/POTCAR')
    ...
    def test_vaspjob(self):
        ...
        """
        td = tempfile.mkdtemp()
        st = pychemia.code.vasp.read_poscar('test/data/vasp_06')
        kp = pychemia.code.vasp.read_kpoints('test/data/vasp_06')
    ...
    def test_outcar(self):
        ...
        """
        vo = pychemia.code.vasp.VaspOutput('test/data/vasp_06/OUTCAR')
    ...
    def test_poscar(self):
        ...
        with self.assertRaises(ValueError) as context:
            st = pychemia.code.vasp.read_poscar('test/data/vasp_07/POSCAR')

        st = pychemia.code.vasp.read_poscar('test/data/vasp_08/POSCAR_old')
        self.assertEqual(st.natom, 2)

        st = pychemia.code.vasp.read_poscar('test/data/vasp_08/POSCAR_new')
        self.assertEqual(st.natom, 2)
        ...
        with self.assertRaises(ValueError) as context:
            pychemia.code.vasp.write_potcar(st, filepath=tmpdir + os.sep + 'POTCAR', basepsp='test/data')

        cwd = os.getcwd()
        os.chdir('test/data/vasp_07')
        st = pychemia.code.vasp.read_poscar('POSCAR_new')
        os.chdir(cwd)
        self.assertEqual(st.natom, 44)

        st = pychemia.code.vasp.read_poscar('test/data/vasp_07/POSCAR_alt')
