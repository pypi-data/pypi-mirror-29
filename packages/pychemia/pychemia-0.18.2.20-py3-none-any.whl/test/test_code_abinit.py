...
def test_abinit_abifiles():
    ...
    filename = "test/data/abinit_01/abinit.files"
    abf = AbiFiles(filename)
    assert abf.filename == "abinit.files"
    assert abf.get_input_filename() == 'test/data/abinit_01/abinit.in'
...
def test_abinit():
    ...
    af = pychemia.code.abinit.AbiFiles(basedir='test/data/abinit_03')
    iv = pychemia.code.abinit.AbinitInput('test/data/abinit_03/rnpg.in')
    af.set_input(iv)
    af.set_psps('LDA', 'FHI')
    af.create()
