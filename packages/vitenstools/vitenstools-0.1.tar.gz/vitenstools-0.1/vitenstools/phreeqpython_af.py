...
def add_solution_af(self, path):
    """ Create a PhreeqPython solution using data from a PI/AF tag """
    if isinstance(path, str):
        attributes = Element(path,persistence=604800).attributes["Waterkwaliteitsmetingen"]
    else:
        attributes = path.attributes["Waterkwaliteitsmetingen"]
    composition = {}
    charge = 0
