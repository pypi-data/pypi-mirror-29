...
There are different ways to extend jscatter by new models.
 ...
 - Use Cython or numba .... (not the scope here).
...
The main part of the cloudScattering function is evaluation of (later averaged on a sphere).

.. math:: F(q)= \sum_N b_i(q) e^{iqr}

Where we calc :math:`S(Q)=F(q) \cdot F^*(q) = |F(q)|^2` and pass :math:`F(q)`
for  calculation of :math:`beta =|< F(q) >|^2 / < |F(q)|^2 >` after spherical averaging.
...
The main work is done in np.einsum which is already a compiled C function (standard numpy function).
np.exp(iqr) is also already a C function. We may loose time in comming back to python between these steps.
