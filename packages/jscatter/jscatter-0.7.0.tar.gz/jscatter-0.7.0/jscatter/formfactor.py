...
def _coneAverage(qlist,qxz,qrpt,qfib,r,blength,formfactor,rms):
    ...
    for il in ilist:
        ...
        #res[il]=np.mean([_sqx(point,r,fa,rms) for point in Rfibpoints])   # python version
        res[il] = np.mean([fscatter.cloud.ffx(point, r, fa, rms) for point in Rfibpoints])  # fortran version
