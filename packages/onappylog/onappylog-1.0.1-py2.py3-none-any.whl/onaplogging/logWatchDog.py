    ...
    def on_modified(self, event):
        try:
            if event.src_path == self.filepath:
                ...
                print ("reload logging configure %s" % event.src_path)
                logging.config.dictConfig(newConfig)
                self.currentConfig = newConfig

        except Exception as e:
            traceback.print_exc(e)
            print ("Resuing the old configuration to avoid this exception terminate program")
            if self.currentConfig:
                logging.config.dictConfig(self.currentConfig)


def _yamlConfig(filename=None,watchDog=None):
