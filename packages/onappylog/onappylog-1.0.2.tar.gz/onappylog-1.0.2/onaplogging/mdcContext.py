...
_replace_func_name = ['info','critical','fatal','debug','error','warn','warning']
...
class MDCContext(threading.local):
    ...
    def isEmpty(self):
        return self._localDict == {} or self._localDict == None
...
def currentframe():
    """patch caller step into stack, so change to 3 get inital frame"""
    try:
        raise Exception
    except:
