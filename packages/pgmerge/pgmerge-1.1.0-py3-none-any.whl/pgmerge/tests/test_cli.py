...
class TestCLI(TestDB):
    ...
    def test_dir_invalid(self):
        ...
        # # logging.basicConfig(stream=log_stream, level=logging.WARN)
        # try:
    ...
    def check_import_output(self, result, tables, results):
        result_lines = result.output.splitlines()
        self.assertEquals(result_lines[0], "{}:".format(tables[0]))
        self.assertEquals(result_lines[1].strip().split(), results[0])
        self.assertEquals(result_lines[-1], "{} tables imported successfully".format(len(tables)))
