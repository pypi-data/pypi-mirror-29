...
from .model import (
    PeriodicTask, CrontabSchedule, PeriodicTasks, get_session, IntervalSchedule
)
...
class ModelEntry(ScheduleEntry):
    ...
    @classmethod
    def from_entry(
        cls, name, session, skip_fields=('relative', 'options'), **entry
    ):
        fields = dict(entry)
        ...
        model, _ = PeriodicTask.update_or_create(
            session, name=name, defaults=fields
        )
...
class DatabaseScheduler(Scheduler):
    ...
    def update_from_dict(self, dict_):
        ...
        for name, entry in dict_.items():
            try:
                entry_dict[name] = self.Entry.from_entry(
                    name, self.session, **entry
                )
        ...
        self.schedule.update(entry_dict)
    ...
    def schedule(self):
        ...
        if not self._initial_read:
            logger.debug('DatabaseScheduler: intial read')
            update = True
            self._initial_read = True
        elif self.schedule_changed():
            logger.debug('DatabaseScheduler: Schedule changed.')
            update = True
