...
class AWSEntity(object):
    ...
    def __init__(self, access_key, access_id, region, s3_base_url='https://s3-us-west-2.amazonaws.com', s3_bucket=None):
        self.__S3_BASE_URL = s3_base_url
        self.__ACCESS_ID = access_id
        self.__REGION = region
        self.__SECRET_KEY = access_key
        self.__S3_BUCKET = s3_bucket

    def create_bucket(self, bucket_name, permission='public-read', region=None):
        try:
            region = region or self.__REGION
            s3 = client('s3', aws_secret_access_key=self.__SECRET_KEY,
                        aws_access_key_id=self.__ACCESS_ID)
    ...
    def upload_b64(self, base64_image, filename, bucket_name, s3_base_url=None):
        """
        Upload base64 encoded image to s3
        :return:
        """
        try:
            base_url = s3_base_url or self.__S3_BASE_URL
            s3 = client('s3', aws_secret_access_key=self.__SECRET_KEY,
                        aws_access_key_id=self.__ACCESS_ID)
            ...
            return '%s/%s/%s' % (base_url, bucket_name, filename)
        except Exception, e:
            raise e

    def upload_file(self, file_path, bucket_name=None, filename=None):
        """
        Upload base64 encoded image to s3
        :return:
        """
        try:
            s3_bucket = bucket_name or self.__S3_BUCKET
            s3 = client('s3', aws_secret_access_key=self.__SECRET_KEY,
                        aws_access_key_id=self.__ACCESS_ID)
            ...
            return '%s/%s/%s' % (self.__S3_BASE_URL, s3_bucket, filename)
        except Exception, e:
            raise e
    ...
    def send_formatted_message(self, sender, subject, to, text_body='', html_body='', cc=list(), bcc=list(),
                               reply_to=list()):
        """
        ...
        if not self.__validate_email(sender):
            raise Exception('Sender address not an email address')

        recipients = to if isinstance(to, (list, tuple)) else [to]
        invalid_recipients = filter(lambda x: not self.__validate_email(x), recipients)
        ...
        invalid_cc = filter(lambda x: not self.__validate_email(x), cc_recipients)
        if len(invalid_cc) > 0:
            raise Exception('Invalid cc addresses - {}'.format(invalid_cc))

        bcc_recipients = bcc if isinstance(bcc, (list, tuple)) else [bcc]
        invalid_bcc = filter(lambda x: not self.__validate_email(x), bcc_recipients)
        ...
        invalid_reply_to = filter(lambda x: not self.__validate_email(x), reply_to_recipients)
        if len(invalid_reply_to) > 0:
            raise Exception('Invalid reply-to addresses - {}'.format(invalid_reply_to))

        try:
            ses_client = client('ses', aws_secret_access_key=self.__SECRET_KEY,
                                aws_access_key_id=self.__ACCESS_ID, region_name=self.__REGION)
    ...
    def send_raw_message(self, sender, subject, body, recipients, is_html=False, cc=list(), bcc=list(), reply_to=list(),
                         file_paths=list()):
        """
    ...
    def set_identity_notification(self, identity, notification_type, sns_topic_arn):
        """
    ...
    def create_sns_topic(self, topic_name):
        ...
        """
        try:
            sns_client = client('sns', aws_secret_access_key=self.__SECRET_KEY, regions_name=self.__REGION,
                                aws_access_key_id=self.__ACCESS_ID)
            return sns_client.create_topic(Name=topic_name)
        except Exception as e:
            raise e

    def subscribe_to_sns_topic(self, sns_topic_arn, protocol, endpoint):
        """
