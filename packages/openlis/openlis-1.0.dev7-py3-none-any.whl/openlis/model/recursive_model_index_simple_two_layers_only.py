...
class rmi_simple(object):
    ...
    def __init__(self,
        ...
        self.hidden_1_w = None
        self.hidden_1_b = None
        self.hidden_2_w = None
        self.hidden_2_b = None
        self.linear_w = None
        self.linear_b = None
        self.stage_2_w = None
        self.stage_2_b = None
    ...
    def _setup_inference_stage_1(self, keys):
        ...
        with tf.name_scope('stage_1'):
            ...
            hidden1_width = self.hidden_layer_widths[0]
            hidden2_width = self.hidden_layer_widths[1]
            ...
            with tf.name_scope('hidden1'):
                weights = tf.Variable(
                    tf.truncated_normal([key_size, hidden1_width],
                                        stddev=1.0 / math.sqrt(float(key_size)),
                                        dtype=tf.float64),
                    name='weights',
                    dtype=tf.float64)
                biases = tf.Variable(tf.zeros([hidden1_width],dtype=tf.float64),
                                     name='biases',
                                     dtype=tf.float64)
                hidden1 = tf.nn.relu(tf.matmul(keys_normed, weights) + biases)
            # Hidden 2
            with tf.name_scope('hidden2'):
                weights = tf.Variable(
                    tf.truncated_normal([hidden1_width, hidden2_width],
                                        stddev=1.0 / math.sqrt(float(hidden1_width)),
                    dtype=tf.float64),
                    name='weights')
                biases = tf.Variable(tf.zeros([hidden2_width],dtype=tf.float64),
                                     name='biases')
                hidden2 = tf.nn.relu(tf.matmul(hidden1, weights) + biases)
            # Linear
            with tf.name_scope('linear'):
                weights = tf.Variable(
                    tf.truncated_normal([hidden2_width, 1],
                                        stddev=1.0 / math.sqrt(float(hidden2_width)),
                    dtype=tf.float64),
                    name='weights')
                biases = tf.Variable(tf.zeros([1],dtype=tf.float64),
                                     name='biases')
                    
                pos_stage_1 = tf.matmul(hidden2, weights) + biases
    ...
    def get_weights_from_trained_model(self,model_save_dir="tf_checkpoints"):
        ...
        with tf.Graph().as_default():
            ...
            hidden_1_w = sess.graph.get_tensor_by_name("stage_1/hidden1/weights:0")
            self.hidden_1_w = sess.run(hidden_1_w)

            hidden_1_b = sess.graph.get_tensor_by_name("stage_1/hidden1/biases:0")
            self.hidden_1_b = sess.run(hidden_1_b)

            hidden_2_w = sess.graph.get_tensor_by_name("stage_1/hidden2/weights:0")
            self.hidden_2_w = sess.run(hidden_2_w)

            hidden_2_b = sess.graph.get_tensor_by_name("stage_1/hidden2/biases:0")
            self.hidden_2_b = sess.run(hidden_2_b)
    ...
    def _run_inference_numpy_test(self,keys):
    ...
    def time_inference(self,keys,N=1,model_save_dir="tf_checkpoints"):
