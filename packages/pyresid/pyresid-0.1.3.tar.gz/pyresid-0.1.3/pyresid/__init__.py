...
def locate_residues_old(fulltext_tokens, unique_residues, fulldoc=None, offset=False, verbose=False):
    """
...
def process(ext_id_list, outdir, verbose=False):
    ...
    with open(outpath, 'w') as outfile:
        json.dump(outdict, outfile, cls=MyEncoder, indent=4)
    pass


def locate_residues2(fulltext_tokens, residue_mentions, text_dict, ext_id, fulltext,
                     unique_proteins, provider="STFC_HARTREE", offset=True, x=50, verbose=False):
    """
...
def process_aravind(ext_id_list, outdir, verbose=False):
    """
