...
def parse_keyvals(keyvals, hinted_index_as_integer):
    ...
    for keyval in keyvals.split(','):
        ...
        values_for_this_key = value.strip('\t \n"\'').split('|')
...
class mdbquery(MDBtool):
    ...
    def __init__(self):
        ...
        cli.add_argument('queries', nargs='+',
                    ...
                    'Multiple values may be given, delimited by the | symbol.'\
                    ' Note that the queries argument may also be a path to a '\
                    'file contining query expressions.')
    ...
    def do_queries(self):
        ...
        if len(queries) == 1 and os.path.isfile(queries[0]):
            queries = open(queries[0], 'rb').read().strip().split('\n')
