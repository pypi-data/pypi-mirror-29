...
class OutBoundAmqpChannel(OutBoundChannel, BaseAmqpChannel):
    ...
    def _send(self, message, routing_key='', content_encoding='utf-8'):
        try:
            properties = pika.BasicProperties(content_type='text/plain', delivery_mode=2, content_encoding=content_encoding)
            if routing_key == '':
