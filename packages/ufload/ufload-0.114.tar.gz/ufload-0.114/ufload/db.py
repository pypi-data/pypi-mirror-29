...
def sync_link(args, hwid, db, sdb):
    ...
    rc = psql('insert into sync_server_entity (create_uid, create_date, write_date, write_uid, name, state) SELECT 1, now(), now(), 1, \'%s\', \'validated\' FROM sync_server_entity WHERE NOT EXISTS (SELECT 1 FROM sync_server_entity WHERE name = \'%s\') ' % (instance, instance) )
