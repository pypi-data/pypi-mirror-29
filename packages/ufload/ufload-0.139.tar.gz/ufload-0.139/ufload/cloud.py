...
def _decrypt(pwd):
    pwd = pwd.strip()
    x = pwd[4:]
    ufload.progress('Decoding password %s (actually %s)' % (pwd, x))
    try:
...
def get_cloud_info(args):
    ...
    ret = {
        'url': args.cloud_url + dir + args.cloud_path,
        'dir': '',
...
def list_files(**kwargs):
    ...
    #directory = 'personal/UF_OCA_msf_geneva_msf_org/'
    directory = 'personal/UF_OCA_msf_geneva_msf_org/_layouts/15/onedrive.aspx?id=%2Fpersonal%2Fuf_oca_msf_geneva_msf_org%2FDocuments%2Fbackups'
    ...
    #all = _get_all_files_and_timestamp(dav, directory)
    all = _get_all_files_and_timestamp(dav)
