import os, sys, subprocess, tempfile, hashlib, urllib, oerplib
import ufload
...
def sync_server_settings(args, sync_server, db):
    _run_out(args, mkpsql(args, 'update sync_client_sync_server_connection set database = \'%s\', login=\'admin\' user_id = 1;' % sync_server , db))
...
def connect_instance_to_sync_server(args, sync_server, db):
    ...
    #netrpc = oerplib.OERP('127.0.0.1', database=db, protocol='xmlrpc', port=12173, timeout=1000, version='6.0')
    netrpc = oerplib.OERP('127.0.0.1', database=db, protocol='xmlrpc', port=8061, timeout=1000, version='6.0')
    ufload.progress('Logging in as admin')
    netrpc.login(netrpc, 'admin', 'admin', db)
    ufload.progress('Updating sync.client.sync_server_connection')
    conn_manager = netrpc.get('sync.client.sync_server_connection')
    conn_ids = conn_manager.search([])
    ufload.progress('Setting sync password')
    conn_manager.write(conn_ids, {'password': 'admin'})
    ufload.progress('Connecting')
