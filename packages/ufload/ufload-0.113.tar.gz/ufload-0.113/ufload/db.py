...
def sync_link(args, hwid, db, sdb):
    ...
    #Check if this instance exists in the sync server
    rc = psql(args, 'select 1 from sync_server_entity where name = \'%s\';' % instance, sdb)
    if rc.count() < 1:
        psql('insert into sync_server_entity (create_uid, create_date, write_date, write_uid, name, state) VALUES (1, now(), now(), 1, \'%s\', \'validated\' ' % instance)
