...
from iotile.core.hw.reports import IndividualReadingReport, FlexibleDictionaryReport
...
class LogisticsTracker(IOTileApp):
    ...
    def MatchInfo(cls):
        ...
        return [(cls.APP_TAG, SemanticVersionRange.FromString(cls.APP_VERSION), 50)]
    ...
    @docannotate
    def upload_trip(self, save=None, clean=False, dryrun=False, acknowledge=True):
        ...
        waves = self._download_waveforms()
        ...
        if len(reports) == 0:
            iprint("Cannot upload waveforms without also uploading reports and there are no reports")
            return

        cloud = IOTileCloud()

        uuid = reports[0].origin
        ...
        wave_report = FlexibleDictionaryReport.FromReadings(uuid, [], events, sent_timestamp=reports[0].sent_timestamp, received_time=reports[0].received_time)
...
def _create_waveform_event(wave):
    ...
    data = {
        'acceleration_data': {
            ...
            'sampling_rate': 1.0 / 3200. * 3,  # We sample at 1066 Hz (3200 hz with 3x decimation)
