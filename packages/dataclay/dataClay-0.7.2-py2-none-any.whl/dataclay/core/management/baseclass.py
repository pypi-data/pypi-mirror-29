...
from dataclay.core import size_tracking
from dataclay.core.primitives import Serializable, Str
...
class ManagementMetaClass(type):
    def __init__(cls, name, bases, kwds):
        ...
        if name != "ManagementObject":
            yaml_tag = u"tag:yaml.org,2002:util.%s.%s" % (
                cls.__module__.split(".", 2)[-1],
                name)
    ...
    def __new__(mcs, name, bases, dct):
        ...
        if name == "ManagementObject":
            dct["__slots__"] = tuple()
            return super(ManagementMetaClass, mcs).__new__(mcs, name, bases, dct)
...
class ManagementObject(Serializable):
    ...
    def __setstate__(self, state):
        ...
        for k, v in state.iteritems():
            assert (k in self._fields) or (k in self._internal_fields), \
                "__setstate__ on class %s called with an unrecognized field: %s" % (
                    self.__class__, k)
        ...
        assert len(missed_fields) == 0, \
            "__setstate__ on class %s called without the following required fields: %s" \
