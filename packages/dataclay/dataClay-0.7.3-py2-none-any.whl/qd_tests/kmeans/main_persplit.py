...
def kmeans_frag(numV, k, dim, epsilon, maxIterations, numFrag):
    ...
    while not has_converged(mu, oldmu, epsilon, n, maxIterations):
        ...
        # FYI: This is a list of lists of dictionaries
        # (a dictionary per each fragment, per each split)
        meta_cluster = list()
        for s in X.split(split_class="KMeans.SplitBoard"):
            # FYI: This is a list of dictionaries
            # (a dictionary per each per fragment)
            cluster_sublist = s.cluster_points(mu)
            meta_cluster.append(cluster_sublist)
            partialResult.append(s.partial_sum(cluster_sublist))
