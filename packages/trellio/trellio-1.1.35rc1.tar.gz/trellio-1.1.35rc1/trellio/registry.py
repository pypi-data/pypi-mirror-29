...
class Repository:
    ...
    def add_pending_service(self, name, version, node_id):
        if node_id not in self._pending_services[self._get_full_service_name(name, version)]:
    ...
    def remove_pending_instance(self, name, version, node_id):
        pending_instances = self.get_pending_instances(name, version)
        if node_id in pending_instances:
            pending_instances.remove(node_id)
        if not len(self.get_pending_instances(name, version)):
            self._pending_services.pop(self._get_full_service_name(name, version), None)
...
class Registry:
    ...
    def deregister_service(self, node_id):
        ...
        if service:
            ...
            if service is not None:
                ...
                self._repository.remove_pending_instance(service.name, service.version, node_id)
    ...
    def _handle_connection_lost(self, protocol):
        for node_id, cp in self._service_protocols.items():
            if protocol == cp:
