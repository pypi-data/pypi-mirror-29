...
def test_zip(target_folder, zip_file_name, checksum_entries):
    if is_valid_folder(target_folder, os.W_OK):
        try:
            ...
            checksum_entries_by_filename = map(lambda x: x.split('\t')[0].strip(), checksum_entries)

            print("%s" % (checksum_entries_by_filename))
            for ze in zip_file.namelist():
                simple_name = os.path.basename(ze).strip()
                print("%s | %s" % (ze, simple_name))
...
def main():
    ...
    | \| | __|_   _| __| |  |_ _\ \/ /    
    | .` | _|  | | | _|| |__ | | >  <     
    |_|\_|___| |_| |_| |____|___/_/\_\    npack.py version 0.6.6
