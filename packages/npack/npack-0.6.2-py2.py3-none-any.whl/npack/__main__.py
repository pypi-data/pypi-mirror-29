...
def main():
    ...
    | \| | __|_   _| __| |  |_ _\ \/ /    
    | .` | _|  | | | _|| |__ | | >  <     
    |_|\_|___| |_| |_| |____|___/_/\_\    npack.py version 0.6.2
        ...
        if response.status_code == 200:
            ...
            for release in data["releases"].keys():
                ...
                if version_number < release_number:
                    wprint("%s | You are using an older version, please update npack by running the following in your terminal:  "
                           "\n pip install npack --ignore-installed --no-cache-dir "
                           "\n ** pip command might be pip3" % datetime.utcnow())
        else:
            wprint("%s | Unable to check for latest python version, please enable Internet connectivity for npack version check" % datetime.utcnow())
    except Exception:
        wprint("%s | Unable to check for latest python version, please enable Internet connectivity for npack version check" % datetime.utcnow())
