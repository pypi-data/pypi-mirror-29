...
class rCCXTGrabber(CCXTGrabber):
    ...
    async def save_status(self, trade_pair=None, ts=0, dump=False):
        if trade_pair:
            ...
            self.tradepair_last_update[trade_pair] = datetime.now().strftime(
                "%Y-%m-%d %H:%M:%S")
        if dump:
            conn = create_engine("sqlite:///" + self.status_file)
            ...
            df = pd.DataFrame.from_records(({"tradepair": i[0],
                                            "latest_ts": i[1],
                                             "latest_bartime": datetime.fromtimestamp(i[1] // 1000).strftime(
                                                 "%Y-%m-%d %H:%M:%S"),
    ...
    def fill(self, df):
        if (not df.empty)\
            ...
            result["bar_time"] = result["timestamp_ms"].apply(
                lambda row: datetime.fromtimestamp(row // 1000).strftime("%Y-%m-%d %H:%M:%S"))
