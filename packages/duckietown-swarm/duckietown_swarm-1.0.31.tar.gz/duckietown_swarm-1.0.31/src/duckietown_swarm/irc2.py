...
from duckietown_swarm.spawn_ipfs import run_ipfs, initialize_ipfs
...
class MessagingInterface():
    ...
    def broadcast_to_all_brains(self, msg, sign):
        ...
        for c in self._get_broadcasting_channels():
            new_to = c + ALL_BRAINS
#            print('broadcasting %r to all brains at %r' % (msg, new_to))
...
#        return True, address[len(x):]
#    for i in range(len(x) + 1):  # also entire string
#        portion = x[:i]
#        print portion
#        if fnmatch.fnmatch(portion, address):
#            rest = x[i:]
#            print('matched %r with %r | rest = %r' % (address, x, rest))
#            return True, rest
#    print('address = %r does not match x = %r' % (address, x))
#    return False, None
...
#    print('sleeping %d seconds' % 10)
#    time.sleep(10)

    print('%s Identity: %s' % (ipfs_path, identity))

#    ipfs_id = q.get()
#    print('ipfs_id: %s' % ipfs_id)
