...
from duckietown_swarm.dcache_wire import MsgCommand
import fnmatch
import socket
import time
...
#            print('Ignoring new in bucket %s = %s' % (str(bucket), data))
    ...
    interval_advertises_ipfs_peers = DoAtInterval(60)
...
def periodic_advertise_ipfs_peers(dc, mi, identity, ipfsi):  #@UnusedVariable
    ...
    for peer in peers:
        t = time.time()
        msg = create_propose_message(('pri', identity, IPFS_PEERS),
                                     peer, validity=[t, t + 60 * 60])
        mi.send_to_brain('', msg, sign=True)
...
def periodic_advertise_self(dc, mi, identity, ipfsi):  #@UnusedVariable
    ...
    for k, v in properties.items():
        ...
        for x in v:
            ...
            msg = create_propose_message(bucket, x, validity=[t, t + DELTA_INFO])
            mi.send_to_brain('', msg, sign=True)
