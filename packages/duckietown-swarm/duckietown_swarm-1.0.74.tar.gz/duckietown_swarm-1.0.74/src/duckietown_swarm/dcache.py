...
class BucketDataItem(object):
    ...
    def propose(self, validity, signatures, from_channel):
        if self.is_redundant(validity, signatures):
            return
    ...
    def is_redundant(self, validity, signatures):
        for s in signatures:
            if not self.is_redundant1(validity, s):
                return False
        return True

    def is_redundant1(self, validity, signatures):
        for v0, signers in self.validity2signatures_propose.items():
            if not signatures or any(x in signatures for x in signers):
                if strictly_dominates(v0, validity):
                    return True
        return False
    ...
    def cleanup(self):
        ...
        # group by signer
        signer2propose_validities = defaultdict(set)
        for validity, signers in self.validity2signatures_propose.items():
            if not signers:
                signers = ['anonymous']
            for signer in signers:
                signer2propose_validities[signer].add(validity)

        dominated = set()
        for signer, validities in signer2propose_validities.items():
            for v1, v2 in itertools.product(validities, validities):
                if strictly_dominates(v1, v2):
                    print ('%s: %s dominates %s' % (signer, v1, v2))
                    dominated.add((signer, v2))
        if 'anonymous' in signer2propose_validities:
            an_validities = signer2propose_validities['anonymous']
            for v2 in an_validities:
                for signer, vals in signer2propose_validities.items():
                    if signer != 'anonymous':
                        for v1 in vals:
                            if strictly_dominates(v1, v2):
                                dominated.add(('anonymous', v2))

        for signer, validity in dominated:
            if signer != 'anonymous':
                print('redundant %s %s' % (signer, validity))
                self.validity2signatures_propose[validity].remove(signer)

        for signer, validity in dominated:
            if signer == 'anonymous' and not self.validity2signatures_propose[validity]:
                print('redundant %s %s' % (signer, validity))
                del self.validity2signatures_propose[validity]
...
def strictly_dominates(validity1, validity2):
...
#                roles = self._get_roles_endorsing(wm, at)
                ...
                if buckets[0] == 'pri' and not (buckets[1] in signatures):
                    msg = 'Not setting %s because signatures=%s' % (buckets, signatures)
                    print(msg)
                else:
                ...
                if buckets[0] == 'pri' and not buckets[1] in signatures:
                    msg = 'Not setting %s because signatures=%s' % (buckets, buckets[1])
                    print(msg)
                else:
