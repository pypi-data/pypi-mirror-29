...
from duckietown_swarm.dcache_wire import MsgDismiss, MsgSignature, \
    create_propose_message
from duckietown_swarm.utils import pretty_print_dictionary, friendly_time_since, \
    duration_compact
import os
import shelve
import time
    ...
    def summary(self):
        ...
        def interval_friendly(interval):
            ...
            if t0:
                ...
                d0 = "forever"
            if t1:
                d1 = duration_compact(t1 - time.time())
            else:
                d1 = 'forever'
...
class Bucket(object):
    ...
    def cleanup(self):
        for dataitem in self.data2dataitem.values():
            dataitem.cleanup()
    ...
    def get_bucket(self, add):
        if isinstance(add, str):
            add = (add,)
        try:
            return self._get_bucket(add)
        except KeyError as e:
            msg = 'Could not resolve bucket "%s": invalid key %s.' % ("/".join(add), e)
            raise NoSuchBucket(msg)

    def _get_bucket(self, add):
        if not add:
            return self
        name = add[0]
        rest = add[1:]
        return self.children[name].get_bucket(rest)
...
class DistributedRepo(object):
    ...
    def _roles_from_signatures(self, peerid, at=None):
        ...
        result = set()
        for role in roles:
            bucket = self.root.get_bucket(role)
    ...
    def add_hook_proposed(self, buckets, f):
        if isinstance(buckets, str):
            buckets = (buckets,)
        bucket = self.root.get_bucket(buckets)
    ...
    def summary(self):
        ...
        s = []
        for channel, last in self.all_channels.items():
            s.append('activity in %s: %s' % (channel, friendly_time_since(last)))
        od['activity'] = "\n".join(s)
    ...
    def init_bucket(self, buckets, allowed):
        ...
        prev = buckets[:-1]
        name = buckets[-1]
        parent = self.root.get_bucket(prev)
    ...
    def process(self, wm, from_channel, at=None):
        ...
        if at is None:
            at = time.time()
        self.all_channels[from_channel] = at
        try:
        ...
        try:
            if isinstance(m, MsgPropose):
                ...
                bucket.propose(data, validity, roles, from_channel)
                ...
                bucket = self.root.get_bucket(buckets)
                roles = self._get_roles_endorsing(wm, at)
                bucket.dismiss(data, validity, roles, from_channel)
    ...
    def query(self, bucket, at=None):
        ...
        if isinstance(bucket, str):
            bucket = (bucket,)
        bucket = self.root.get_bucket(bucket)
        ...
        for data in datas:
            ...
            for channel in channels:
                ...
                if need_update:
                    ...
                    options.append((last, channel, msg))
                    dataitem.last_broadcast[channel] = time.time()
                    self.all_channels[channel] = time.time()

        s = [(_[1], _[2]) for _ in sorted(options, key=lambda _: _[0])]
        s = s[:max_n]
        return s
...
class DistributedRepoSwarm(DistributedRepo):
    ...
    def __init__(self):
        ...
        self.init_bucket('verified', allowed=['trusted'])
