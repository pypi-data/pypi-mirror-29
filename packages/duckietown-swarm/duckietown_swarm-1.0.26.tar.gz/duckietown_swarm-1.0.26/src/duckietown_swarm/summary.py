...
def publisher_summary(read_from, ipns_queue, write_to):
    KEY_NAME = 'summary'
    known = set()
    ...
    while True:
        ...
        hashes = get_at_least_one(read_from, timeout)
        changes = 0
        for x in hashes:
            what, ipfs_hash = x
            if what == 'add':
...
def publish_ipns(ipns_queue):
    ...
    while True:
        #print('summary')
        timeout = 5
        what = get_at_least_one(ipns_queue, timeout)
        if what:
            to_add = {}
            for key, v in what:
                to_add[key] = v
