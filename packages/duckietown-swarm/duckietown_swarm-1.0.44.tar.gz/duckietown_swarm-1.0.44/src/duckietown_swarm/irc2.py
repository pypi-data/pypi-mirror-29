...
from duckietown_swarm.pubsub_friends import connecting_to_peer
...
from .spawn_ipfs import run_ipfs, initialize_ipfs
...
class MessagingInterface():
    ...
    def __init__(self, key, ipfs_path):
        ...
        self.MADDR_IRC = '/dns4/frankfurt.co-design.science/tcp/6667/irc'
        self.IRC_CHANNEL = '#duckiebots'
        self.MADDR_PUBSUBWRITER = APP + '/pubsubwriter'
        self.MADDR_READ_SUMMARIES = APP + '/read_summaries'

        self.pubsub_topic = 'duckiebots'

        self.MADDR_PUBSUBWRITER = '/ipfs-pubsub/' + self.pubsub_topic

        name2queue = {}
        name2queue[self.MADDR_PINNER] = Queue()
        name2queue[self.MADDR_BRAIN] = Queue()
        name2queue[self.MADDR_VERIFIER] = Queue()
        name2queue[self.MADDR_IPNS] = Queue()
        name2queue[self.MADDR_IRC] = Queue()
        name2queue[self.MADDR_PUBSUBWRITER] = Queue()
        name2queue[self.MADDR_READ_SUMMARIES] = Queue()
        name2queue[self.MADDR_CONNECTOR] = Queue()
    ...
    @contract(mh='multihash')
    def send_to_connector(self, mh):
        e = Envelope('', self.MADDR_CONNECTOR, mh)
        self.dispatch(e)
...
def duckietown_swarm_main():
    ...
    ipfs_path = os.path.join(cache_dir, '.ipfs')
...
#        pm.start()
    ...
    from duckietown_swarm.offline import read_nodes_summaries

    pm = ProcessManager(read_nodes_summaries,
                        (mi.copy_as(mi.MADDR_READ_SUMMARIES),), 'read_nodes_summaries',
                        restart=True)
    pm.start()
