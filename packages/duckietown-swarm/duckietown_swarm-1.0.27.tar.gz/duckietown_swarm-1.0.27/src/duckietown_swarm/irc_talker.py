...
class SwarmBot(irc.bot.SingleServerIRCBot):
    ...
    def _handle(self, _c, e):
        s = str(e.arguments[0])
        self.mi.send_to_brain(self.stream_name, s)
...
def start_irc(stream_name, servers, mi):
    random.seed()
    channel = '#duckiebots'
    ...
    c = SwarmBot(servers, stream_name, mi, channel, nickname)
    while True:
        ...
        while True:
            ...
            xs = get_at_least_one(mi.name2queue[stream_name], timeout=1.0)

            for to_channel, x in xs:
                if to_channel == stream_name:
                    try:
                        c.send_message(x)
                        mi.send_to_brain(to_channel, x)
