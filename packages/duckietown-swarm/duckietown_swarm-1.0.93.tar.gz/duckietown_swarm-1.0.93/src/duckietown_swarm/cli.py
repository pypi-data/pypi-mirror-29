...
#        print 'mh', mh.__repr__(), type(mh)
        nodes.append(dict(id=id_from_mh(mh), label=data['label']))
        for n in G.neighbors(mh):
            direct = G.get_edge_data(mh, n)['direct']
            edges.append({'from': id_from_mh(mh), 'to': id_from_mh(n), 'dashes': not direct})
#        print G[mh]

    t = template

    t = t.replace('EDGES', json.dumps(edges))
