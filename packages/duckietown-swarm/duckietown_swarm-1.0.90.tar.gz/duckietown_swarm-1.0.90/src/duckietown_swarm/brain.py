...
def brain(mi, cache_dir):
    ...
    proposed = set(dc.query(DSC.DLC_BUCKET_FILES))
    verified = set(dc.query(DSC.DLC_BUCKET_VERIFIED))
    for p in proposed:
...
#            print('Ignoring new in bucket %s = %s' % (str(bucket), data))
    ...
    dc.add_hook_proposed(DSC.DLC_BUCKET_VERIFIED, on_new_verifed)
...
def periodic_advertise_self(dc, mi, identity, ipfsi):  #@UnusedVariable
    t = time.time()
    msg = create_propose_message(DSC.DLC_BUCKET_PEER, identity, validity=[t, t + DSwarmConstants.DELTA_PEER])
    mi.send_to_brain('', msg, sign=True)
...
def periodic_make_summary(dc, mi, identity, ipfsi):
    ...
    delta = as_seconds(DSwarmConstants.SDELTA_MAKE_SUMMARY)
    print('make summary delta: %r' % delta)
    validity = [t, t + delta * 2]
