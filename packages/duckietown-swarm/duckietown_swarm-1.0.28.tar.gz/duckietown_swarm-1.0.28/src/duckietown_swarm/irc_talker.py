...
def start_irc(mi, server, channel):
    ...
    for irc_name in irc_names:
        ...
        msg_sign = key0.generate_sign_message(msg)
        mi.send_to_brain('', msg_sign)
    ...
    while True:
        ...
        while True:
            ...
            xs = get_at_least_one(mi.name2queue[mi.my_maddr], timeout=1.0)

            for envelope in xs:
                try:
#                    print('Sending envelope %s' % envelope)
                    if not envelope.maddr_to or envelope.maddr_to[0] != '/':
                        msg = 'Invalid addr_to: %r' % envelope.maddr_to
                        print(msg)
                        continue
                    where = envelope.maddr_to[1:]
                    c.send_message(where, envelope.contents)
