...
def udp_broadcaster(mi):
    ...
    for name, x in interfaces.items():
...
#            my_name = name
    ...
    if my_address is None:
        msg = 'Could not find address for broadcast address %s. (interface is down?)' % broadcast_address
        raise_desc(Exception, msg, interfaces=interfaces)
