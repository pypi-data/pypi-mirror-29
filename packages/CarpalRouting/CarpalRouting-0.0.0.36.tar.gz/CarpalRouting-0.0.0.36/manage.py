    ...
    if not file:
        ...
        try:
            with open(file) as setting_file:
                ...
                for index, row in df.iterrows():
                    ...
                    if pickup_address in locations:                
                        new_item = {'address': row['delivery_address'],
                                    'coordinates': map_address_to_coordinates(row['delivery_address'], settings.get('googleAPIKey')),
                        ...
                        if pickup_window in locations[pickup_address]:             
                            ...
                            locations[pickup_address][pickup_window] = [{
                                ...
                                'coordinates': map_address_to_coordinates(pickup_address, settings.get('googleAPIKey')),
                ...
                data = LocationGrouping().group_locations(locations, schedules, settings.get('mergeIdentical'))
                        ...
                        for pickup_window, obj in pickup_windows.items():
                            ...
                            if locs:
                                indices = Routing(locations=locs,
                                                  num_vehicles=len(locs)).generate_routes(vehicle_capacity=settings['publicDriver']['specification']['capacity'],
...
def map_address_to_coordinates(address, key):
    response = requests.get('https://maps.googleapis.com/maps/api/geocode/json?address={}&key={}'.format(address, key))
