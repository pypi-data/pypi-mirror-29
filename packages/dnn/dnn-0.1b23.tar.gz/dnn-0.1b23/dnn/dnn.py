...
class DNN:
    ...
    def lstm (self, n_input, hidden_size, lstm_layers = 1, activation = None, dynamic = True, dropout = True):
        try:
            rnn = tf.nn.rnn_cell
    ...
    def fully_connected (self, outputs, n_output):
        return tf.reshape (outputs, [-1, n_output])
    
    def sequencial (self, outputs, hidden_size, seq_len, n_output):
        # outputs is rnn outputs
        fc = self.fully_connected (outputs, hidden_size)
        outputs = tf.layers.dense (fc, n_output, activation = None)
        return tf.reshape (outputs, [self.n_sample, seq_len, n_output])
    
    def batch_norm (self, n_input, momentum = 0.99):  
       return tf.layers.batch_normalization (n_input, momentum = momentum, training = self.phase_train) 
    ...
    def dense_with_dropout (self, n_input, n_output, activation = None, normalize = True):
        layer = tf.layers.dense (inputs = n_input, units = n_output)
        if normalize:
    ...
    def conv1d (self, n_input, filters = 6, activation = None,  padding = "same", dropout = True):
        # decreased to half
    ...
    def conv2d (self, n_input, filters, kernel_size = (4, 4), pool_size = (2, 2), activation = None, padding = "same", dropout = True):
    ...
    def make_logit (self):
        ...
        layer2 = self.dense (self.x, n_hidden_2)
        layer3 = self.dense (layer2, n_hidden_3)
        layer4 = self.dense (layer3, n_hidden_4)
