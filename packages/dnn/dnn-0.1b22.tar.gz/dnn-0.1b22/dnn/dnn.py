...
class DNN:
    ...
    def dense_with_dropout (self, n_input, n_output, activation = None, batchnorm = True):
        layer = tf.layers.dense (inputs = n_input, units = n_output)
        if batchnorm:
            layer = tf.layers.batch_normalization (layer, momentum = 0.99, training = self.phase_train)
