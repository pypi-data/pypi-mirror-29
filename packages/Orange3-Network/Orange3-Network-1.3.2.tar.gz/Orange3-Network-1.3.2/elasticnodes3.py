...
from PyQt4.QtCore import QLineF, QPointF, QRectF, QRect, QSize, Qt
...
class GraphView(QtGui.QGraphicsView):
    ...
    def mousePressEvent(self, event):
        ...
        origin = self._click_origin = event.pos()
        self.rubberband.setGeometry(QRect(origin, QSize()))
        self.rubberband.show()

        multiselect = bool(event.modifiers() & (Qt.ControlModifier | Qt.ShiftModifier))
        if multiselect:
    ...
    def mouseMoveEvent(self, event):
        if not event.buttons():
            # No buttons are pressed any longer
            return
        self.rubberband.setGeometry(QRect(self._click_origin, event.pos()).normalized())
        path = QtGui.QPainterPath()
        path.addPolygon(self.mapToScene(self.rubberband.geometry()))
    ...
    def resizeEvent(self, event):
        ...
        self.fitInView(self.scale().sceneRect(), Qt.IgnoreAspectRatio)
...
if __name__ == '__main__':
    ...
    QtCore.QTimer.singleShot(0, lambda:
        fruchterman_reingold_layout(G,
                                    iterations=50,
                                    callback=widget.update_positions))
