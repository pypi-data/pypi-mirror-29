...
class SNSG(object):
    ...
    def filter(self, filter_cond=0, filter_free=5, flag=True):
        """
        过滤一些不重要的数据
        [出现次数,出现频率,凝固程度,自由程度]
        :param filter_cond:  过滤凝聚度
        :param filter_free:  过滤自由度
        :param flag:  是否过滤凝聚度，默认是TRUE，过滤
        :return:
        """
        key_words = dict()
        for key in self.sns_words.keys():
        ...
        key = key_words.keys()
        for max_key in list(key):
            for min_key in list(key):
                if min_key.find(max_key) > 0 and key_words[max_key][3] < filter_free:
                    if flag:
                        key_words.pop(max_key)
                    elif key_words[max_key][2] < filter_cond:
                        key_words.pop(max_key)

        print("filter over!")
        return sorted(key_words.items(), key=lambda d: d[1], reverse=True)
