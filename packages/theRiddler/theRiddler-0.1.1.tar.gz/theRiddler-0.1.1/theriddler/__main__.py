...
class RiddlerGUI(tk.Tk):
	...
	def __init__(self, *args, **kwargs):
		...
		self.iconbitmap(path + "brushed_r.ico")# changes the window bar icon.
		...
		self.status_var.trace("w", self.timer)# timer() is called when status_var 
		# gets written (-->argument "w").
		...
		for F in (EntryGUI, MainGUI):# additional frames go here.
			frame = F(master, self)
			self.frames[F] = frame
			frame.grid(row=0, column=0, sticky="nsew")# the frames are put on each 
			# other and the active one is raised over.
	...
	def cancel(self):
		...
		if self._job is not None:# if within set time the timer is called again, 
	...
	def timer(self, *args):
		...
		if status_phrase == "":# if status was cleared and set to "empty", 
		# nothing happens.
			return
		if status_phrase == self.entrygui_status:# entrygui_status set for the entry 
		# page shall not be cleared until it gets overwritten.
			return
		self.cancel()# cancel() stops the execution of a "pending" after() function, 
		# if there is one.
		self._job = self.after(7000, self.clear_status_var)# status displayed 7000 
		# milliseconds before clear_status_var() gets called. the function after()(or 
		# rather "self._job") returns "None" after set time.
	...
	def enter(self, event):
		...
		if not entry_string.isspace():# "isspace()" returns true if the string only 
		# contains whitespace characters or is empty, otherwise returns false.
...
class EntryGUI(tk.Frame):
	...
	def __init__(self, parent, controller):
		...
		entry_label.pack(expand=True)#in this special case "expand" centers the 
		# (only and small enough) widget on the frame, so this does the job.
...
class MainGUI(tk.Frame):
	...
	def __init__(self, parent, controller):
		...
		self.hints = tk.Label(self, textvariable=self.hint_var, 
			bg="dim grey", justify="left")
	...
	def load_photo(self):
		...
		photo.thumbnail((450, 450), Image.BICUBIC)# "thumbnail()" keeps the aspect 
		# ratio. if the images are smaller, they may appear smaller, but they never 
		# will be bigger than (450,450). BICUBIC is the render method.
	...
	def set_image(self):
		...
		self.image_label.image = img # keeps a reference to the tkinter object by 
		# attaching it to a widget attribute. Otherwise it will be 
		# "destroyed"(blanked) by pythons garbage collector.
...
class Guard(object):
	...
	wrong_key_on_entry_screen_answer = """No matching keyword. Clear the entry and 
		press "Enter" to start with the first riddle."""
	...
	def check_key(self):	
		...
		if isinstance(self.active_riddle, Raccourci):# removes the characters '",\,/' 
		# and space from the entry string if the active_riddle is "Raccourci". 
		# This is mangy.
			to_remove = '"/\\ '
			for chars in to_remove:
				self.key = self.key.replace(chars, "")
			self.key = self.key.replace("Set-Location", "cd")# The "Set-Location" 
			# command is an alias for "cd" in the shell and would therefore be part of 
			# a right answer too.	
