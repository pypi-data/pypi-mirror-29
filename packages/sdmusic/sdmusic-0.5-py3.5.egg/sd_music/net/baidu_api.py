...
class BaiduCloud(BaseApi):
    ...
    def get_music_url(self,music_name,page_num,index):
        ...
        if len(infos) >= index:
            ...
            music_format = music_info['format']
            if music_format == 'flac':
                print('小伙子恭喜你，是FLAC格式音乐')
                download_url['format'] = 'flac'
            else:
                download_url['format'] = '.mp3'
            download_url['url'] = music_info['songLink']
            return download_url
        else:
            show_out_of_bound()
    ...
    def get_music_url_and_info(self,music_name,page_num,index):
        ...
        if len(infos) >= index:
            ...
            download_url_r=download_url_r.json()
            music_info = download_url_r['data']['songList'][0]
            self.music.author=music_info['artistName']
            self.music.album_name=music_info['albumName']
            self.music.album_pic_url=music_info['songPicBig']
            self.music.name = music_info['songName']
            self.music.download_url = music_info['songLink']
            self.music.music_format = music_info['format']
        else:
            show_out_of_bound()