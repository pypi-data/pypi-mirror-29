...
class KugouCloud(BaseApi):
    ...
    def get_music_hash(self,music_name,page_num,index):
        infos = self.get_music_info(music_name, page_num)
        if len(infos) >= index:
            info = infos[index]
            hash_code = info['hash']
    ...
    def get_music_url(self,music_name,page_num,index):
        hash_code=self.get_music_hash(music_name,page_num,index)
    ...
    def get_music_lyric(self,music_name,page_num,index):
        hash_code=self.get_music_hash(music_name,page_num,index)
