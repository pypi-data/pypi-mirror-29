...
from flask import Flask as _Flask, request as _request, redirect as _redirect, url_for as _url_for
...
# Test:
#   curl -F 'file=@mnist-cpu.tar.gz' http://localhost:6970/predict-server-upload/mnist/cpu

#TODO:  Fix return statements
#TODO:  Add --force
@app.route('/predict-server-upload/<string:model_name>/<string:model_tag>', methods=['POST'])
def _upload_file(model_name, model_tag):
    print(_request.files['file'])
    if _request.method == 'POST':
        # check if the post request has the file part
        if 'file' not in _request.files:
            #_flash('No file part')
            #return _redirect(_request.url)
            return 'NOT_OK_NO_FILE'
...
#            return _redirect(request.url)
            return 'NOT_OK_NO_FILENAME'
        if f and _allowed_file(f.filename):
            filename = _secure_filename(f.filename)
            # TODO:  Support subdirectories after /uploads
            if not _os.path.exists(_UPLOAD_FOLDER):
                _os.makedirs(_UPLOAD_FOLDER)
#            full_path = _os.path.join(_UPLOAD_FOLDER, filename)
            # TODO:  use .join instead of '/'
            full_path = _os.path.join(_UPLOAD_FOLDER, '%s/%s/%s' % (model_name, model_tag, filename))
            f.save(full_path)
            # TODO:  Unzip and return status of everything including contents and location on disk
            #_flash('Successfully uploaded file "%s"!' % filename)
#            return _redirect(_url_for('_upload_file',
#                             filename=filename))
            return 'OK'
    return 'NOT_OK_NEED_POST'
