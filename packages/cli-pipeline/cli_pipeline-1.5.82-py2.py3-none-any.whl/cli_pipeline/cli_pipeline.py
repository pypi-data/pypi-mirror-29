...
_kube_router_routerules_template_registry = {'predict-router': (['predict-router-routerules.yaml.template'], [])}
...
def stream_kube_start(model_name,
    ...
    endpoint_url = 'http://%s/topics' % stream_url
...
#    print("")
#    print("Describing stream at '%s'." % endpoint_url)
#    print("")
...
def stream_kube_consume(model_name,
    ...
    endpoint_url = 'http://%s/consumers/%s' % (stream_url, stream_consumer_name)
    ...
    endpoint_url = 'http://%s/consumers/%s/instances/%s/subscription' % (stream_url, stream_consumer_name, stream_consumer_name)
    response = _requests.post(url=endpoint_url,
                              headers=content_type_headers,
                              data=body.encode('utf-8'),
                              timeout=timeout_seconds)
    ...
    endpoint_url = 'http://%s/consumers/%s/instances/%s/records' % (stream_url, stream_consumer_name, stream_consumer_name)
    ...
    endpoint_url = 'http://%s/consumers/%s/instances/%s' % (stream_url, stream_consumer_name, stream_consumer_name)
...
# TODO:  refactor stream_kube_produce to use this
def stream_http_produce(endpoint_url,
    ...
    print("")
    print("Producing messages for stream '%s'." % endpoint_url)
    print("")
...
# TODO:  Route this traffic through Istio!
def stream_kube_produce(model_name,
    ...
    # TODO:  This should route through Istio!
    endpoint_url = 'http://%s/topics/%s' % (stream_url, stream_topic)

    print("")
    print("Producing stream for topic '%s' at '%s'." % (stream_topic, endpoint_url))
    print("")
    ...
    with _warnings.catch_warnings():
        ...
        if not endpoint:
            try:
                ...
                istio_ingress_nodeport = '80'

            endpoint = 'localhost:%s' % istio_ingress_nodeport
...
def _get_cluster_service(service_name,
    ...
    with _warnings.catch_warnings():
        ...
        if not endpoint:
            ...
            endpoint = '<ingress-hostname-or-ip>:%s' % istio_ingress_nodeport
