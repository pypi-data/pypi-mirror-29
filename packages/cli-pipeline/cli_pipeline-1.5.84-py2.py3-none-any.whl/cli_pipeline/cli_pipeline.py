...
# TODO:  Add saved_model_cli type stuff here: https://medium.freecodecamp.org/how-to-deploy-an-object-detection-model-with-tensorflow-serving-d6436e65d1d9
...
_kube_router_deploy_template_registry = {'predict-router-split': (['predict-deploy.yaml.template'], []),
                                         'predict-router-gpu-split': (['predict-gpu-deploy.yaml.template'], [])}

_kube_router_ingress_template_registry = {'predict-router-split': (['predict-ingress.yaml.template'], [])}
_kube_router_svc_template_registry = {'predict-router-split': (['predict-svc.yaml.template'], [])}
_kube_router_routerules_template_registry = {'predict-router': (['predict-routerules.yaml.template'], [])}
_kube_router_autoscale_template_registry = {'predict-router-split': (['predict-autoscale.yaml.template'], [])}

_kube_stream_deploy_template_registry = {'stream': (['stream-deploy.yaml.template'], [])}
_kube_stream_svc_template_registry = {'stream': (['stream-svc.yaml.template'], [])}
_kube_stream_ingress_template_registry = {'stream': (['stream-ingress.yaml.template'], [])}
_kube_stream_routerules_template_registry = {'stream': (['stream-routerules.yaml.template'], [])}
...
def version():
    ...
    print('')
    print('Capabilities enabled: %s' % ['train-server', 'train-kube', 'train-sage', 'predict-server', 'predict-kube', 'predict-sage', 'stream-kube'])
    print('Capabilities available: %s' % ['jupyter', 'spark', 'airflow'])
    print('')
...
#        os.makedirs(model_path)
    ...
    # TODO:  Use inja template rendering here - instead of _shutil.copytree()
    #   Some of the contents inside these /functions are already templatized, but we're not emphasizing them at this time
    #   (ie. pipeline_train.sh, pipeline_predict.sh)
...
def predict_server_start(model_name,
    ...
    if not stream_input_topic:
        stream_stream_topic = '%s-%s-input' % (model_name, model_tag)
...
def _untar(model_name,
    ...
    print("")
    print("Decompressing '%s' into model_path '%s'." % (full_untar_path, model_path))
...
#   cd ./models
#
#   pipeline _tar --model-name=mnist --model-tag=upload --model-path=./tensorflow/mnist-cpu/model
#
#   curl -F 'file=@mnist-upload.tar.gz' http://localhost:6970/predict-server-upload/mnist/upload/tensorflow/tfserving/cpu/Li9tb2RlbHMvbW9kZWxzL3RlbnNvcmZsb3cvbW5pc3QtdXBsb2FkLw==
#
# path: ./models/models/tensorflow/mnist-upload/
# file: ./mnist-upload.tar.gz <-- THIS MUST MATCH <model_name>-<model_tag>.tar.gz (generated from _tar)

#TODO:  Add --force
@app.route("/predict-server-upload/<string:model_name>/<string:model_tag>/<string:model_type>/<string:model_runtime>/<string:model_chip>/<string:model_path>/", methods=['POST'])
def predict_server_upload(model_name,
                          model_tag,
                          model_type,
                          model_path, # relative to /root/ (ie. ./models/models/tensorflow/mnist-cpuupload/)
                          model_runtime=None,
                          model_chip=None):
    ...
    # TODO:  Check if path is secure 
    model_path = _os.path.expandvars(model_path)
    model_path = _os.path.expanduser(model_path)
    model_path = _os.path.abspath(model_path)
    model_path = _os.path.normpath(model_path)

    print(_request.files['file'])
    if _request.method == 'POST':
        # check if the post request has the file part
        if 'file' not in _request.files:
            return_dict = {"status": "incomplete",
                           "error_message": "NOT_OK_NEED_FILE",
                           "model_name": model_name,
                           "model_tag": model_tag,
                           "model_path": model_path}
        ...
        if f.filename == '':
            #_flash('No selected file')
#            return _redirect(request.url)
            return_dict = {"status": "incomplete",
                           "error_message": "NOT_OK_NEED_FILENAME",
                           "model_name": model_name,
                           "model_tag": model_tag,
                           "model_path": model_path}
        ...
        if f and _allowed_file(f.filename):
            ...
            full_path = _os.path.join(model_path, filename)
    ...
    return_dict = {"status": "incomplete",
                   "error_message": "NOT_OK_NEED_POST",
                   "model_name": model_name,
                   "model_tag": model_tag,
                   "model_path": model_path}
...
def stream_kube_start(model_name,
    ...
    endpoint_url = 'http://%s/stream/topics' % stream_url
...
def stream_http_describe(stream_url,
    ...
    if stream_topic:
        endpoint_url = 'http://%s/stream/topics/%s' % (stream_url, stream_topic)
    else:
        endpoint_url = 'http://%s/stream/topics' % stream_url
...
def stream_kube_describe(model_name,
    ...
    # TODO:  Enrich return_dict with model_name and model_tag
...
def stream_http_consume(stream_url,
    ...
    endpoint_url = 'http://%s/stream/consumers/%s' % (stream_url, stream_consumer_name)
    ...
    endpoint_url = 'http://%s/stream/consumers/%s/instances/%s/subscription' % (stream_url, stream_consumer_name, stream_consumer_name)
    print(endpoint_url)
    response = _requests.post(url=endpoint_url,
                              headers=content_type_headers,
                              data=body.encode('utf-8'),
    ...
    endpoint_url = 'http://%s/stream/consumers/%s/instances/%s/records' % (stream_url, stream_consumer_name, stream_consumer_name)
    print(endpoint_url)
    response = _requests.get(url=endpoint_url,
                             headers=accept_headers,
                             timeout=timeout_seconds)
    ...
    endpoint_url = 'http://%s/stream/consumers/%s/instances/%s' % (stream_url, stream_consumer_name, stream_consumer_name)
    print(endpoint_url)
...
# TODO:  We shouldn't specify model_tag since this should route through Istio
...
def predict_kube_route(model_name,
    ...
    if type(model_shadow_tag_list) is str:
        ...
        model_shadow_tag_list = model_shadow_tag_list[1:-1].split(',') 
...
# TODO:  Figure out why this doesn't work the 2nd time we call it (to update the routes)
# This is busted, too:
...
# Http Mode:
#     pipeline http <port> <debug=True|False>
