...
def stream_kube_produce(model_name,
    ...
    full_stream_url = 'http://%s/topics/%s' % (stream_url, stream_input_topic)

    print("")
    print("Producing stream for topic '%s' at '%s'." % (stream_input_topic, full_stream_url))
    print("")
...
def predict_kube_route(model_name,
    ...
    for model_tag in model_shadow_tag_list:
        if not model_split_tag_and_weight_dict[model_tag] or int(model_split_tag_and_weight_dict[model_tag]) != 0:
            error_message = 'Since model_tag "%s" is targeted for traffic showing, it must be included in the model_split_tag_and_weight_dict with traffic split set to 0 as follows: --model-split-tag-and-weight-dict=\'{"%s":0,...}\'.'
