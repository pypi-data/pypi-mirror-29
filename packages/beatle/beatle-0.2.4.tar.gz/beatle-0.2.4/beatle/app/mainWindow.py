...
from beatle import tran, activity, plugin
from beatle.ctx import localcontext as context
from beatle.app import res, resources
from beatle.app.ui import dlg, pane
from beatle.app.ui.tools import unset_menu_handlers
from beatle.app.ui.dlg import BuildTools, PreferencesDialog
from  .proCxxGUI import MainWindow as MainWindowBase
from beatle.lib.handlers import Identifiers
from beatle.model import Workspace, Project
from beatle.tran import TransactionStack, TransactionalMethod
...
class mainWindow(MainWindowBase):
    ...
    _save = Identifiers.register("ID_SAVE_PROJECT")
    ...
    def OnEditUndo(self, event):
        ...
        wx.LogMessage('undo {0}'.format(TransactionStack.instance.UndoName()))
        TransactionStack.DoUndo()

    def OnUpdateEditUndo(self, event):
        """Handle update edit undo"""
        if TransactionStack.instance.CanUndo():
            event.Enable(True)
            name = TransactionStack.instance.UndoName()
            if not name is None:
    ...
    def OnEditRedo(self, event):
        ...
        wx.LogMessage('redo {0}'.format(TransactionStack.instance.RedoName()))
        TransactionStack.DoRedo()

    def OnUpdateEditRedo(self, event):
        """Handle update edit redo"""
        if TransactionStack.instance.CanRedo():
            event.Enable(True)
            name = TransactionStack.instance.RedoName()
            if not name is None:
    ...
    @TransactionalMethod('create workspace {0}')
    @wxx.CreationDialog(dlg.WorkspaceDialog, Workspace)
    ...
    def OnEraseStartPage(self, event):
        ...
        bmp.LoadFile(os.path.join(res.__path__[0],'beatle.jpg'), wx.BITMAP_TYPE_JPEG)
    ...
    @TransactionalMethod('open workspace {0}')
    def OnOpenWorkspace(self, event):
        ...
        if workspace:
            TransactionStack.DoLoad(workspace)
        return True

    @TransactionalMethod('reload last workspace')
    ...
    @TransactionalMethod('open recent')
