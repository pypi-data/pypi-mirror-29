...
from beatle.model import Member, decorator as ctx
...
class Constructor(Member):
    ...
    def AutoArgs(self):
        ...
        for x in args:
            if x._name in used:
                continue
            used.append(x._name)
            if type(x) is RelationFrom:
                ti = typeinst(type=x.GetTo().parent, ref=True, const=True)
            else:
                ti = copy.copy(x._typei)
            Argument(parent=self, name=x._name, type=ti)
    ...
    def AutoInit(self, save=True):
        ...
        inh = [x._ancestor for x in cls(Inheritance, filter=self_cls, cut=True)]
        ...
        members = cls(MemberData, filter=lambda x: self_cls(x) and not x._static)
        if len(members) > 0:
        ...
        if hasattr(self, '_pane') and not self._pane is None:
            book = get_context().frame.docBook
    ...
    def SetPreferred(self, flag):
        ...
        if flag:
            ...
            pre = cls(Constructor, filter=criteria)
            assert len(pre) < 2
            if len(pre):
                pre[0].SaveState()
    ...
    def GetTabBitmap(self):
        ...
        from beatle.app.resources import GetBitmap
