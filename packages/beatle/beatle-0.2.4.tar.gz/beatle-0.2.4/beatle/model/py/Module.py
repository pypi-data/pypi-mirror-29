...
from beatle.model import TComponent, Folder
...
class Module(TComponent):
    ...
    def locals(self):
        ...
        raw = self(Class, Data, Function, filter=f)
    ...
    def _local_import(self, name):
        ...
        for x in self(Module, filter=f, cut=True):
            if x._name == name:
                return x
        f = lambda x: x.parent.inner_package == container
        for x in self(Package, filter=f, cut=True):
            if x._name == name:
                return x
        return None
