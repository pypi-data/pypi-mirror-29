...
class xmlmetadoc(object):
    ...
    def __init__(self, partname, **kwargs):
        ...
        self.ok = False
        from beatle.plugin.models import relation
        path = os.path.join(relation.__path__[0],
        ...
        try:
            ...
            self.parent = self.dictionary.get('parent', None)
    ...
    def filter_set(self, component_set):
        ...
        for component in component_set:
            puntuation = 0
            _filter = component.get('filter', None)
            if _filter is None:
                result.append((component, 0))
                continue
            _filter = _filter.strip()[1:-1]  #  { and } left
            filtered_out = False
            for condition in _filter.split(','):
    ...
    def load_constructors(self, container = None, container_instance = None):
        ...
        if container is None:
            container = self.root
        constructor_set = container.find('ctors')
        if  constructor_set is None:
            constructor_set = container.find('constructors')
        if constructor_set is None:
            return
        if container_instance is None:
            container_instance = self.target
    ...
    def load_destructor(self, container = None, container_instance = None):
        ...
        if container is None:
            container = self.root
        destructor = container.find('dtor')
        if  destructor is None:
            destructor = container.find('destructor') 
        if destructor is None:
            return
        if container_instance is None:
            container_instance = self.target
...
#                 keys.update({'parent': cls})
#                 beatle.model.cc.MemberData(**keys)
#             # constructores
...
#                 keys.update({'parent': cls})
# 
#                 method = beatle.model.cc.MemberMethod(**keys)
#                 # argumentos
