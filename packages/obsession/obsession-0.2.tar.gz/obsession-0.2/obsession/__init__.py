...
__version__ = '0.2'
__all__ = ['SessionMiddleware', 'CookieIdPersistence', 'Session',
           'FileBackend', 'MemoryBackend']

logger = logging.getLogger(__name__)

VALID_ID_CHARS = (b"ABCDEFGHIJKLMNOP"
                  b"QRSTUVWXYZabcdef"
                  b"ghijklmnopqrstuv"
                  b"wxyz0123456789.-")
...
def _generate_id(count=ID_LEN,
                 _random_bytes=os.urandom,
                 _trans=ID_TRANS_TABLE):
    """
...
class CookieIdPersistence(IdPersistence):
    ...
    def __init__(self,
                 ...
                 max_age=None, path='/', domain='', secure=False):
        self.set_cookie_template = cookie_name + '=%s;HttpOnly'
        if path:
        ...
        self._extract_cookie = re.compile(
            r'\b{}=({})'.format(re.escape(cookie_name), ID_PATTERN)).search
...
class JSONSerializer(Serializer):
    ...
    def __init__(self, separators=(',', ':'), indent=None, sort_keys=False):
        self._iterencode = json.JSONEncoder(separators=separators,
                                            indent=indent,
                                            sort_keys=sort_keys).iterencode
    ...
    def load(self):
        ...
        if id is not None:
            try:
                ...
                logger.warn("Could not load session %r from %r", id, self.backend)
                self.orig_id = None
                self.id = _generate_id()
                self.id_needs_persisting = True
                self.data = {}
...
class FileBackend(Backend):
    ...
    def keys(self):
        ...
        l = len(self.prefix)
        return (s[l:] for s in os.listdir(self.directory) if p(s))


def SessionMiddleware(app,
                      id_persister=CookieIdPersistence(),
                      backend=FileBackend()):

    def wsgi_app_with_sessions(environ,
                               start_response,
                               app=app,
