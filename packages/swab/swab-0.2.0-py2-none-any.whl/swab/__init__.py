...
class Swab(object):
    ...
    def __init__(self, datadir, wsgi_mountpoint='/swab'):
        ...
        # Function evaluted to determine whether to include a user in
        # experiments. Users not included will always see the control variant
        # (ie the first listed)
        self.include_test = lambda environ, experiment: True
        self.exclude_test = lambda environ, experiment: False

    def include(self, environ, experiment):
        e = self.experiments[experiment]
        return e.include(environ) and not e.exclude(environ)

    def middleware(self, app, cookie_domain=None, cookie_path=None,
                   cache_control=True):
        """
...
def show_variant(environ, experiment, record=False, variant=None):
    ...
    if not swab.include(environ, experiment):
        return variant if variant is not None else exp.control
