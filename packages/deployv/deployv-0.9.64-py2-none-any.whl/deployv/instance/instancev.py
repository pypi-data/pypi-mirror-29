...
class InstanceV(DockerV):
    ...
    def add_repo_web(self):
        for repo in self.__full_config.instance_config.get("repositories"):
        ...
        branch_web = {
            ...
            "type": "git"}
        if not self.__full_config.instance_config.get("repositories"):
            self.__full_config.instance_config.update({'repositories': [branch_web]})
        else:
            self.__full_config.instance_config.get("repositories").append(branch_web)
...
def deactivate_database(config, db_name):
    ...
    if config.get('use_mailhog'):
        sqls.update({
            "out_mail": "UPDATE ir_mail_server SET smtp_host = '1.1.1.1', smtp_encryption = 'none'"
                        ", smtp_port = 25, smtp_user = 'user', smtp_pass = 'pass';",
            "mail_cron": "UPDATE ir_cron set active = true where id in "
                         "(select res_id from ir_model_data where "
                         "module = 'mail' and name = 'ir_cron_mail_scheduler_action');"
            })
