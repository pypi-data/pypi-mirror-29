...
def create(ctx, config_file, backup_src, external, key_file, without_db, database, without_demo):
    ...
    temp_folder = (config_dict.get('container_config').get('temp_folder') or
                   config.has_option('deployer', 'temp_folder') and
                   config.get('deployer', 'temp_folder'))
    config_dict.get('container_config').update({
        'temp_folder': utils.makedir(temp_folder or "/tmp")
        })
...
def backupdb(ctx, config_file, backup_dir, external, reason, tmp_dir, database, cformat,
    ...
    temp_folder = (config.has_option('deployer', 'temp_folder') and
                   config.get('deployer', 'temp_folder'))
    config_dict.get('container_config').update({
            'temp_folder': utils.makedir(temp_folder or "/tmp")
        })
...
def restore(ctx, config_file, backup_src, database, container, admin_pass,
    ...
    for key, value in params_required.items():
        if not config_dict.get('container_config').get(key):
            config_dict.get('container_config').update({key: value})
    if nginx_url:
...
def deactivate_backup(ctx, prefix, store_path, backup_src, admin_password,
    ...
    temp_folder = (config.has_option('deployer', 'temp_folder') and
                   config.get('deployer', 'temp_folder'))
    default_config.get('container_config').update({
        'temp_folder': utils.makedir(temp_folder or "/tmp")
    })
...
def build(ctx, config_file, repo, branch, image_name, working_folder,
    ...
    temp_folder = utils.makedir((config_dict.get('container_config').get('temp_folder') or
                                config.has_option('deployer', 'temp_folder') and
                                config.get('deployer', 'temp_folder') or "/tmp"))
    config_dict.get('container_config').update({
        'temp_folder': temp_folder
        })
    config_dict.get('container_config').update({'build_image': True})
    working_folder = working_folder or temp_folder
...
def update_production(ctx, config_file, container, db_name, db_user, db_password, db_host, db_port,
    ...
    if not backup_dir:
        backup_dir = utils.get_backup_src(
            config_dict=config_dict, deployv_config=config
        ) or os.getcwd()
