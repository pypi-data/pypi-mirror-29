...
class BaseWorker(multiprocessing.Process):
    ...
    def execute_rpc(self, message):
        ...
        parameters.get('instance', {}).get('config', {}).update({
            'db_owner': self.config_obj.postgres.get('db_owner'),
            'db_owner_passwd': self.config_obj.postgres.get('db_owner_passwd')
        })
        ...
        command_config.set_worker_config_sections(self.config_obj.worker_config)
        if module == 'commandv':
            command = commandv.CommandV(command_config)
