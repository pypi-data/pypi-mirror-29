...
import optel.datalake.config.elastic as ce
...
def write_to_elastic(table, destination, elastic_config=None):
    """
    ...
    Args:
        ...
        destination (str): The name of the index under which the dataframe
            will be written.
        elastic_config: Elastic Search configuration object.  See also
            :mod:`optel.datalake.config.elastic`
    """
    elastic_config = elastic_config or ce
    ...
    table.write \
        ...
        .option("es.nodes", elastic_config.es_nodes) \
        .option("es.net.ssl.protocol", "true") \
        .option("es.nodes.wan.only", "true") \
        .option("es.net.http.auth.user", elastic_config.es_user) \
        .option("es.field.read.empty.as.null", "yes") \
        .option("es.net.http.auth.pass", elastic_config.es_password) \
        .save()


def read_from_elastic(index, spark=None):
    """
