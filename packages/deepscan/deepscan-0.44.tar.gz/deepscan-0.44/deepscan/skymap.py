...
from . import sextractor, masking, NTHREADS, BUFFSIZE
...
def skymap(data, meshsize_initial, meshsize_final, mask=None, makeplots=False, 
           ...
           create_mask=False, sex_kwargs=None, ps=1, mzero=0, **kwargs):
    ...
    if create_mask:
        ...
        if mask is None:
            mask = masking.mask_ellipses(np.zeros_like(data, dtype='bool'),
                                         es=es_sex, rms=None, fillval=1
                                         ).astype('bool')
        else:
            mask = masking.mask_ellipses(mask, es=es_sex, rms=None, fillval=1
                                         ).astype('bool')
