...
class Source():
    ...
    def measure_colour(self, data1, data2, segmap, mask=None, sky1=None,
                       sky2=None):
        '''
    ...
    def fit_1Dsersic(self, data, segmap, ps, mzero, dr=1, Rmax=250, mask=None,
                     minpts=5, makeplots=False, p0=None, pix_corr=1, Nreps=1,
                     tol=1.05, smooth_size=None, verbose=False, mask_radius=None,
                     skybox=None, sky=None, **kwargs):
...
class Cutout():
    ...
    def __init__(self, data, x0, y0, size, sky=None, rms=None, mask=None,
        ...
        if wcs is None:
            _ = Cutout2D(data,(x0,y0),size,copy=True,mode=mode,
                                                                 **kwargs)
            self.data = _.data
            self.header = _.wcs.to_header()
        else:
            _ = Cutout2D(data,(x0,y0),size,copy=True,**kwargs).data
            self.data = _.data
            self.header = _.wcs.to_header()
