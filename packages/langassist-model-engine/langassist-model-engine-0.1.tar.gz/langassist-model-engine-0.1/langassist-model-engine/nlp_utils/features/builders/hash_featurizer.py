...
from nlp_utils.features import sentence_features
from nlp_utils.features.builders.base import BaseFeaturizer

class HashFeaturizer(BaseFeaturizer):
    '''
    This class will turn a piece of text into a *vocabulary-free* set of features
    ...
    '''
    def __init__(self, word_range, char_range):
        BaseFeaturizer.__init__(self)
        self.word_range = word_range
        self.char_range = char_range
        self.word_pres_vec = HashingVectorizer(ngram_range=self.word_range, binary=True)
        self.char_count_vec = HashingVectorizer(ngram_range=self.char_range, analyzer='char', binary=False)
    ...
    def featurize_all(self, samples):
        ...
        self.logger.debug("Done in %0.3fs" % (time() - t0))
