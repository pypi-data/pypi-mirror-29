...
class Vlab(object):
    ...
    def __init__(
        ...
        self._cache_file = os.path.join(self._working_directory, 'jemu.cache.sha1')
    ...
    def load(self, file_path):
        ...
        self.perform_silent_remove(gen_new_jemu, gen_new_so)
        if self._local_jemu:
            self.load_from_local(gen_new_so, self._firmware)
        else:  # from cloud
            self.load_from_cloud(gen_new_so, gen_new_jemu, self._firmware, new_signature)
    ...
    def perform_silent_remove(self, gen_new_jemu, gen_new_so):
        if gen_new_jemu:
