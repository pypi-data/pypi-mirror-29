...
class Vlab(object):
    ...
    def _get_latest_jemu_version(self):
        return self._web_api.get_jemu_version() if not self._local_jemu else self._get_jemu_version()
    ...
    def _is_instructions_latest_version(self, jemu_latest_version):
        if jemu_latest_version and os.path.isfile(self._instruction_lib):
            lib = ctypes.cdll.LoadLibrary(self._instruction_lib)
            try:
