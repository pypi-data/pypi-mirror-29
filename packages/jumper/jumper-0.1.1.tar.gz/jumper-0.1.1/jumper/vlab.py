...
DEFAULT_JEMU_FILE = os.path.join(os.path.expanduser('~'), '.jumper', 'jemu')
...
class Vlab(object):
    ...
    :param print_trace: If true, a trace with the registers value at each tick of the execution will be generated.
    :param print_interrupts_trace: If true, a trace with the interrupts will be generated.
    ...
    def is_jemu_latest_version(self):
        ...
        if jumper_latest_version and os.path.isfile(self._jemu_path):
    ...
    def __init__(self, working_directory=None, config_file=None, gdb_mode=False, sudo_mode=False, print_trace=False, print_interrupts_trace=False, trace_output_file="", print_uart=False, uart_output_file="", jemu_file=None):
        self.check_version()
        # self._args = args
        ...
        self._instruction_lib = os.path.join(self._working_directory, 'instructions.so')
        self._program_bin = os.path.join(self._working_directory, 'program.bin')
        self._instructions_tgz = os.path.join(self._working_directory, 'instructions_lib.tgz')
        ...
        self._print_interrupts_trace = print_interrupts_trace
        ...
        self._jemu_path = self._jemu_bin_src if self._local_jemu else (jemu_file or DEFAULT_JEMU_FILE)
        self._instructions_lib = os.path.join(self._working_directory, "instructions.so")
    ...
    def _read_file_signature_backup(self):
        ...
        if os.path.isfile(self._cache_file):
            if os.path.isfile(self._instruction_lib):
    ...
    def _unzip_jemu(self):
        try:
            ...
            raise MissingFileError('Error: Could not open jemu file')

        if os.path.isfile(self._jemu_bin):
            os.chmod(self._jemu_bin, 0o777)
            copyfile(self._jemu_bin, self._jemu_path)
    ...
    def _unzip_instructions_lib(self):
        ...
        if os.path.isfile(self._instruction_lib):
            os.chmod(self._instruction_lib, 0o777)
    ...
    def load(self, file_path):
        ...
        gen_new_jemu = True
        if self._local_jemu or self.is_jemu_latest_version():
            gen_new_jemu = False

        gen_new_so = True
        if not gen_new_jemu:
            prev_signature = self._read_file_signature_backup()
            if prev_signature == new_signature and os.path.isfile(self._program_bin) :
                gen_new_so = False

        self.preform_silent_remove(gen_new_jemu, gen_new_so)
        if self._local_jemu:
            self.load_from_local(gen_new_so, file_path)
        else:  # from cloud
            self.load_from_cloud(gen_new_so, gen_new_jemu, file_path, new_signature)

        if gen_new_so and os.path.isfile(self._instruction_lib) and os.path.isfile(self._jemu_path):
            self._write_file_signature_backup(new_signature)

    def preform_silent_remove(self, gen_new_jemu, gen_new_so):
        if gen_new_jemu:
    ...
    def load_from_local(self, gen_new_so, file_path):
        """
        Loads locally firmware to a virtual device and initialises a Virtual Lab session.
        :param gen_new_so: True if need to generated a new instruction lib
    ...
    def load_from_cloud(self, gen_new_so, gen_new_jemu, file_path, new_signature):
    ...
    def start(self, ns=None):
        ...
        jemu_cmd.append(self._jemu_path)
        jemu_cmd.append('-w')
        jemu_cmd.append('--sdk-port')
        jemu_cmd.append(str(self._jemu_port))

        if self._gdb_mode:
            jemu_cmd.append('-g')
        if self._sudo_mode:
            jemu_cmd.append('-s')
        if self._print_trace or self._print_interrupts_trace:
            if self._print_trace:
                jemu_cmd.append('-t')
            if self._print_interrupts_trace:
                jemu_cmd.append('-i')
        ...
        # if self._args.call_trace != "":

            # try:
            #     cmd = "arm-none-eabi-objdump -d " + self._args.call_trace + "> objdump.txt"
            ...
            # jemu_cmd.append('--call-trace')
            # jemu_cmd.append('objdump.txt')
            # try:
            #     cmd = "arm-none-eabi-objdump -d " + self._args.call_trace + "> objdump.txt"
            #     p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,cwd=self._working_directory)
            #     # for line in p.stdout.readlines():
        ...
        if self._jemu_debug:
            ...
            self._jemu_port = self.get_port_number()
            jemu_connection()
