...
def restore_versions(x):
    ...
    try:
        ...
        try:
            ...
            # print('[Key not in glacier] - %s/%s/%s' %(bucket, key, v_id))
            logging.warning( '[Key not in glacier] - Key: %s/%s VersionID: %s' % (bucket, key, v_id) )
            pass

    except ClientError as e:
        if e.response['ResponseMetadata']['HTTPHeaders']['x-amz-delete-marker'] == 'true':
            logging.warning( '[Delete Marker Found] - Ignoring key %s/%s VersionID: %s' % (bucket, key, v_id) )
            pass
        elif e.response['ResponseMetadata']['HTTPStatusCode'] != "200":
            err = {
                'key': key,
            ...
            click.echo(err)
        # if e.response['ResponseMetadata']['HTTPHeaders']['x-amz-delete-marker'] == 'true':
        #     # that's for the keys with delete marker
        #     logging.warning( '[Delete Marker Found] - Ignoring key %s/%s VersionID: %s' % (bucket, key, v_id) )
        #     pass
