...
class ChatRoom(ce.rooms.Room):
    ...
    def watch_room(self):
        self.watch(self._function_callback)
    ...
    def add_privileged_user(self, user_id, privilege_type):
        self.privileged_users.append(PrivilegedChatUser(user_id, self.id, privilege_type))
    ...
    def save_privileged_users(self):
        filename = self._save_directory + "room_" + str(self.id) + "_name_" + self.name.replace(" ", "_") + "_privileged_users"
