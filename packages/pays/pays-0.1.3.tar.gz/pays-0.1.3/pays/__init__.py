...
def update_data():
    ...
    try:
        ...
        raise FileNotFoundError(
            'Avez-vous récupéré le dépôt "countries" ? git submodule udpate ?')
    for country in countries:
        data = {
...
class Country:
    ...
    def __init__(self, data: dict):
        ...
        try:
            ...
            raise AttributeError('Country instancié à vide')
...
class Countries:
    ...
    def __iter__(self):
        ...
        """
        for _, data in self.data[self.locale].items():
            yield Country(data)
    ...
    def load_data(self, locale: str = None):
        """
        Récupération des données depuis les fichiers .json selon la
        `locale` demandée.
    ...
    def _get_by_alpha3(self, alpha: int, locale: str = None) -> dict:
        """
