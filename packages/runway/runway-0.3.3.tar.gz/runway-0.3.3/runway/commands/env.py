...
class Env(Base):
    ...
    def run(self, deployments=None, deployments_config_path=None,  # noqa pylint: disable=too-many-branches
            command='plan'):
        """Execute apps/code command."""
        if deployments is None:
            # Grab deployment list from yaml in repo
            if deployments_config_path is None:
                deployments_config_path = self.deployments_config_path
            if os.path.isfile(deployments_config_path):
                with open(deployments_config_path) as data_file:
                    deployments = yaml.safe_load(data_file)['deployments']
            else:
                deployments = []
    ...
    def plan(self, deployments=None, deployments_config_path=None):
        """Plan apps/code deployment."""
        self.run(deployments=deployments,
                 deployments_config_path=deployments_config_path,
                 command='plan')

    def deploy(self, deployments=None, deployments_config_path=None):
        """Deploy apps/code."""
        self.run(deployments=deployments,
                 deployments_config_path=deployments_config_path,
                 command='deploy')
