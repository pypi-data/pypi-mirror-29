...
class SCA(object):
    ...
    def _iter_clust(self):
        ...
        model = AgglomerativeClustering(n_clusters=2, affinity='cosine', 
                                        linkage='average')
        T = model.fit_predict(data)
        s = silhouette_score(X=sca_dist, labels=T, metric='precomputed')
        optimum = (2,0)
        p = s
        
        for k in range(3,11):
            model = AgglomerativeClustering(n_clusters=k, affinity='cosine',
                                        linkage='average')
            T = model.fit_predict(data)
        ...
        #S = fclusterdata(data, t=optimum[0], criterion='maxclust',
          #               metric='cosine', method='average')
        model = AgglomerativeClustering(n_clusters=optimum[0], affinity='cosine', 
                                        linkage='average')
        S = model.fit_predict(data)
        n_secs = optimum[0]
        
        sectors = {}
        for i in range(0, n_secs):
            sectors[i+1] = self.sig_sec[S==i]
