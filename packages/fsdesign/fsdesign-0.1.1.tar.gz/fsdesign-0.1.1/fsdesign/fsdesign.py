...
class FSDesign:
    ...
    def generate_vignettes(self):
        ...
        for params in vignette_params:
            yield self.text_template.render(**params._asdict())

    def to_csv(self, path):
        """Write vignettes to a csv file"""

        with open(path, 'w', encoding='utf8') as f:
            writer = csv.DictWriter(f, ['ID', 'Text'])
            writer.writeheader()
            for index, vignette in enumerate(self.generate_vignettes()):
                id = index + 1
                row = {'ID': id, 'Text': vignette}
                writer.writerow(row)
