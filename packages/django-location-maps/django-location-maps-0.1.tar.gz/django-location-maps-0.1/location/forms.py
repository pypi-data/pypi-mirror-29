...
class LocationForm(forms.ModelForm):
    ...
    states = forms.ModelChoiceField(queryset=State.objects.all())
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk:
            ...
            self.fields['country'].initial = self.instance.city.estado.pais.pk
            self.fields['states'].choices = [(0, '---------')] + [(state.pk, state.nome) for state in State.objects.filter(pais_id=self.instance.city.estado.pais.pk).all()]
            self.fields['states'].inital = self.instance.city.estado.pk
