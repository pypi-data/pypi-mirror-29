    ...
    def get_container_content_paged(self, container_id, num_page, page_size, main_category, is_temp):
        ...
        if main_category == 3: #shopping
            if num_page == 0:
                ...
                return watched_uno.reverse()[num_page*page_size:(num_page+1)*page_size]
        elif main_category == 1: #news
            if is_temp == 1:
                ts = time()
                # per i contenitori temporanei le news devono essere mostrate dalla meno recente alla piu' recente
                result_news_reversed = list(selected_collection.find({}).sort({"birthday":-1}))
                return result_news_reversed[num_page*page_size:(num_page+1)*page_size]
            else:
                # per i contenitori personalizzati le news devono essere mostrate dala piu' recente alla meno recente
                result_news = list(selected_collection.find({}).sort({"birthday": 1}))
