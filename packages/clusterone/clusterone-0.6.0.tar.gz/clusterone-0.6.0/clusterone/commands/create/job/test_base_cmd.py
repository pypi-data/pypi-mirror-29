...
def test_framework_aliases(mocker):
    base_cmd.path_to_project = mocker.Mock(return_value={'id': "project-id-123456", "commits": [OrderedDict([('id', '4a82d16c7995856c7973af38f2f5ba4eac0cd2d1')]), OrderedDict([('id', '4a82d16c79elorapc7973af38f2f5ba4eac0cd2d1')])]})
    base_cmd.time_limit_to_minutes = mocker.Mock(return_value=123456)
    config = Config()
    config.load()
    ...
    result = base_cmd.base(context, {'framework': 'tensorflow', 'project_path': 'mnist-demo', 'python_version': '3', 'requirements': None, 'commit': 'latest', 'name': 'old-morning-562', 'module': 'mymodule.py', 'package_manager': 'pip', 'package_path': '', 'description': '', 'time_limit': 2880, 'datasets': '',})

    assert result['parameters']['framework']['name'] == 'tensorflow'
...
def test_default_requirement_conda(mocker):
    ...
    result = base_cmd.base(context, {'framework': 'tf', 'project_path': 'mnist-demo', 'python_version': '3', 'requirements': None, 'commit': 'latest', 'name': 'old-morning-562', 'module': 'mymodule.py', 'package_manager': 'anaconda', 'package_path': '', 'description': '', 'time_limit': 2880, 'datasets': '',})
...
def test_framework_version_default(mocker):
    base_cmd.path_to_project = mocker.Mock(return_value={'id': "project-id-123456", "commits": [OrderedDict([('id', '4a82d16c7995856c7973af38f2f5ba4eac0cd2d1')]), OrderedDict([('id', '4a82d16c79elorapc7973af38f2f5ba4eac0cd2d1')])]})
    base_cmd.time_limit_to_minutes = mocker.Mock(return_value=123456)
    config = Config()
    config.load()
    ...
    result = base_cmd.base(context, {'framework': 'tensorflow', 'project_path': 'mnist-demo', 'python_version': '3', 'requirements': None, 'commit': 'latest', 'name': 'old-morning-562', 'module': 'mymodule.py', 'package_manager': 'anaconda', 'package_path': '', 'description': '', 'time_limit': 2880, 'datasets': '',})

    assert result['parameters']['framework']['slug'] == 'tensorflow-130'
