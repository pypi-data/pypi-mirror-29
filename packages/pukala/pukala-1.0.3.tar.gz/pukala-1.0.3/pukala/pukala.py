...
def make_fcall(fn, prefix=None, *args, **kw):
    fname = prefix and "%s.%s" % (prefix, fn.__name__) or fn.__name__
    if not kw:
        return ["_fcall", fname] + list(args)
    else:
        return ["_fcall", kw, fname] + list(args)

def mkf(fn, *args, **kw):
    return make_fcall(fn, None, *args, **kw)
...
class CallContext(object):
    def __init__(self, doc, path, args):
        ...
        self.path = path
        self.args = args
    def clone(self, args=None, deep=True):
        if deep:
        ...
        return res
    def getv(self, path):
        p = Path(path, self.doc, self.path.as_arr())
        return p.get_val(self.doc)
...
def grow(doc, context_obj):
    ...
    class Conf(object):
        def __init__(self, doc):
            ...
            self.defaults = {
                ...
                "thread_pool_size": 20,
                "verbose_fcall_history": False,
                "history_path_array": True
            }
            self.refresh()
        def refresh(self):
            self.is_vh = self.get_val("verbose_fcall_history")
            self.is_hpa = self.get_val("history_path_array")
        ...
        def is_verbose_history(self):
            return self.is_vh

        def is_history_path_array(self):
            return self.is_hpa
    ...
    class History(object):
        ...
        def add_call(self, call):
            p, f = call
            if self.conf.is_history_path_array():
                path = p.as_arr()
            else:
                path = p.as_str()

            if self.conf.is_verbose_history():
                self.__append_fcall(["single_call", f.get_fcall_arr(), path])
            else:
                self.__append_fcall(["single_call", f.get_fcall_name(), path])
        ...
        def add_cgroup(self, calls):
            if self.conf.is_verbose_history():
                for i in calls:
                    ...
                    self.__append_fcall(["cgroup_single", f.get_fcall_arr(), path])
            else:
                for i in calls:
                    p, f = i
                    if self.conf.is_history_path_array():
                    ...
                    self.__append_fcall(["cgroup_single", f.get_cgroup(), f.get_fcall_name(), path])
