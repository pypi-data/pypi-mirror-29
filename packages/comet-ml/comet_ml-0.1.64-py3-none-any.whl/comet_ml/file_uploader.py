...
def upload_repo(project_id, experiment_id, file_path, get_path_endpoint,
    ...
    try:
        ...
        zip_folder, zip_path = compress_py_files(repo_root_path, config['uploaded_extensions'])

        send_zip_file(post_zip_endpoint, experiment_id, project_id, zip_path)
        Reporting.report(event_name="repo_uploaded",experiment_key=experiment_id,project_id=project_id,api_key=api_key)


        # Cleanup temp directory
        shutil.rmtree(zip_folder)
    except ValueError as e:
        print("\ncomet.ml error, repo files would not be synced. message: %s" % e)
        Reporting.report(event_name="repo_uploaded_failed",experiment_key=experiment_id,project_id=project_id,api_key=api_key, err_msg=str(e))



def upload_repo_start_process(project_id, experiment_id, file_path,
                              get_path_endpoint, post_zip_endpoint,api_key, config):
