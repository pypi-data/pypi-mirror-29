...
class DockerMixin(object):
    ...
    def set_compiler(self, compiler):
        if self.use_docker:
            ...
            if not self.docker_helper or self.docker_helper.image != docker_image:
                ...
                self.docker_helper.run_in_docker("pip install -U conan conan_sword_and_sorcery=={version} && conan user".format(version=__version__), sudo=True)
    ...
    def upload(self, username, channel):
        self.docker_helper.run_in_docker("chown -R `whoami`:`whoami` {}".format(self.docker_storage_path))
