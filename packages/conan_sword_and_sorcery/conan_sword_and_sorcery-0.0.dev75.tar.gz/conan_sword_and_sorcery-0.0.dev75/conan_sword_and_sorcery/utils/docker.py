...
class DockerHelper(object):
    ...
    def run(self, allocate_tty=True):
        # Run the container using "my" user (--user $UID)
        self._running = "docker run --user `id -u` {options} {mnt} --name {name} --detach {image}".format(
    ...
    def run_in_docker(self, command):
        return cmd("docker exec -it {name} /bin/sh -c \"{command}\"".format(
