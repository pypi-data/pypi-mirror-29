        ...
        if self.use_docker:
            ...
            if not self.docker_helper or self.docker_helper.image != docker_image:
                ...
                try:
                    ...
                    cmd("id -u")
                    print("=== DOCKER ===")
                    self.docker_helper.run_in_docker("ls -la {}".format(remote_storage))
                    self.docker_helper.run_in_docker("id -u")
                except Exception:
                    pass

                # Install what is needed
                self.docker_helper.run_in_docker("pip install --user -U conan conan_sword_and_sorcery=={version} && conan user".format(version=__version__))
