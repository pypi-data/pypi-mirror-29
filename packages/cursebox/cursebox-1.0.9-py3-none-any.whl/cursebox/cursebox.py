        ...
        if self.check_version:
            try:
                with request.urlopen(
                    'https://gitlab.com/gorgonzola/cursebox/raw/master'
                    '/cursebox/version.py',
                    timeout=5
                ) as version_data:
                    version_available = version_data.read().decode('utf-8')
                    version_available = version_available.split('\n')[3]
                    version_available = version_available.split(' ')[2]
                    version_available = version_available.strip('\'')
...
The following options are recognised:
  ...
  -V, --check_version   Check online whether a newer version is available.
                        Optional -- no check is done by default. Not for the
                        paranoid, as an HTTPS request to gitlab.com (hosting
                        the code of Cursebox) is done if this is enabled.
...
def main():
    ...
    # Parse options.
    try:
        opts, args = getopt.getopt(argv, 'c:s:p:u:P:b:hV', [
            'config=',
            'server=',
            'port=',
            'username=',
