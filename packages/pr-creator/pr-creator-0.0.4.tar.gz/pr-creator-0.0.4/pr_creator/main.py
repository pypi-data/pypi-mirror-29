...
def process(app_bases, args):
    ...
    for app, url in github.pr_urls.iteritems():
    ...
    messages.extend(["{0}: {1}".format(project, x['html_url']) for project, x in prs.iteritems()])
    message_str = "\n".join(messages)
    logger.info("Composed the message string")
    for app, pr in prs.iteritems():
    ...
    # update_ticket(args)
    logger.info("Done!")
    # for proj, x in prs.iteritems(): print(proj, x['html_url'])
