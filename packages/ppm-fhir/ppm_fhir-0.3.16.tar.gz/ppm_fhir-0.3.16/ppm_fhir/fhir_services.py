...
class FHIRServices:
    ...
    def update_FHIR_object(self, object):
        ...
        try:
            ...
            logger.debug('[P2M2][DEBUG][update_FHIR_object]: Resource: {}, ID:{}'
                         .format(resource['resourceType'], resource['id']))

            # Make the FHIR request.
            response = requests.put(object_url.url, json=resource)

            logger.debug('[P2M2][DEBUG][update_FHIR_object]: Response: {}'.format(response))
        ...
        try:
            ...
            if flag_entries.total == 0:
                ...
                logger.debug('[P2M2][DEBUG][update_patient_enrollment]: Updating Flag "{}" with code: "{}"'
                             .format(flag_url.url, status))

                # Post it.
                response = requests.put(flag_url.url, json=flag.as_json())
                logger.debug('[P2M2][DEBUG][update_patient_enrollment]: Updating Flag response: {}'.format(response))
