...
class FHIRServices:
    ...
    def get_patient_everything(self, fhir_id):
        logger.debug("[P2M2][DEBUG][get_patient_everything: {}]".format(fhir_id))
    ...
    def update_FHIR_object(self, user_consent):
        ...
        try:
            ...
            object_url = furl.furl(user_consent['fullUrl'])
            object_url.scheme = 'https'

            user_data = user_consent['resource']

            # Make the FHIR request.
            response = requests.put(object_url.url, json=user_data)
    ...
    def update_user_consent(self, user_consent):
        ...
        try:
            # Get the URL and make sure it's using SSL
            consent_url = furl.furl(user_consent['fullUrl'])
            consent_url.scheme = 'https'
    ...
    def update_patient_enrollment(self, patient_id, status):
        ...
        try:
            ...
            if flag_entries.total == 0:
                ...
                # Ensure SSL
                flag_url = furl.furl(entry.fullUrl)
                flag_url.scheme = 'https'

                # Post it.
                return requests.put(flag_url.url, json=flag.as_json())
