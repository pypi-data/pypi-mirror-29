...
class ClassWithManagedProperties( object ):
	...
	def _Organize( cls ):
		...
		for name in dir( cls ):
			...
			if isinstance( obj, ( ManagedProperty, ManagedShortcut ) ):
	...
	def _RunDynamics( self, t ):
		...
		updates = {}
		for key, func in sorted( dynamics.items() ):
			( order, name ) = key
			try:
				value = func( t )
				...
				value = None
				for arg in exc.args:
					if isinstance( arg, dict ): updates.update( arg )
					else: value = arg
				if value is None: dynamics.pop( key )
				else: setattr( self, name, value )
			except:
				einfo = sys.exc_info()
				sys.stderr.write( 'Exception while evaluating dynamic .%s property of %s:\n' % ( name, self ) )
				getattr( self, 'excepthook', sys.excepthook )( *einfo )
				dynamics.pop( key )
			else:
	...
	def SetDynamic( self, name, func, order=None ):
		"""
		DOC-TODO
		"""
		if order is None:
			if not func: return self.UnsetDynamic( self, name ) # slightly slower
			else: order = -1
		dynamics = getattr( self, '_dynamics', None )
		if dynamics is None: dynamics = self._dynamics = {}
		key = ( order, name )
		if func: dynamics[ key ] = func
		else: dynamics.pop( key, None )
	
	def UnsetDynamic( self, name ):
		"""
		DOC-TODO
		"""
		dynamics = getattr( self, '_dynamics', None )
		if not dynamics: return
		keys = [ key for key in dynamics if key[ 1 ] == name ]
		for key in keys: dynamics.pop( key )
	...
	def GetDynamic( self, name ):
		...
		values = [ value for key, value in dynamics.items() if key[ 1 ] == name ]
	...
	def Set( self, **kwargs ):
		...
		for propertyName, value in kwargs.items():
			if not hasattr( self, propertyName ): raise AttributeError( '%s class does not have a %r property' % ( self.__class__.__name__, propertyName ) )
	...
	def Inherit( self, other ):
		...
		for ( flag, name ), func in getattr( other, '_dynamics', {} ).items():
	...
	def ShareProperties( self, *pargs, **kwargs ):
		...
		A similar effect can be obtained via `b.LinkPropertiesWithMaster( a, ... )` although there you are limited to
		one sharee at a time.
		
		Undo with `.MakePropertiesIndependent()`
		...
		for propertyName in props:
			...
			for other in others:
				other.UnsetDynamic( propertyName )
	...
	def LinkPropertiesWithMaster( self, master, *pargs, **kwargs ):
		...
		Returns:
		    `self`
		"""
		master.ShareProperties( self, *pargs, **kwargs )
		return self
...
class ManagedProperty( object ):
	def __init__( self, default, transfer=None, doc='', canonical_name=None, read_only=False, notes='', names=() ):
		"""
		For general documentation about the ManagedProperty class,
		see elsewhere (DOC-TODO)
		"""
	...
	def __doc__( self ): # ...whereas this enables `X.backgroundColor?` where `X` is the class itself.
		...
		s += '\n* Default value:  `{}`'.format( self.default[ 0 ] if numel( self.default ) == 1 else list( self.default ) )
	...
	def __set__( self, instance, value ):
		name = self.names[ 0 ]
		if callable( value ): return instance.SetDynamic( name, value, order=0 )
		...
		instance.SetDynamic( name, None, order=0 )
...
class ManagedShortcut( object ):
	def __init__( self, prop, index ):
		"""
		For general documentation about the ManagedShortcut class,
		see elsewhere (DOC-TODO)
		"""
	...
	def __set__( self, instance, value ):
		name = self.names[ 0 ]
		if callable( value ): return instance.SetDynamic( name, value, order=1 )
		...
		instance.SetDynamic( name, None, order=1 )
