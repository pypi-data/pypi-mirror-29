...
class Athena(object):
    ...
    def load_partitions(self, table, result_output=None):
        """
        ...
        if not self.database:
            self._logger.error('Set the database before running a query')
            return 'error'

        query = 'MSCK REPAIR TABLE {db}.{table};'.format(db=self.database, table=table)

        return self.execute_query(query, result_output)

    def execute_query(self, query, result_output=None):
        """
