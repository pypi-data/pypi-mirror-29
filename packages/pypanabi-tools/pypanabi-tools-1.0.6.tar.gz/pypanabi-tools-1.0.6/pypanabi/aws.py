...
class Athena(object):
    ...
    def update_partitions(self, table, database=None, result_output=None):
        """
        Update partitions for an specific table
        :param table: table name in the catalog [String]
        :param database: database name [String]
        :param result_output: S3 prefix to save output [String]
        :return: query identifier [String]
        """
        query = 'MSCK REPAIR TABLE {table};'.format(table=table)

        return self.execute_query(query, database, result_output)

    def execute_query(self, query, database=None, result_output=None):
        """
        ...
        :return: query identifier [String]
        """
        result_configuration = result_output or 's3://aws-athena-query-results-657245296933-us-west-2/tmp/'

        args = {
                'QueryString': '{query}'.format(query=query),
                'ResultConfiguration': {'OutputLocation': '{output}'.format(output=result_configuration)}
                }

        if database:
            args['QueryExecutionContext'] = {'Database': '{db}'.format(db=database)}
