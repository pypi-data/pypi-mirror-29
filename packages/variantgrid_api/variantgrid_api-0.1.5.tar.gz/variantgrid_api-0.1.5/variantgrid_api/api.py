...
class VariantGridAPI(object):
    ...
    def _add_classification_json(self, path, classification, dbsnp=None, variant=None, public=False):
        url_path = os.path.join(self.url, path) + ".json"
        data = {"classification" : classification,
                "public" : public}
        if dbsnp:
            data["dbsnp_string"] = dbsnp
        elif variant:
            data["variant_string"] = variant
        else:
            msg = "Need to specify either 'dbsnp' or 'variant'"
            raise ValueError(msg)
    ...
    def add_classifications_for_dbsnp(self, dbsnp, classification, public):
        url = 'variantclassification/api/add_classification'   
        return self._add_classification_json(url, classification, dbsnp=dbsnp, public=public)

    def add_classifications_for_variant(self, variant_string, classification, public):
        url = 'variantclassification/api/add_classification'   
        return self._add_classification_json(url, classification, variant=variant_string, public=public)
