...
class Controller(FloatLayout):
    ...
    def __init__(self, *args, **kwargs):
        ...
        t = threading.Thread(target=self.init_params)
        t.start()
    ...
    def init_params(self):
        ...
        try:
            self.true_caser.load_model()
            self.done_loading_model = True
        except Exception as e:
            rollbar.report_exc_info()
        self.asr_result = "Ready!"

        if self.config.get("user", "report") == "yes" and self.config.get("user", "apikey") != "":
            api_key = self.config.get("user", "apikey")
            info.get_system_info(api_key)
            self.config.set("user", "report", "no")
    ...
    def progress(self, done_percentage, text=None, is_finished=False):
    ...
    def run_asr_on_file(self, full_path):
        ...
        self.tokenizer, self.observer = audio_token.segment_audio(full_path, cur_api_key, callback=self.progress)
        if self.tokenizer and self.observer:
            self.observer.start()
            self.asr_result = "Audio segmentation ..."
    ...
    def start(self):
        ...
        print(long_audio, type(long_audio))
        if long_audio and long_audio != "0":
...
class VaisLongAudioApp(App):
    ...
    def build(self):
        ...
        config = self.config
        self.c = Controller(asr_result='Demo nhận dạng tiếng nói tiếng Việt của VAIS!')
