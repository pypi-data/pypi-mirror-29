...
class Controller(FloatLayout):
    ...
    def init(self):
        ...
        if self.btn_load_audio_file: self.btn_load_audio_file.on_press = self.show_mic_dialog
        if self.btn_stop:
            self.btn_stop.disabled = True
            self.btn_stop.color = (0, 0, 0, 1)

    def show_mic_dialog(self):
        devices = [DataItem(x) for x in device_info.get_device_list(input_only=False)]
        devices.reverse()
        for d in devices:
