...
@ram.cache(_cache_key)
def _cached_all_yomi(tool):
    return tool.get_all_yomi()

def _get_near_word(tool, s, k=1):
    s = _s_uni(s)
    words = _cached_all_yomi(tool)
    m = Matcher(words)
...
def get_near_main(tool, s):
    ...
    for p, y in ss:
        k = default_k if len(_s_uni(y)) < sleuth_len else long_k
        if p != y: # is Japanese?
            k = k + k_ja_minus
