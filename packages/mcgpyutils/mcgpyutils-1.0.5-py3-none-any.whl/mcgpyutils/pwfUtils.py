...
class ProWorkflowUtils:
    ...
    def get_project_by_id(self, project_id, params=None):
        ...
        if params:
            extra_params += self._build_extra_params_string(params)
            extra_params =  extra_params[1:] # Remove the leading &

        r = requests.get("{0}/projects/{1}?{2}".format(self.base_url, project_id, extra_params), auth=(self.user, self.password), headers=headers)
        return r.json()
