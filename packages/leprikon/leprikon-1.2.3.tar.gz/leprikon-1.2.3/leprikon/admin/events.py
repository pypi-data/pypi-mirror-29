...
class EventRegistrationAdmin(SubjectRegistrationBaseAdmin):
    ...
    def event_payments(self, obj):
        status = obj.get_payment_status()
        html = format_html(
            '<a class="related-lookup" style="color: {color}" href="{href_list}" title="{title}"><b>{amount}</b></a>',
            color       = status.color,
