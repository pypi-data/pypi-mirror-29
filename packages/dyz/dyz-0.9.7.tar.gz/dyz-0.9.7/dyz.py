...
def workers_thread(func):
    @wraps(func)
    def wrapper(*args, **kwargs):
        ctx = kwargs.get('ctx', False) or args[0]
        workers = ctx.obj.get('workers', 1) or 1
        if workers == 1:
...
def __section_list(ctx, arg):
    ...
    x = PrettyTable()
    if not arg:
        x.field_names = ["Index", "Section", "Database", "Host", "Port", "User", "Password", "Super User Password",
                         "Protocol",
                         "Mode"]
    else:
    ...
    for function in function.split(','):
        ...
        if not ids or batch_mode:
            ...
            for item_id in ids:
                ...
                click.echo('id=%s, args=%s' % (item_id, args))
                res = getattr(Model, function)(**args)
                if key:
                    res = res.get(key)
                click.echo(res)
...
def search_expr(ctx, expr, model, type):
    ...
    click.echo('Search for the expression <%s> on the views of the models %s types %s' % (expr, model, type))
    odoo = ctx.obj['action_login']()
    echo = ctx.obj['echo']
    xml_id = ctx.obj['xml_id']
    View = odoo.env['ir.ui.view']
...
def prompt(ctx, model, fields, domain, limit, order, slug, ou, edit):
    ...
    domain = __normalize_domain(ctx, domain, ou, slug)
    click.echo('Edit the data of the model %s ' % model)
    odoo = ctx.obj['action_login']()
    echo = ctx.obj['echo']
    Model = odoo.env[model]
...
def view(ctx, model, show, duplicates, fields, buttons, pages, xpath, view_id, view_type, expr, attrs):
    ...
    if expr:
        click.secho('', fg='blue')
        click.secho('Execute XPATH', fg='blue')
...
def __qweb(ctx, xmlid, show, divs, xpath, expr, first_class, last_class, all_classes):
    ...
    click.echo('Inspect the sweb with xmlid=%s' % (xmlid,))
    ctx.obj['action_login']()
    echo = ctx.obj['echo']
    object_from_xml_id = ctx.obj['object_from_xml_id']
...
def yaml_eval(value, context, before_eval=False):
    ...
    for v_value in values1:
        ...
        try:
            ...
            click.secho('Can not process %s in the context %s' % (v_value, tmp_ctx), fg='red')
            sys.exit(-1)
...
def __yaml_load(ctx, blocks):
    ...
    if workers <= 1:
        odoo = ctx.obj['action_login']()
        __yaml_load_item(ctx, odoo, blocks)
    else:
        threads = []
        for i in range(workers):
            t = threading.Thread(target=__yaml_load_item, args=(ctx, blocks))
            threads.append(t)
            t.start()
        for t in threads:
            t.join()
...
def __yaml_load_item(ctx, blocks):
    odoo = ctx.obj['get_new_odoo']()
