...
@cli.command()
@click.argument('arg', type=click.STRING, required=False, default="Section")
@click.pass_context
def select(ctx, arg):
    """Select a section"""
    echo = ctx.obj['echo']
    ...
    for index, section in enumerate(config.sections()):
        ...
        if arg and (section == arg or kwargs.get('database', '') == arg):
            __use_section(ctx, arg)
            return True
...
def search_expr(ctx, expr, model, type):
    ...
    x = PrettyTable()
    x.field_names = ["ID", "View name", "View Type", "Model", "XML-ID"]
    for f in x.field_names:
        x.align[f] = 'l'
    for view in views:
        x.add_row([view.id, view.name, view.type, view.model or '', xml_id(view)])
    echo(x)
