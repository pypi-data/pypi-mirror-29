...
def main():
    ...
    print('Конфигурационный файл - %s' % os.path.normpath(config))
    if not os.path.exists(config):
        print('Файл %s не существует' % os.path.normpath(config))
    else:
        cfg = import_from_path(config)
        timezone = pytz.timezone(getattr(cfg, 'TIMEZONE', 'UTC'))
        print('Временная зона - %s' % timezone)
        ...
        dsn = "host='%(db_host)s' port=%(db_port)d dbname='%(db_name)s' user='%(db_user)s'" % {
            'db_host': cfg.DB_HOST,
            'db_port': cfg.DB_PORT,
            'db_name': cfg.DB_NAME,
            'db_user': cfg.DB_USER,
        }
        print('Подключение %s' % dsn)
        dsn += 'password=%(db_password)s' % {'db_password': cfg.DB_PASSWORD}
        report = monitor.get_lock_report(dsn, duration, tz=timezone)
        print(report or 'Блокировки продолжительностью более %s не зафиксированы' % duration)
        if report:
