...
from requestWuWien import RequestWuWien
...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        tk.Entry(self, textvariable=self.v6).grid(row=7, column=1)
        tk.Label(self, text="ASPP wert:").grid(row=7, column=0, sticky="W")


        self.v7 = tk.StringVar(self)
        self.v7.set("Prüfung")  # default value
        tk.OptionMenu(self, self.v7, "Prüfung", "Lehrveranstaltung").grid(row=8, column=1)



        f = Separator(self, orient="horizontal")
        f.grid(row=9, column=0, sticky="we", padx=5, pady=5, columnspan=2)

        tk.Button(self, text="Start", command=lambda: self.anmelden(controll=controller)).grid(row=10, column=0)
        tk.Button(self, text="Quit", command= lambda: self.quitter(controll=controller)).grid(row=10, column=1)

    def quitter(self, controll):
        if controll.requester:
            controll.requester.cancelTimer()
        exit(0)

    def anmelden(self, controll):
        username = str(self.v1.get())
        password = str(self.v2.get())
        anmeldeFensterTag = str(self.v3.get())
        anmeldeSpan = str(self.v4.get())
        uhrzeit = str(self.v5.get())

        hours = int(uhrzeit.split(":")[0])
        minutes = int(uhrzeit.split(":")[1])
        asppvalue = str(self.v6.get())
        poolCount = 3
        if str(self.v7.get()) == "Prüfung":
            isPRF = True
        else:
            isPRF = False
        ...
        print("Tag: " + anmeldeFensterTag)
        print("Span: " + anmeldeSpan)
        print("ASPP: " + asppvalue)
        print("Uhrzeit: " + str(hours) + ':' + str(minutes))
        print("PRF: " + str(isPRF))

        controll.requester = RequestWuWien(hours=hours, minutes=minutes, user=username, passw=password,
                                  tagLoginWindow=anmeldeFensterTag, tagLogin=anmeldeSpan, isPRF=isPRF,
                                  requesterCount=poolCount, aspp=asppvalue)
        controll.requester.setController(controll)
        controll.requester.requestCookie()
        controll.requester.login()
...
class LoadingPage(tk.Frame):
    ...
    def stopLoading(self, controll):
        #print("Controller: " + str(controll.requester))
        if controll.requester:
            print("---------> Cancelled all <-----------")
            controll.requester.cancelTimer()
            controll.requester.finish = True
            controll.requester.parser.stopPool()
