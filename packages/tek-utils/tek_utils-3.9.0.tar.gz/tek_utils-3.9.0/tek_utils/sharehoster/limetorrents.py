...
from amino import do, Do, Maybe, List, Nil, Lists, Boolean, Regex, Try, Just
...
@do(Maybe[int])
def parse_size(size_str: str) -> Do:
...
@do(Maybe[dict])
def parse_row(row: etree.Element) -> Do:
    texts = cell_texts(row)
    title, size_str, seeders = yield texts.lift_all(0, 2, 3)
...
@do(Maybe[List[dict]])
def parse(tree: etree.Element) -> Do:
